...
if __name__ == "__main__":
    ...
    if term is None:
        print(
            "WARNING: The TERM environment variable is not defined. The curses-based menuconfig "
            "will probably fail to run. Please consult the documentation of your terminal to set it up."
        )
    elif term.endswith("256color") or term in ["alacritty"]:
        print('TERM environment variable is set to "{}"'.format(term))
    else:
        print(
            "WARNING: Menuconfig may fail because of the TERM environment variable is set "
