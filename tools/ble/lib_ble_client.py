...
class BLE_Bluez_Client:
        ...
        try:
            ...
            for path, interfaces in dbus_objs.items():
                ...
                if adapter is not None and path.endswith(self.hci_iface):
                    self.adapter.iface = dbus.Interface(
                        self.bus.get_object(BLUEZ_SERVICE_NAME, path), ADAPTER_IFACE
                    )
                    self.adapter.props = dbus.Interface(
            ...
            for cnt in range(10, 0, -1):
                ...
                if powered_on == 1:
                    ...
                    self.adapter.props = dbus.Interface(
                        self.bus.get_object(BLUEZ_SERVICE_NAME, self.adapter.path),
                        DBUS_PROP_IFACE,
                    )
    ...
    def connect(self, devname=None, devaddr=None):
        ...
        try:
            ...
            for cnt in range(10, 0, -1):
                ...
                time.sleep(5)
                device_found = self.get_device(devname=devname, devaddr=devaddr)
                if device_found:
                    break
                # Retry
                print("number of retries left ({})".format(cnt - 1))
            ...
            print(
                "connecting to device {} [ {} ] ".format(
                    self.device.name, self.device.addr
                )
            )
            ...
            for cnt in range(10, 0, -1):
                ...
                if connected == 1:
                    ...
                    self.device.props = dbus.Interface(
                        self.bus.get_object(BLUEZ_SERVICE_NAME, self.device.path),
                        DBUS_PROP_IFACE,
                    )
            ...
            try:
                if start_discovery:
                    ...
                    for cnt in range(10, 0, -1):
                        time.sleep(5)
                        discovery_val = self.adapter.props.Get(
                ...
                print(
                    "Warning: Failure during cleanup for device connection : {}".format(
                        dbus_err
                    )
                )
    ...
    def get_device(self, devname=None, devaddr=None):
        ...
        print(
            "checking if expected device {} [ {} ] is present".format(devname, devaddr)
        )
        ...
        # Set device details
        self.device.iface = dbus.Interface(
            self.bus.get_object(BLUEZ_SERVICE_NAME, dev_path), DEVICE_IFACE
        )
        self.device.props = dbus.Interface(
            self.bus.get_object(BLUEZ_SERVICE_NAME, dev_path), DBUS_PROP_IFACE
        )
    ...
    def get_chars(self):
        ...
        try:
            ...
            for service in self.device.services:
                ...
                if not char_found:
                    print(
                        "Characteristic not found for service: {}".format(service.iface)
                    )
    ...
    def get_char_if_exists(self, char_uuid):
        ...
        try:
            ...
            raise Exception(
                "Failed to get char based on uuid {} - {}".format(char_uuid, err)
            )
    ...
    def get_service_if_exists(self, service_uuid):
        try:
            ...
            raise Exception(
                "Failed to get service based on uuid {} - {}".format(service_uuid, err)
            )
    ...
    def start_notify(self, char):
        try:
            ...
            try:
                ...
                print(
                    "Warning: Failure during cleanup for start notify : {}".format(
                        dbus_err
                    )
                )
    ...
    def register_gatt_app(self):
        ...
        try:
            ...
            raise DBusException(
                "Failed to create GATT Application : {}".format(dbus_err)
            )
        except Exception as err:
    ...
    def check_le_iface(self):
        ...
        try:
            ...
            raise Exception(
                "Failed to find LEAdvertisingManager1 interface: {}".format(err)
            )
    ...
    def register_adv(self, adv_host_name, adv_type, adv_uuid):
        try:
            ...
            leadv_obj = lib_gap.Advertisement(
                self.bus, adv_iface_index, adv_type, adv_uuid, adv_host_name
            )
            print("Advertisement registered")

            # Register Advertisement
            leadv_mgr_iface_obj = dbus.Interface(
                self.bus.get_object(BLUEZ_SERVICE_NAME, le_iface_path),
            ...
            raise DBusException(
                "Failure during registering advertisement : {}".format(dbus_err)
            )
        except Exception as err:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "Warning: Failed to remove connection from dbus for advertisement object: {} - {}".format(
                            leadv_obj, err
                        )
                    )
                ...
                try:
                    ...
                    print(
                        "Warning: Failed to remove connection from dbus for Gatt application object: {} - {}".format(
                            self.gatt_app, err
                        )
                    )
                ...
                print(
                    "Warning: Failure during cleanup of Advertisement: {}".format(err)
                )
    ...
    def check_adv(self):
        ...
        try:
            ...
            if (
                self.gatt_app.service.get_char_status(
                    lib_gatt.CHAR_UUIDS["SUPPORT_NEW_ALERT_UUID"], "read"
                )
                and self.gatt_app.service.get_char_status(
                    lib_gatt.CHAR_UUIDS["ALERT_NOTIF_UUID"], "write"
                )
                and self.gatt_app.service.get_char_status(
                    lib_gatt.CHAR_UUIDS["UNREAD_ALERT_STATUS_UUID"], "notify"
                )
            ):
            ...
            self.loop_cnt += 1
            print(
                "Check read/write/subscribe events are received...Retry {}".format(
                    self.loop_cnt
                )
            )
