...
sys.path.append(
    path.expandvars(path.join("$IDF_PATH", "tools", "test_apps", "system", "panic"))
)
...
def get_line_number(lookup: str, offset: int = 0) -> int:
    src_file = path.join(
        path.dirname(path.abspath(__file__)), "main", "test_app_main.c"
    )
...
def run_and_break(dut: PanicTestDut, cmd: str) -> dict[Any, Any]:
    ...
    if not dut.find_gdb_response(
        "stopped", "notify", responses
...
def test_gdbstub_runtime(dut: PanicTestDut) -> None:
    ...
    assert payload["frame"]["line"] == str(
        get_line_number("label_3:", 1)
        if dut.is_xtensa
        else get_line_number("foo();", 0)
    )
    assert payload["frame"]["func"] == "app_main"
    assert payload["stopped-threads"] == "all"
...
@pytest.mark.temp_skip_ci(
    targets=["esp32", "esp32s2", "esp32s3"], reason="fix IDF-7927"
)
