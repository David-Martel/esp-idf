...
def test_app_mmu_page_size_32k_and_bootloader_mmu_page_size_64k(
    ...
    path_to_mmu_page_size_64k_build = os.path.join(
        dut.app.app_path, f"build_{dut.target}_{app_config}"
    )
    if app_downloader:
...
def test_app_mmu_page_size_64k_and_bootloader_mmu_page_size_32k(
    dut: Dut, app_downloader, config
) -> None:  # type: ignore
    dut.expect("App is running")
    ...
    path_to_mmu_page_size_32k_build = os.path.join(
        dut.app.app_path, f"build_{dut.target}_{app_config}"
    )
    if app_downloader:
        app_downloader.download_app(
