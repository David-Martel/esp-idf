...
CONFIGS_EXTRAM_STACK = [
    pytest.param(
        "coredump_flash_extram_stack_heap_esp32",
        marks=[pytest.mark.esp32, pytest.mark.psram],
    ),
    pytest.param(
        "coredump_flash_extram_stack_heap_esp32s2",
        marks=[pytest.mark.esp32s2, pytest.mark.generic],
    ),
    pytest.param(
        "coredump_flash_extram_stack_heap_esp32s3",
        marks=[pytest.mark.esp32s3, pytest.mark.quad_psram],
    ),
    pytest.param(
        "coredump_flash_extram_stack_bss_esp32",
        marks=[pytest.mark.esp32, pytest.mark.psram],
    ),
    pytest.param(
        "coredump_flash_extram_stack_bss_esp32s2",
        marks=[pytest.mark.esp32s2, pytest.mark.generic],
    ),
    pytest.param(
        "coredump_flash_extram_stack_bss_esp32s3",
        marks=[pytest.mark.esp32s3, pytest.mark.quad_psram],
    ),
]
...
CONFIG_COREDUMP_SUMMARY_FLASH_ENCRYPTED = [
    pytest.param(
        "coredump_flash_encrypted", marks=[pytest.mark.esp32, pytest.mark.esp32c3]
    ),
    pytest.param(
        "coredump_flash_encrypted_coredump_plain",
        marks=[pytest.mark.esp32, pytest.mark.esp32c3],
    ),
]
...
def test_panic_extram_stack(dut: PanicTestDut, config: str) -> None:
    ...
    if dut.target == "esp32":
        ...
        coredump_pattern = re.compile(
            ".coredump.tasks.data (0x3[fF][8-9a-bA-B][0-9a-fA-F]{5}) (0x[a-fA-F0-9]+) RW"
        )
        ...
        coredump_pattern = re.compile(
            ".coredump.tasks.data (0x3[c-dC-D][0-9a-fA-F]{6}) (0x[a-fA-F0-9]+) RW"
        )

    common_test(
        dut, config, expected_backtrace=None, expected_coredump=[coredump_pattern]
    )
...
def test_cache_error(dut: PanicTestDut, config: str, test_func_name: str) -> None:
    ...
    if dut.target in ["esp32c3", "esp32c2"]:
        ...
        dut.expect_exact(
            "Cached memory region accessed while ibus or cache is disabled"
        )
    ...
    common_test(
        ...
        config,
        expected_backtrace=expected_backtrace,
        check_cpu_reset=(dut.target != "esp32"),
    )
...
def test_instr_fetch_prohibited(
    ...
    if dut.is_xtensa:
        ...
        dut.expect_reg_dump(0)
        dut.expect_backtrace()
        expected_backtrace = ["_invalid_pc_placeholder"] + get_default_backtrace(
            test_func_name
        )
    else:
...
def check_x_prohibited(
    dut: PanicTestDut, config: str, test_func_name: str, operation: str
) -> None:
    dut.run_test_func(test_func_name)
    if dut.is_xtensa:
...
def test_abort(dut: PanicTestDut, config: str, test_func_name: str) -> None:
    ...
    common_test(
        dut,
        config,
        expected_backtrace=["panic_abort", "esp_system_abort", "abort"]
...
def test_assert(dut: PanicTestDut, config: str, test_func_name: str) -> None:
    ...
    coredump_pattern = re.compile(
        PANIC_ABORT_PREFIX + regex_pattern.decode("utf-8"), re.MULTILINE
    )
    common_test(
        dut,
        config,
        expected_backtrace=["panic_abort", "esp_system_abort", "__assert_func"]
        + get_default_backtrace(test_func_name),
        expected_coredump=[coredump_pattern],
    )
...
@pytest.mark.generic
@pytest.mark.xfail(
    'config.getvalue("target") == "esp32s2"',
    reason="Incorrect panic reason may be observed",
    run=False,
)
...
@pytest.mark.xfail(
    'config.getvalue("target") == "esp32s2"',
    reason="Multiple panic reasons for the same test may surface",
    run=False,
)
...
@pytest.mark.generic
def test_rtc_fast_reg1_execute_violation(
    dut: PanicTestDut, test_func_name: str
) -> None:
    dut.run_test_func(test_func_name)
    dut.expect_exact(r"Test error: Test function has returned")
    dut.expect_cpu_reset()
...
def test_rtc_fast_reg3_execute_violation(
    dut: PanicTestDut, test_func_name: str
) -> None:
    dut.run_test_func(test_func_name)
...
@pytest.mark.generic
def test_rtc_slow_reg1_execute_violation(
    dut: PanicTestDut, test_func_name: str
) -> None:
    dut.run_test_func(test_func_name)
    dut.expect_gme("Memory protection fault")
...
@pytest.mark.parametrize("config", CONFIGS_MEMPROT_RTC_SLOW_MEM, indirect=True)
@pytest.mark.generic
def test_rtc_slow_reg2_execute_violation(
    dut: PanicTestDut, test_func_name: str
) -> None:
    dut.run_test_func(test_func_name)
    dut.expect_gme("Memory protection fault")
...
@pytest.mark.parametrize(
    "config", CONFIGS_MEMPROT_INVALID_REGION_PROTECTION_USING_PMA, indirect=True
)
@pytest.mark.generic
def test_invalid_memory_region_write_violation(
    dut: PanicTestDut, test_func_name: str
) -> None:
    dut.run_test_func(test_func_name)
    dut.expect_gme("Store access fault")
...
@pytest.mark.generic
def test_invalid_memory_region_execute_violation(
    dut: PanicTestDut, test_func_name: str
) -> None:
    dut.run_test_func(test_func_name)
    dut.expect_gme("Instruction access fault")
...
def test_hw_stack_guard_cpu(dut: PanicTestDut, cpu: int) -> None:
    dut.expect_exact(
        f"Guru Meditation Error: Core  {cpu} panic'ed (Stack protection fault)."
    )
...
def test_capture_dram(dut: PanicTestDut, config: str, test_func_name: str) -> None:
    ...
    assert re.search(
        r'0x[0-9a-fA-F]+ "Coredump Test"', dut.gdb_data_eval_expr("g_heap_ptr")
    )
...
def _test_coredump_summary(
    dut: PanicTestDut, flash_encrypted: bool, coredump_encrypted: bool
) -> None:
    ...
    if flash_encrypted and not coredump_encrypted:
        dut.expect_exact(
            "Flash encryption enabled in hardware and core dump partition is not encrypted!"
        )
        return
    ...
    dut.expect(
        PANIC_ABORT_PREFIX + r"assert failed:[\s\w()]*?\s[.\w/]*\.(?:c|cpp|h|hpp):\d.*$"
    )
...
@pytest.mark.parametrize(
    "config", CONFIG_COREDUMP_SUMMARY_FLASH_ENCRYPTED, indirect=True
)
...
@pytest.mark.parametrize(
    "config", [pytest.param("coredump_flash_elf_sha", marks=TARGETS_ALL)], indirect=True
)
@pytest.mark.generic
def test_tcb_corrupted(
    dut: PanicTestDut, target: str, config: str, test_func_name: str
) -> None:
    dut.run_test_func(test_func_name)
    if dut.is_xtensa:
    ...
    coredump_pattern = [
        re.compile(pattern.decode("utf-8")) for pattern in regex_patterns
    ]
...
@pytest.mark.generic
def test_panic_print_backtrace(
    dut: PanicTestDut, config: str, test_func_name: str
) -> None:
    dut.run_test_func(test_func_name)
    regex_pattern = rb"abort\(\) was called at PC [0-9xa-f]+ on core 0"
