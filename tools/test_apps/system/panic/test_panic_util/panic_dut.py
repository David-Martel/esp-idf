...
class PanicTestDut(IdfDut):
    ...
    def expect_backtrace(self, corrupted: bool = False) -> None:
        match = self.expect(
            r"Backtrace:( 0x[0-9a-fA-F]{8}:0x[0-9a-fA-F]{8})+(?P<corrupted> \|<-CORRUPTED)?"
        )
        if corrupted:
    ...
    def expect_coredump(
        self, output_file_name: str, patterns: List[Union[str, re.Pattern]]
    ) -> None:
        with open(output_file_name, "r") as file:
            ...
            for pattern in patterns:
                if isinstance(pattern, str):
                    ...
                    raise ValueError(
                        f"Unsupported input type: {type(pattern).__name__}"
                    )
    ...
    def _call_espcoredump(self, extra_args: List[str], output_file_name: str) -> None:
        ...
        try:
            subprocess.check_call(
                espcoredump_args,
                stdout=self.coredump_output,
                stderr=self.coredump_output,
            )
    ...
    def process_coredump_flash(
        self, expected: Optional[List[Union[str, re.Pattern]]] = None
    ) -> Any:
        ...
        self._call_espcoredump(
            ["--core-format", "raw", "--save-core", coredump_file_name],
            output_file_name,
        )
        if expected:
            self.expect_coredump(output_file_name, expected)
    ...
    def run_gdb(self) -> None:
        ...
        logging.info(
            f'Running command: "{" ".join(quote_string(c) for c in gdb_command)}"'
        )
        for _ in range(10):
            try:
