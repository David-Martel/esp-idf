...
@pytest.mark.parametrize(
    "embedded_services, skip_autoflash, erase_all",
    [
        ("esp,idf,jtag", "y", "y"),
    ],
    indirect=True,
...
def test_loadable_elf(dut: IdfDut, offset: str) -> None:
    ...
    assert 'Thread 2 "main" hit Temporary breakpoint 2, app_main ()' in dut.gdb.write(
        "c"
    )
    dut.gdb.write("tb esp_restart")
    assert (
        'Thread 2 "main" hit Temporary breakpoint 3, esp_restart ()'
        in dut.gdb.write("c")
    )
