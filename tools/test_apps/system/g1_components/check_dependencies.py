...
def parse_dependencies(file_path: str) -> Tuple[Dict[str, List[str]], List[str]]:
                ...
                if len(parts) >= 2:
                    ...
                    if source in g1_g0_components and target not in g1_g0_components:
                        if target in expected_dep_violations[source]:
                            logging.debug(
                                f"Removing dependency {target} from {source} in list of expected violations"
                            )
    ...
    false_dependencies = {
        k: v for k, v in expected_dep_violations.items() if len(v) > 0
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="The path to the component_deps.dot file",
    )

    args = parser.parse_args()

    (false_dependencies, new_dependency_errors) = parse_dependencies(
        args.component_deps_file
    )
