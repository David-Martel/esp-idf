...
def validate_directory_structure(
    base_path: str, expected_structure: FileStructure
) -> None:
    """
...
def test_examples_fatfsgen(config: str, dut: Dut) -> None:
    ...
    # 30 days by default,
    def check_dates(
        date_reference: datetime, date_actual: datetime, days_date_tolerance: int = 30
    ) -> None:
    ...
    stat_filename = (
        "sublongnames/testlongfilenames.txt" if config_long_names else "sub/test.txt"
    )
    ...
    if config_long_names:
        run(
            ["python", fatfs_parser_path, "--long-name-support", "temp.img"],
            check=False, stderr=STDOUT,
        )
    else:
        run(["python", fatfs_parser_path, "temp.img"], check=False, stderr=STDOUT)
    ...
    file_structure: FileStructure = (
        long_names_read_only if config_long_names else short_names_read_only
    )
