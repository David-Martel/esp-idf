...
def corrupt_signature(
    ...
    new_blocks = tuple(
        corrupt_sig_block(s, seed, corrupt_sig, corrupt_crc) for s in sig_blocks
    )
    ...
    corr_sig_blocks = tuple(
        new_blocks[n] if corrupt_block in [CORRUPT_ALL_BLOCKS, n] else sig_blocks[n]
        for n in range(3)
    )
...
def dut_start_secure_app(dut: Dut) -> None:
    ...
    dut.serial.bootloader_flash(
        os.path.join(dut.app.binary_path, "bootloader/bootloader.bin")
    )
    dut.serial.partition_table_flash(
        os.path.join(dut.app.binary_path, "partition_table/partition-table.bin")
    )
...
def test_examples_security_secure_boot_corrupt_app_sig(dut: Dut) -> None:
    ...
    dut.expect(
        "Sig block 0 invalid: {}".format(
            "CRC mismatch"
            if dut.target == "esp32p4" or dut.target == "esp32c5"
            else "Stored CRC ends"
        ),
        timeout=2,
    )
