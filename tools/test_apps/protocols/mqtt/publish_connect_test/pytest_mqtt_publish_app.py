...
def get_scenarios() -> List[Dict[str, int]]:
    ...
    for i in count(0):
        ...
        if os.getenv(env_dict["len"]) and os.getenv(env_dict["repeat"]):
            scenarios.append(
                {var: int(os.getenv(env_dict[var])) for var in ["len", "repeat"]}
            )  # type: ignore
            continue
        break
...
def run_publish_test_case(dut: Dut, config: Any) -> None:
    ...
    with MqttPublisher(config) as publisher, connected_and_subscribed(dut):
        assert publisher.event_client_subscribed.wait(timeout=config["test_timeout"]), (
...
@pytest.mark.parametrize(
    "test_case", make_cases(local_broker_supported_transports, local_broker_scenarios)
)
