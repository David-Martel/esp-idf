...
class MQTTHandler(socketserver.StreamRequestHandler):
    def handle(self) -> None:
        ...
        if message[0:16] == "101800044d515454":
            ...
            raise Exception(
                " - error process_mqtt_connect unexpected connect received: {}".format(
                    message
                )
            )
...
class TlsServer(socketserver.TCPServer):
    ...
    def __init__(
        ...
        ServerHandler: Callable[
            [Any, Any, Any], socketserver.BaseRequestHandler
        ] = MQTTHandler,
        ...
        self.context.load_cert_chain(
            certfile=_path("srv.crt"), keyfile=_path("server.key")
        )
        if use_alpn:
...
def get_test_cases(dut: Dut) -> Any:
    ...
    try:
        ...
        logging.error(
            "ENV_TEST_FAILURE: Some mandatory CONNECTION test case not found in sdkconfig"
        )
        raise
    return cases
    ...
    try:
        ...
        for case in [
            ...
            with TlsServer(), connect_dut(dut, uri, cases[case]):
                logging.info(
                    f"Running {case}: default server - expect to connect normally"
                )
            ...
            with TlsServer(refuse_connection=True), connect_dut(dut, uri, cases[case]):
                logging.info(
                    f"Running {case}: ssl shall connect, but mqtt sends connect refusal"
                )
                dut.expect(f"MQTT_EVENT_ERROR: Test={cases[case]}", timeout=30)
                dut.expect(
                    "MQTT ERROR: 0x5"
                )  # expecting 0x5 ... connection not authorized error
            with (
                TlsServer(client_cert=True) as server,
                connect_dut(dut, uri, cases[case]),
            ):
                logging.info(
        ...
        for case in [
            ...
            with TlsServer(client_cert=True), connect_dut(dut, uri, cases[case]):
                logging.info(
                    f"Running {case}: server with client verification - expect to connect normally"
                )
                dut.expect(f"MQTT_EVENT_CONNECTED: Test={cases[case]}", timeout=30)
        ...
        with TlsServer() as s, connect_dut(dut, uri, cases[case]):
            logging.info(
                f"Running {case}: invalid server certificate on default server - expect ssl handshake error"
            )
            dut.expect(f"MQTT_EVENT_ERROR: Test={cases[case]}", timeout=30)
            dut.expect(
                "ESP-TLS ERROR: ESP_ERR_MBEDTLS_SSL_HANDSHAKE_FAILED"
            ...
            if re.match(".*alert.*unknown.*ca", s.last_ssl_error(), flags=re.I) is None:
                raise Exception(
                    f"Unexpected ssl error from the server: {s.last_ssl_error()}"
                )
        ...
        with TlsServer(client_cert=True) as s, connect_dut(dut, uri, cases[case]):
            ...
            if "CERTIFICATE_VERIFY_FAILED" not in s.last_ssl_error():
                raise Exception(
                    "Unexpected ssl error from the server {}".format(s.last_ssl_error())
                )
        ...
        for case in [
            ...
            with TlsServer(use_alpn=True) as s, connect_dut(dut, uri, cases[case]):
                logging.info(
                    f"Running {case}: server with alpn - expect connect, check resolved protocol"
                )
                dut.expect(f"MQTT_EVENT_CONNECTED: Test={cases[case]}", timeout=30)
                if case == "EXAMPLE_CONNECT_CASE_NO_CERT":
                    ...
                    assert False, (
                        f"Unexpected negotiated protocol {s.get_negotiated_protocol()}"
                    )
    finally:
        dut.write("stop")
        dut.write("destroy")
