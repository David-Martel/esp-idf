...
class KconfigWriter:
    ...
    def add_entry(self, name, entry_type, value):  # type: (str, str, typing.Any) -> None
        ...
        entry = self.KCONFIG_ENTRY_TEMPLATE.substitute(
            name=name, entry_type=entry_type, value=value
        )
    ...
    def add_source(self, source_path, condition):  # type: (str, str) -> None
        self.kconfig_text.write("\n\n")
        if condition:
            source = self.KCONFIG_CONDITION_SOURCE_TEMPLATE.substitute(
                source_path=source_path, condition=condition
            )
...
def parse_include(inc_line):  # type: (str) -> typing.Any[typing.Type[ParserElement]]
    ...
    condition = (
        CaselessLiteral("condition")
        + Optional(condition_deli)
...
def parse_define(define_line):  # type: (str) -> typing.Any[typing.Type[ParserElement]]
    ...
    int_value = (
        Combine(Optional("-") + Word(nums))("int_value")
        + ~Char(".")
...
def generate_defines(soc_caps_dir, filename, always_write):  # type: (Path, str, bool) -> bool
    ...
    for line in defines:
        try:
            ...
            if (
                "inc_path" in inc
                and "recursive" in inc
                and inc["inc_path"][-6:] == "caps.h"
            ):
                source_path = path.join(
                    "$IDF_PATH",
                    str(soc_caps_dir),
                    path.dirname(inc["inc_path"]),
                    "Kconfig.soc_caps.in",
                )
    ...
    updated = writer.update_file(
        Path(soc_caps_dir) / "Kconfig.soc_caps.in", always_write
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        "--dir",
        help="SoC caps folder paths, support wildcards",
        nargs="+",
        default=[],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--always-write", help="Always generate new output files", action="store_true"
    )
    ...
    for caps_dir in args.dir:
        ...
        files_updated += [
            generate_defines(d, args.filename, args.always_write)
            for d in soc_caps_dirs
            if path.isdir(d)
        ]
