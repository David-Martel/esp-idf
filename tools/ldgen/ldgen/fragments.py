...
class Sections(Fragment):
    ...
    ENTRY = (
        Combine(Word(alphas + "_.", alphanums + "._-") + Opt("+"))
        + LineEnd().suppress()
    )
    ...
    def __eq__(self, other):
        if isinstance(other, Flag):
            if (
                ...
                and len(self.flags) == len(other.flags)
            ):
...
class Mapping(Fragment):
    ...
    ENTRY_WITH_FLAG = (
        _entry + Suppress(";") + delimited_list(Flag.FLAG.set_parse_action(Flag.parse))
    )
    ...
    def parse_archive(s, loc, toks):
        ...
        if len(this) != 1:
            raise ParseFatalException(
                s, loc, "Could only specify one archive file in one mapping fragment"
            )
...
def parse_fragment_file(path, sdkconfig):
    ...
    def get_conditional_stmt(_stmt):
        ...
        return Group(if_ + Opt(OneOrMore(elif_)) + Opt(else_)).set_parse_action(
            parse_conditional
        )
    ...
    section = Group(
        section_header + Suppress("entries:") + section_entries_suite
    ).set_parse_action(Sections.parse)
    ...
    scheme = Group(
        scheme_header + Suppress("entries:") + scheme_entries_suite
    ).set_parse_action(Scheme.parse)
    # mapping
    ...
    mapping_entry_with_flag = Mapping.ENTRY_WITH_FLAG.set_parse_action(
        Mapping.parse_entry_with_flag
    )
    ...
    mapping = Group(
        ...
        + Group(Suppress("archive:") + mapping_archive_suite).set_parse_action(
            Mapping.parse_archive
        )
        + Group(Suppress("entries:") + mapping_entries_suite).set_parse_action(
            Mapping.parse_entries
        )
    ).set_parse_action(Mapping.parse)

    # highest level
    fragment = (
        section | scheme | mapping | get_conditional_stmt(section | scheme | mapping)
    )
