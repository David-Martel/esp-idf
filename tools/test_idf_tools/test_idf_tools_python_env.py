...
TOOLS_DIR = os.environ.get("IDF_TOOLS_PATH") or os.path.expanduser(
...
REQ_CORE = "- {}".format(
    os.path.join(IDF_PATH, "tools", "requirements", "requirements.core.txt")
)
REQ_GDBGUI = "- {}".format(
    os.path.join(IDF_PATH, "tools", "requirements", "requirements.gdbgui.txt")
)
...
class BasePythonInstall(unittest.TestCase):
    def run_tool(self, cmd):  # type: (List[str]) -> str
        ret = subprocess.run(
            cmd, check=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=600
        )
        decoded_output = ret.stdout.decode("utf-8", "ignore")
        with open(
            os.path.join(
                IDF_PATH, "tools", "test_idf_tools", "test_python_env_logs.txt"
            ),
            "a+",
        ) as w:
...
class TestCustomPythonPathInstall(BasePythonInstall):
    ...
    def test_default_arguments(self):  # type: () -> None
        output = self.run_idf_tools(["check-python-dependencies"])
        self.assertIn(
            f"{os.path.join(self.CUSTOM_PYTHON_DIR, PYTHON_BINARY)} doesn't exist",
            output,
        )
...
class TestCheckPythonDependencies(BasePythonInstall):
    ...
    def setUpClass(cls):  # type: () -> None
        cls.constraint_file = idf_tools.get_constraints(
            idf_tools.get_idf_version(), online=False
        )
        cls.requirement_core_file = os.path.join(
            IDF_PATH, "tools", "requirements", "requirements.core.txt"
        )
        for file_path_var in ["constraint_file", "requirement_core_file"]:
            with tempfile.NamedTemporaryFile() as f:
                setattr(cls, f"backup_{file_path_var}", f.name)
            shutil.copyfile(
                getattr(cls, file_path_var), getattr(cls, f"backup_{file_path_var}")
            )
    ...
    def test_check_python_dependencies(self):  # type: () -> None
        ...
        con_list = [
            r.replace("==", ">")
            for r in freeze_output.splitlines()
            if r.split("==")[0] in req_list
        ]
