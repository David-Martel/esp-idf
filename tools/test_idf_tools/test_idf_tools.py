...
def get_version_dict():
    ...
    """
    with open(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "tools.json")
    ) as f:
        tools_obj = json.loads(f.read())

    return dict(
        (tool["name"], tool["versions"][0]["name"]) for tool in tools_obj["tools"]
    )
...
RISCV_ELF_ARCHIVE_PATTERN = (
    ...
    + "-"
    + (
        RISCV_ELF_VERSION[len("esp-") :]
        if RISCV_ELF_VERSION.startswith("esp-")
        else RISCV_ELF_VERSION
    )
)
...
XTENSA_ELF_ARCHIVE_PATTERN = (
    ...
    + "-"
    + (
        XTENSA_ELF_VERSION[len("esp-") :]
        if XTENSA_ELF_VERSION.startswith("esp-")
        else XTENSA_ELF_VERSION
    )
)
...
class TestUsageBase(unittest.TestCase):
    ...
    def setUpClass(cls):
        with open(
            os.path.join(os.getenv("IDF_PATH"), "tools/tools.json"), "r"
        ) as json_file:
    ...
    def assert_tool_not_installed(
        self, output, tool, tool_version, tool_archive_name=None
    ):
        if tool_archive_name is None:
            tool_archive_name = tool
    ...
    def run_idf_tools_with_error(self, action, assertError=False):
        ...
        if assertError:
            self.assertTrue(
                is_error_occured, "idf_tools should fail if assertError is set"
            )
        output = output_stream.getvalue()
...
class TestUsage(TestUsageBase):
    def test_tools_for_wildcards1(self):
        ...
        self.assert_tool_not_installed(
            output, RISCV_ELF, RISCV_ELF_VERSION, RISCV_ELF_ARCHIVE_PATTERN
        )
        self.assert_tool_installed(output, RISCV_ESP_GDB, RISCV_ESP_GDB_VERSION)
        self.assert_tool_not_installed(
            output, XTENSA_ELF, XTENSA_ELF_VERSION, XTENSA_ELF_ARCHIVE_PATTERN
        )
        ...
        self.assertIn(
            "Destination: {}".format(os.path.join(self.temp_tools_dir, "dist")), output
        )
    ...
    def test_uninstall_option(self):
        ...
        output = self.run_idf_tools_with_action(
            ["--tools-json", tools_json_new, "uninstall", "--dry-run"]
        )
        self.assertIn("For removing old versions of " + test_tool_name, output)
        output = self.run_idf_tools_with_action(
            ["--tools-json", tools_json_new, "uninstall"]
        )
        self.assertIn(
            os.path.join(
                self.temp_tools_dir, "tools", test_tool_name, XTENSA_ELF_VERSION
            )
            + " was removed.",
            output,
        )
    ...
    def test_deactivate(self):
        ...
        self.assertIn(
            "export IDF_DEACTIVATE_FILE_PATH=",
            output,
            "No IDF_DEACTIVATE_FILE_PATH exported into environment",
        )
        deactivate_file = re.findall(
            r'(?:IDF_DEACTIVATE_FILE_PATH=")(.*)(?:")', output
        )[0]
        self.assertTrue(
            os.path.isfile(deactivate_file),
            "File {} was not found. ".format(deactivate_file),
        )
        self.assertNotEqual(
            os.stat(self.idf_env_json).st_size,
            0,
            "File {} is empty. ".format(deactivate_file),
        )
        ...
        for tool in self.tools_dict["tools"]:
            ...
            for version in tool["versions"]:
                ...
                always_install_and_recommended_tools.append(
                    {"name": tool["name"], "version": version["name"]}
                )
        self.run_idf_tools_with_action(["install"])
        output = self.run_idf_tools_with_action(["export"])
...
class TestUsageUnix(TestUsage):
    def test_usage_basic(self):
        ...
        for tool_version in [
            ...
            self.assertIn(
                "version installed in tools directory: " + tool_version, output
            )

        output = self.run_idf_tools_with_action(["export"])
        self.assertIn(
            "%s/tools/esp32ulp-elf/%s/esp32ulp-elf/bin"
            % (self.temp_tools_dir, ESP32ULP_VERSION),
            output,
        )
        self.assertIn(
            "%s/tools/xtensa-esp-elf/%s/xtensa-esp-elf/bin"
            % (self.temp_tools_dir, XTENSA_ELF_VERSION),
            output,
        )
        self.assertIn(
            "%s/tools/openocd-esp32/%s/openocd-esp32/bin"
            % (self.temp_tools_dir, OPENOCD_VERSION),
            output,
        )
        self.assertIn(
            "%s/tools/riscv32-esp-elf/%s/riscv32-esp-elf/bin"
            % (self.temp_tools_dir, RISCV_ELF_VERSION),
            output,
        )
        self.assertIn(
            "%s/tools/xtensa-esp-elf-gdb/%s/xtensa-esp-elf-gdb/bin"
            % (self.temp_tools_dir, XTENSA_ESP_GDB_VERSION),
            output,
        )
        self.assertIn(
            "%s/tools/riscv32-esp-elf-gdb/%s/riscv32-esp-elf-gdb/bin"
            % (self.temp_tools_dir, RISCV_ESP_GDB_VERSION),
            output,
        )
        self.assertIn(
            "%s/tools/esp-rom-elfs/%s/" % (self.temp_tools_dir, ESP_ROM_ELFS_VERSION),
            output,
        )
        ...
        tools_json_outdated = os.path.join(
            self.temp_tools_dir, "tools", "tools.outdated.json"
        )
        new_version = "zzzzzz"
        self.run_idf_tools_with_action(
            [
        ...
        output = self.run_idf_tools_with_action(
            ["--tools-json", tools_json_outdated, "list", "--outdated"]
        )
        self.assertIn(
            (
                f"{XTENSA_ELF}: version {XTENSA_ELF_VERSION} "
                f"is outdated by {new_version}"
            ),
            output,
        )
    ...
    def test_tools_for_esp32p4(self):
        ...
        for tool_version in [
            ...
            self.assertIn(
                f"version installed in tools directory: {tool_version}", output
            )

        output = self.run_idf_tools_with_action(["export"])
        self.assertIn(
            os.path.join(
                self.temp_tools_dir, "tools", OPENOCD, OPENOCD_VERSION, OPENOCD, "bin"
            ),
            output,
        )
        self.assertIn(
        ...
        self.assertIn(
            os.path.join(
                self.temp_tools_dir, "tools", ESP_ROM_ELFS, ESP_ROM_ELFS_VERSION, ""
            ),
            output,
        )
        self.assertIn(
    ...
    def test_tools_for_qemu_with_required(self):
        ...
        output = self.run_idf_tools_with_action(
            ["install", "required", "qemu-xtensa", "qemu-riscv32"]
        )
        self.assert_tool_installed(output, OPENOCD, OPENOCD_VERSION)
        self.assert_tool_installed(
            output, RISCV_ELF, RISCV_ELF_VERSION, RISCV_ELF_ARCHIVE_PATTERN
...
class TestUsageWin(TestUsage):
    def test_usage_basic_win(self):
        ...
        self.assertIn(
            os.path.join(self.temp_tools_dir, "tools", CMAKE, CMAKE_VERSION, "bin"),
            output,
        )
        self.assertIn(
            os.path.join(self.temp_tools_dir, "tools", NINJA, NINJA_VERSION), output
        )
        self.assertIn(
            os.path.join(self.temp_tools_dir, "tools", IDF_EXE, IDF_EXE_VERSION), output
        )
        self.assertIn(
            os.path.join(
                self.temp_tools_dir,
    ...
    def test_tools_for_esp32_win(self):
        ...
        self.assertNotIn(
            "version installed in tools directory: " + DFU_UTIL_VERSION, output
        )
...
class TestMaintainer(unittest.TestCase):
    ...
    def test_json_rewrite(self):
        ...
        with open(self.tools_new, "r") as f:
            json_new = f.read()
        self.assertEqual(
            json_old, json_new, "Please check 'tools/tools.new.json' to find a cause!"
        )
    ...
    def test_add_version_artifact_addition(self):
        ...
        expected_json = self.add_version_get_expected_json(
            "add_version/artifact_expected_addition.json"
        )
        with open(self.tools_new, "r") as f1:
            self.assertEqual(
                json.load(f1),
                expected_json,
                "Please check 'tools/tools.new.json' to find a cause!",
            )
    ...
    def test_add_version_checksum_addition(self):
        ...
        expected_json = self.add_version_get_expected_json(
            "add_version/checksum_expected_addition.json"
        )
        with open(self.tools_new, "r") as f1:
            self.assertEqual(
                json.load(f1),
    ...
    def test_add_version_checksum_with_override(self):
        ...
        expected_json = self.add_version_get_expected_json(
            "add_version/checksum_expected_override.json", True
        )
        with open(self.tools_new, "r") as f1:
            self.assertEqual(
                json.load(f1),
        ...
        for exec_platform in (
            ...
            with patch("sys.executable", TestArmDetection.ELF_HEADERS[exec_platform]):
                for platform in TestArmDetection.ARM_PLATFORMS:
                    self.assertEqual(
                        idf_tools.Platforms.detect_linux_arm_platform(platform),
                        exec_platform,
                    )
...
@unittest.skipUnless(
    int(os.getenv("IDF_TEST_MAY_BREAK_DEPENDENCIES", 0)) == 1,
    reason="Do not run destructive tests by default",
)
class TestSystemDependencies(TestUsageBase):
    ...
    def test_commands_when_nodeps(self):
        ...
        with open(os.devnull, "w") as devnull:
            check_call(
                ["dpkg", "--purge", "libsdl2-2.0-0"], stdout=devnull, stderr=STDOUT
            )
        ...
        self.assertIn(
            f"ERROR: tool {QEMU_XTENSA} version {QEMU_XTENSA_VERSION} is installed, but cannot be run",
            err_output,
        )
        ...
        _, err_output = self.run_idf_tools_with_error(
            ["install", "qemu-xtensa"], assertError=True
        )
        self.assertIn(
            f"ERROR: tool {QEMU_XTENSA} version {QEMU_XTENSA_VERSION} is installed, but getting error",
            err_output,
        )
