...
def main() -> None:
    ...
    if args.json:
        ...
        if args.bin:
            try:
                ...
                valid = [
                    k if all(x in v for x in ("offset", "file")) else None
        ...
        files += [
            (addr, process_json_file(f_name)) for addr, f_name in flash_dic.items()
        ]
    ...
    sys.exit(subprocess.run(cmd + files_flatten, check=False).returncode)
