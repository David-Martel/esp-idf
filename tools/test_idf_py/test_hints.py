...
try:
    ...
    print(
        (
            "This test needs to run within ESP-IDF environmnet. "
...
def run_idf(args: List[str], cwd: Path) -> str:
    ...
    proc = run(cmd + args, check=False, capture_output=True, cwd=cwd, text=True)
...
class TestHintModuleComponentRequirements(unittest.TestCase):
    def setUp(self) -> None:
        ...
        maindir.mkdir()
        (maindir / "CMakeLists.txt").write_text(
            'idf_component_register(SRCS "foo.c" REQUIRES esp_timer)'
        )
    ...
    def test_component_requirements(self) -> None:
        ...
        self.assertIn(
            'Missing "component1.h" file name found in the following component(s): component1(',
            output,
        )
        ...
        component1cmake = (
            self.projectdir / "components" / "component1" / "CMakeLists.txt"
        )
        component1cmake.write_text('idf_component_register(INCLUDE_DIRS ".")')
        output = run_idf(["app"], self.projectdir)
        self.assertIn(
            "To fix this, add component1 to PRIV_REQUIRES list of idf_component_register call",
            output,
        )
        ...
        maincmake.write_text(
            ('idf_component_register(SRCS "foo.c" REQUIRES esp_timer INCLUDE_DIRS ".")')
        )
        output = run_idf(["app"], self.projectdir)
        self.assertIn(
            "To fix this, add component1 to REQUIRES list of idf_component_register call",
            output,
        )
        ...
        (self.projectdir / "components" / "component1" / "component1.h").write_text(
            '#include "esp_psram.h"'
        )
        component1cmake.write_text(
            'idf_component_register(INCLUDE_DIRS "." PRIV_REQUIRES esp_psram)'
        )
        output = run_idf(["app"], self.projectdir)
        self.assertIn(
            "To fix this, move esp_psram from PRIV_REQUIRES into REQUIRES", output
        )
...
class TestNestedModuleComponentRequirements(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (maindir / "CMakeLists.txt").write_text(
            'idf_component_register(SRCS "component1.c" PRIV_REQUIRES esp_timer)'
        )
        ...
        maindir.mkdir()
        (maindir / "CMakeLists.txt").write_text(
            'idf_component_register(SRCS "foo.c" REQUIRES component1)'
        )
    ...
    def test_nested_component_requirements(self) -> None:
        ...
        output = run_idf(["app"], self.projectdir)
        self.assertNotIn(
            "esp_timer.h found in component esp_timer which is already in the requirements list of component1",
            output,
        )
        self.assertIn(
            "To fix this, add esp_timer to PRIV_REQUIRES list of idf_component_register call",
            output,
        )
...
class TestTrimmedModuleComponentRequirements(unittest.TestCase):
    ...
    def test_trimmed_component_requirements(self) -> None:
        output = run_idf(["app"], self.projectdir)
        self.assertIn(
            "To fix this, add esp_http_client to PRIV_REQUIRES list of idf_component_register call in",
            output,
        )
