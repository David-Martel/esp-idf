...
class TestExtensions(TestWithoutExtensions):
    def test_extension_loading(self):
        try:
            ...
            os.environ["IDF_EXTRA_ACTIONS_PATH"] = os.path.join(
                current_dir, "extra_path"
            )
            output = subprocess.check_output(
                [sys.executable, idf_py_path, "--help"], env=os.environ
            ).decode("utf-8", "ignore")
    ...
    def test_extension_execution(self):
        try:
            os.symlink(extension_path, link_path)
            os.environ["IDF_EXTRA_ACTIONS_PATH"] = ";".join(
                [os.path.join(current_dir, "extra_path")]
            )
            output = subprocess.check_output(
                [
                    sys.executable,
                    idf_py_path,
                    "--some-extension-option=awesome",
                    "test_subcommand",
                    "extra_subcommand",
                ],
                env=os.environ,
...
class TestDependencyManagement(TestWithoutExtensions):
    ...
    def test_complex_case(self):
        ...
        self.assertEqual(
            ["fullclean", "clean", "flash", "monitor"], list(result.keys())
        )
    ...
    def test_dupplicated_commands_warning(self):
        ...
        sys.stderr = sys.__stderr__
        self.assertIn(
            'WARNING: Command "clean" is found in the list of commands more than once.',
            capturedOutput.getvalue(),
        )
...
class TestDeprecations(TestWithoutExtensions):
    def test_exit_with_error_for_subcommand(self):
        try:
            subprocess.check_output(
                [sys.executable, idf_py_path, "-C%s" % current_dir, "test-2"],
                env=os.environ,
                stderr=subprocess.STDOUT,
            )
        except subprocess.CalledProcessError as e:
            self.assertIn(
                'Error: Command "test-2" is deprecated and was removed.',
    ...
    def test_deprecation_messages(self):
        ...
        self.assertIn(
            'Warning: Option "test_sub_1" is deprecated and will be removed in future versions.',
            output,
        )
        self.assertIn(
        ...
        self.assertIn(
            'Warning: Option "test_1" is deprecated and will be removed in future versions.',
            output,
        )
        self.assertIn(
        ...
        self.assertIn(
            'Warning: Option "test_3" is deprecated and will be removed in future versions.',
            output,
        )
...
class TestHelpOutput(TestWithoutExtensions):
    def action_test_idf_py(self, commands: List[str], schema: Any) -> None:
        ...
        with open(output_file, "w") as outfile:
            subprocess.run(commands, check=False, env=env, stdout=outfile)
    ...
    def test_output(self):
        with open(
            os.path.join(current_dir, "idf_py_help_schema.json"), "r"
        ) as schema_file:
...
class TestFileArgumentExpansion(TestCase):
    def test_file_expansion(self):
        ...
        try:
            ...
            self.fail(
                f"Process should have exited normally, but it exited with a return code of {e.returncode}"
            )
        ...
        try:
            output = subprocess.check_output(
                [
                    ...
                    idf_py_path,
                    "--version",
                    "@file_args_expansion_inputs/args_recursive",
                ],
                env=os.environ,
                stderr=subprocess.STDOUT,
            ).decode("utf-8", "ignore")
        ...
        with self.assertRaises(subprocess.CalledProcessError) as cm:
            subprocess.check_output(
                [
                    ...
                    idf_py_path,
                    "--version",
                    "@file_args_expansion_inputs/args_circular_a",
                ],
                env=os.environ,
                stderr=subprocess.STDOUT,
            ).decode("utf-8", "ignore")
        self.assertIn(
            "Circular dependency in file argument expansion",
            cm.exception.output.decode("utf-8", "ignore"),
        )
    ...
    def test_missing_file(self):
        ...
        self.assertIn(
            "(expansion of @args_non_existent) could not be opened",
            cm.exception.output.decode("utf-8", "ignore"),
        )
...
class TestWrapperCommands(TestCase):
    @classmethod
    def setUpClass(cls):
        cls.sample_project_dir = os.path.join(
            current_dir, "..", "test_build_system", "build_test_app"
        )
    ...
    def call_command(self, command: List[str]) -> str:
        try:
            output = subprocess.check_output(
                command, env=os.environ, stderr=subprocess.STDOUT
            ).decode("utf-8", "ignore")
            return output
        except subprocess.CalledProcessError as e:
            self.fail(
    ...
    def tearDownClass(cls):
        subprocess.run(
            [sys.executable, idf_py_path, "fullclean"], check=False, stdout=subprocess.DEVNULL
        )
...
class TestEFuseCommands(TestWrapperCommands):
    ...
    def test_efuse_summary(self):
        ...
        output = self.call_command(
            summary_command + ["--format", "value-only", "WR_DIS"]
        )
    ...
    def test_efuse_burn_key(self):
        ...
        subprocess.run(
            [
                ...
                os.path.join(current_dir, key_name),
            ],
...
class TestSecureCommands(TestWrapperCommands):
    ...
    def setUpClass(cls):
        super().setUpClass()
        subprocess.run(
            [sys.executable, idf_py_path, "build"], check=False, stdout=subprocess.DEVNULL
        )
    ...
    def secure_generate_flash_encryption_key(self):
        ...
        self.assertIn(
            f"Writing 256 random bits to key file {self.flash_encryption_key}", output
        )
    ...
    def secure_generate_signing_key(self):
        ...
        output = self.call_command(generate_key_command)
        self.assertIn(
            f"RSA 3072 private key in PEM format written to {self.signing_key}", output
        )
        self.assertIn("Done", output)
...
class TestMergeBinCommands(TestWrapperCommands):
    ...
    def test_merge_bin(self):
        ...
        self.assertIn(
            f"file {merged_binary_name}, ready to flash to offset 0x0", output
        )
        self.assertIn(
            f"Merged binary {merged_binary_name} will be created in the build directory...",
            output,
        )
