...
try:
    ...
    sys.path.append(
        os.path.expandvars(os.path.join("$IDF_PATH", "tools", "unit-test-app", "tools"))
    )
    import CreateSectionTable
...
def parse_elf_test_cases(elf_file: str, idf_target: str) -> List[Dict]:
    ...
    try:
        subprocess.check_output(
            "{} -s {} > section_table.tmp".format(objdump, elf_file), shell=True
        )
    ...
    try:
        subprocess.check_output(
            "{} -t {} | grep test_desc > case_address.tmp".format(objdump, elf_file),
            shell=True,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--idf_target",
        type=str,
        default=os.environ.get("IDF_TARGET", ""),
