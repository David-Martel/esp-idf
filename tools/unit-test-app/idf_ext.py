...
def action_extensions(base_actions, project_path=os.getcwd()):
    ...
    def ut_apply_config(ut_apply_config_name, ctx, args):
        ...
        new_cache_values = {}
        sdkconfig_set = list(
            filter(lambda s: "SDKCONFIG=" in s, args.define_cache_entry)
        )
        ...
        if config_name in CONFIG_NAMES:
            ...
            new_cache_values["EXCLUDE_COMPONENTS"] = config.get(
                "EXCLUDE_COMPONENTS", "''"
            )
            new_cache_values["TEST_EXCLUDE_COMPONENTS"] = config.get(
                "TEST_EXCLUDE_COMPONENTS", "''"
            )
            new_cache_values["TEST_COMPONENTS"] = config.get("TEST_COMPONENTS", "''")
            new_cache_values["TESTS_ALL"] = int(
                new_cache_values["TEST_COMPONENTS"] == "''"
            )
            new_cache_values["IDF_TARGET"] = target
            new_cache_values["SDKCONFIG_DEFAULTS"] = ";".join(
                [os.path.join(project_path, "sdkconfig.defaults"), config_path]
            )

            args.define_cache_entry.extend(
                ["%s=%s" % (k, v) for k, v in new_cache_values.items()]
            )
    ...
    def test_component_callback(ctx, global_args, tasks):
        ...
        if cache_entries:
            ...
            global_args.define_cache_entry.extend(
                ["%s=%s" % (k, v) for k, v in cache_entries.items()]
            )
