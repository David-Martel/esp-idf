...
def parse_log(input_path: str, output_tag: str) -> None:
    ...
    if parsed_num > 0:
        print(
            f"Log parsing completed, parsed_num {parsed_num}, all_line_num {all_line_num}.\nOutput saved to: {output_file}"
        )
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        "-p", "--path", required=True, help="Path to the input log file"
    )
    parser.add_argument(
        "-o", "--output", required=True, help="Name tag for the output file"
    )
