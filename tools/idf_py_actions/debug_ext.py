...
def action_extensions(base_actions: Dict, project_path: str) -> Dict:
    ...
    def _check_openocd_errors(
        fail_if_openocd_failed: Dict, target: str, ctx: Context
    ) -> None:
            ...
            if "openocd" in processes and processes["openocd"] is not None:
                ...
                for _ in range(5):
                    ...
                    with open(name, "r", encoding="utf-8") as f:
                        content = f.read()
                        if re.search(
                            r"Listening on port \d+ for gdb connections", content
                        ):
                ...
                raise FatalError(
                    'Action "{}" failed due to errors in OpenOCD'.format(target), ctx
                )
    ...
    def _get_espcoredump_instance(
        ...
        coredump_to_flash = (
            coredump_to_flash_config.rstrip().endswith("y")
            if coredump_to_flash_config
            else False
        )
        ...
        if core:
            ...
            espcoredump_kwargs["chip"] = get_sdkconfig_value(
                project_desc["config_file"], "CONFIG_IDF_TARGET"
            )
    ...
    def is_gdb_with_python(gdb: str) -> bool:
        ...
        return (
            subprocess.run(
                [gdb, "--batch-silent", "--ex", "python import os"],
    ...
    def openocd(
        ...
        ctx: Context,
        args: PropertyDict,
        openocd_scripts: Optional[str],
        openocd_commands: str,
    ) -> None:
        """
        ...
        openocd_arguments = (
            os.getenv("OPENOCD_COMMANDS")
            if openocd_commands is None
            else openocd_commands
        )
        project_desc = get_project_desc(args, ctx)
        ...
        try:
            process = subprocess.Popen(
                args, stdout=openocd_out, stderr=subprocess.STDOUT, bufsize=1
            )
        except Exception as e:
            print(e)
            raise FatalError(
    ...
    def get_gdb_args(
        project_desc: Dict[str, Any],
        gdb_x: Tuple,
        gdb_ex: Tuple,
        gdb_commands: Optional[str],
    ) -> List[str]:
        # check if the application was built and ELF file is in place.
        app_elf = os.path.join(
            project_desc.get("build_dir", ""), project_desc.get("app_elf", "")
        )
        ...
        if not gdbinit_files:
            raise FatalError(
                'Please check if the project was configured correctly ("gdbinit_files" not found in "project_description.json").'
            )
    ...
    def gdbui(
        ...
        if gdbgui_version >= (0, 14, 0, 0):
            ...
            gdb_args_str = (
                '"{}"'.format(" ".join(gdb_args_list))
                if len(gdb_args_list) == 1
                else " ".join(gdb_args_list)
            )
    ...
    def coredump_debug(
        ...
        espcoredump = _get_espcoredump_instance(
            ctx=ctx, args=args, core=core, chip_rev=chip_rev, save_core=save_core
        )
