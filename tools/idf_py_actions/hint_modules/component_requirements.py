...
ENOENT_RE = re.compile(
    r"^(.+):\d+:\d+: fatal error: (.+): No such file or directory$", flags=re.MULTILINE
)
...
ENOENT_FULL_RE = re.compile(
    r"^(In file included.*?No such file or directory)$", flags=re.MULTILINE | re.DOTALL
)
...
def generate_hint(output: str) -> Optional[str]:
    ...
    for component_name, component_info in proj_desc["all_component_info"].items():
        ...
        if source_filename.startswith(component_dir):
            if found_source_component_info and len(
                found_source_component_info["dir"]
            ) >= len(component_dir):
                continue
    ...
    if hint_match_full:
        ...
        if original_file_match:
            original_filename = _get_absolute_path(
                original_file_match.group(1), proj_desc["build_dir"]
            )
            for component_name, component_info in proj_desc[
                "all_component_info"
            ].items():
                component_dir = os.path.normpath(component_info["dir"])
                ...
                return _bug(
                    (
                        f"cannot find original component for source "
            ...
            return _bug(
                (
                    f"cannot match original component filename for "
    ...
    for candidate_component_name, candidate_component_info in proj_desc[
        "all_component_info"
    ].items():
        ...
        for candidate_component_include_dir in candidate_component_include_dirs:
            candidate_header_path = os.path.join(
                component_dir, candidate_component_include_dir, missing_header
            )
        ...
        for component_name, component_info in proj_desc["all_component_info"].items():
            ...
            for root, _, _ in os.walk(component_dir):
                ...
                if os.path.isfile(full_path):
                    candidate_component_include_dirs.append(
                        f"{component_name}({full_path})"
                    )

        if candidate_component_include_dirs:
            candidates = ", ".join(candidate_component_include_dirs)
            return (
                f'Missing "{missing_header}" file name found in the following component(s): {candidates}. '
    ...
    all_reqs = (
        found_source_component_info["reqs"]
    ...
    if found_original_component_name == found_source_component_name:
        ...
        # interface.
        all_reqs += (
            found_source_component_info["priv_reqs"]
    ...
    for dep_component_name in found_dep_component_names:
        if dep_component_name in all_reqs:
            ...
            return _bug(
                (
                    f"{missing_header} found in component {dep_component_name} "
    ...
    if original_filename == source_filename:
        ...
        for include_dir in found_original_component_info["include_dirs"]:
            include_dir = _get_absolute_path(
                found_original_component_info["dir"], include_dir
            )
    ...
    cmakelists_file_to_fix = os.path.normpath(
        os.path.join(found_source_component_info["dir"], "CMakeLists.txt")
    )
    problem_description = (
