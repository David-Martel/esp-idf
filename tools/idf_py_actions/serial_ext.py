...
def action_extensions(base_actions: Dict, project_path: str) -> Dict:
    ...
    def _get_esptool_args(args: PropertyDict) -> List:
        ...
        with open(
            os.path.join(args.build_dir, "flasher_args.json"), encoding="utf-8"
        ) as f:
    ...
    def merge_bin(
        ...
        if md5_disable:
            if format != "uf2":
                yellow_print(
                    "idf.py merge-bin: --md5-disable is only valid for UF2 format. Option will be ignored."
                )
            else:
        ...
        if flash_offset:
            if format != "raw":
                yellow_print(
                    "idf.py merge-bin: --flash-offset is only valid for RAW format. Option will be ignored."
                )
            else:
    ...
    def secure_generate_flash_encryption_key(
        ...
        **extra_args: str,
        ...
        ensure_build_directory(args, ctx.info_name)
        generate_flash_encryption_key_args = [
            PYTHON,
            "-m",
            "espsecure",
    ...
    def secure_generate_signing_key(
        ...
        **extra_args: str,
        ...
        project_desc = _get_project_desc(ctx, args)
        ecdsa_scheme = get_sdkconfig_value(
            project_desc["config_file"], "CONFIG_SECURE_SIGNED_APPS_ECDSA_SCHEME"
        )
        ecdsa_v2_scheme = get_sdkconfig_value(
            project_desc["config_file"], "CONFIG_SECURE_SIGNED_APPS_ECDSA_V2_SCHEME"
        )
        rsa_scheme = get_sdkconfig_value(
            project_desc["config_file"], "CONFIG_SECURE_SIGNED_APPS_RSA_SCHEME"
        )
        if ecdsa_scheme:
            version = "1"
    ...
    def _parse_efuse_args(
        ctx: click.core.Context, args: PropertyDict, extra_args: Dict
    ) -> List:
        efuse_args = []
        if args.port:
            efuse_args += ["-p", args.port]
        elif (
            not args.port and not extra_args["virt"]
        ):  # if --virt, no port will be found and it would cause error
    ...
    def efuse_burn(
        action: str, ctx: click.core.Context, args: PropertyDict, **extra_args: Dict
    ) -> None:
        ensure_build_directory(args, ctx.info_name)
    ...
    def efuse_burn_key(
        action: str, ctx: click.core.Context, args: PropertyDict, **extra_args: str
    ) -> None:
        ensure_build_directory(args, ctx.info_name)
        ...
        RunTool(
            "espefuse.py", burn_key_args, args.project_dir, build_dir=args.build_dir
        )()
    ...
    serial_actions = {
        ...
        "actions": {
            ...
            "merge-bin": {
                ...
                "options": [
                    ...
                    },
                    {
                        ...
                        "help": (
                            "[ONLY RAW] Flash offset where the output file will be flashed."
                        ),
                    },
                    {
                        "names": ["--fill-flash-size"],
                        "help": (
                            "[ONLY RAW] If set, the final binary file will be padded with FF bytes up to this flash size."
                        ),
                        "type": click.Choice(
                            [
                                "256KB",
            ...
            "secure-generate-signing-key": {
                ...
                "help": (
                    "Generate a private key for signing secure boot images as per the secure boot version. Key file is generated in PEM"
            ...
            "secure-sign-data": {
                ...
                "help": (
                    "Sign a data file for use with secure boot. Signing algorithm is deterministic ECDSA w/ SHA-512 (V1) or either RSA-"
            ...
            "secure-verify-signature": {
                ...
                "help": (
                    "Verify a previously signed binary image, using the ECDSA (V1) or either RSA or ECDSA (V2) public key."
                ),
                "options": [
                    {
                        "names": ["--version", "-v"],
                    ...
                    },
                    {
                        "names": ["--keyfile", "-k"],
                        "help": (
                            "Public key file for verification. Can be private or public key in PEM format."
                        ),
                    },
                ],
                "arguments": [
