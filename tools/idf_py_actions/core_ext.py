...
from idf_py_actions.constants import (
    GENERATORS,
    PREVIEW_TARGETS,
    SUPPORTED_TARGETS,
    URL_TO_DOC,
...
def action_extensions(base_actions: Dict, project_path: str) -> Any:
    def build_target(target_name: str, ctx: Context, args: PropertyDict) -> None:
        ...
        run_target(
            target_name,
            args,
            force_progression=GENERATORS[args.generator].get(
    ...
    def size_target(
        ...
        if not legacy and output_format != "json":
            try:
                ...
                print_warning(
                    "WARNING: refactored esp-idf-size not installed, using legacy mode"
                )
                legacy = True
            else:
        ...
        if legacy and output_format in ["json2", "raw", "tree"]:
            ...
            raise FatalError(
                f"Legacy esp-idf-size does not support {output_format} format"
            )
        ...
        if diff_map_file:
            ...
            if os.path.isdir(diff_map_file):
                ...
                if not files:
                    raise FatalError(
                        f"No diff map file found in {diff_map_file} directory"
                    )
                if len(files) > 1:
                    map_files = ", ".join(files)
                    raise FatalError(
                        f"Two or more diff map files {map_files} found in {diff_map_file} directory"
                    )
    ...
    def list_build_system_targets(
        target_name: str, ctx: Context, args: PropertyDict
    ) -> None:
        """Shows list of targets known to build system (make/ninja)"""
        build_target("help", ctx, args)

    def menuconfig(
        target_name: str, ctx: Context, args: PropertyDict, style: str
    ) -> None:
        """
    ...
    def save_defconfig(
        target_name: str, ctx: Context, args: PropertyDict, add_menu_labels: bool
    ) -> None:
        if add_menu_labels:
    ...
    def fallback_target(target_name: str, ctx: Context, args: PropertyDict) -> None:
        ...
        try:
            subprocess.check_output(
                GENERATORS[args.generator]["dry_run"] + [target_name],
                cwd=args.build_dir,
            )
            ...
            if target_name in ["clang-check", "clang-html-report"]:
                raise FatalError(
                    ...
                    'Please install it via "pip install --upgrade pyclang"'.format(
                        target_name
                    )
                )
    ...
    def set_target(
        action: str, ctx: Context, args: PropertyDict, idf_target: str
    ) -> None:
    ...
    def validate_root_options(ctx: Context, args: PropertyDict, tasks: List) -> None:
        ...
        if args.build_dir is not None and args.project_dir == os.path.realpath(
            args.build_dir
        ):
            raise FatalError(
    ...
    def show_docs(
        ...
        if not version:
            ...
            version_search = re.search(
                r"v\d+\.\d+\.?\d*(-dev|-beta\d|-rc)?", idf_version() or "0.0-dev"
            )
        ...
        try:
            ...
            print(
                "We can't check the link's functionality because you don't have an internet connection"
            )
        if redirect_link:
    ...
    def help_and_exit(
        action: str, ctx: Context, param: List, json_option: bool, add_options: bool
    ) -> None:
        if json_option:
            output_dict = {}
    ...
    size_options = [
        {
            ...
            "type": click.Choice(
                ["default", "text", "csv", "json", "json2", "tree", "raw"]
            ),
