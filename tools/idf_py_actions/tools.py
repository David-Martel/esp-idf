...
def get_default_serial_port() -> Any:
    ...
    try:
        ...
        raise FatalError(
            "An exception occurred during detection of the serial port: {}".format(e)
        )
...
def load_hints() -> Dict:
    ...
    with open(
        os.path.join(current_module_dir, "hints.yml"), "r", encoding="utf-8"
    ) as file:
    ...
    for _, name, _ in iter_modules([hint_modules_dir]):
        ...
        try:
            hints["modules"].append(
                importlib.import_module(name).generate_hint
            )
        except ModuleNotFoundError:
            red_print(
                f'Failed to import "{name}" from "{hint_modules_dir}" as a module'
            )
            raise SystemExit(1)
...
def generate_hints_buffer(output: str, hints: Dict) -> Generator:
    ...
    for hint in hints["yml"]:
        ...
        try:
                ...
                for variables in variables_list:
                    ...
                    if re.compile(regex).search(output):
                        try:
                            ...
                            red_print(
                                "Argument {} missing in {}. Check hints.yml file.".format(
                                    e, hint
                                )
                            )
                            sys.exit(1)
            ...
            red_print(
                "Argument {} missing in {}. Check hints.yml file.".format(e, hint)
            )
            sys.exit(1)
        except re.error as e:
            red_print(
                "{} from hints.yml have {} problem. Check hints.yml file.".format(
                    hint["re"], e
                )
            )
            sys.exit(1)
        if hint_list:
            ...
            extra_info = (
                ", ".join(match.groups()) if hint.get("match_to_output", "") else ""
            )
            try:
                yield " ".join(["HINT:", hint["hint"].format(extra_info)])
            except KeyError:
                raise KeyError(
                    "Argument 'hint' missing in {}. Check hints.yml file.".format(hint)
                )
...
class RunTool:
    ...
    def __call__(self) -> None:
        ...
        if self.hints:
            process, stderr_output_file, stdout_output_file = asyncio.run(
                self.run_command(self.args, env_copy)
            )
        else:
            process = subprocess.run(self.args, check=False, env=env_copy, cwd=self.cwd)
        ...
        if self.custom_error_handler:
            self.custom_error_handler(
                process.returncode, stderr_output_file, stdout_output_file
            )
            return
        ...
        raise FatalError(
            "{} failed with exit code {}".format(self.tool_name, process.returncode)
        )

    async def run_command(
        self, cmd: List, env_copy: Dict
    ) -> Tuple[Process, Optional[str], Optional[str]]:
        ...
        stderr_output_file = os.path.join(
            self.build_dir, log_dir_name, f"idf_py_stderr_output_{p.pid}"
        )
        stdout_output_file = os.path.join(
            self.build_dir, log_dir_name, f"idf_py_stdout_output_{p.pid}"
        )
        if p.stderr and p.stdout:  # it only to avoid None type in p.std
            try:
                await asyncio.gather(
                    self.read_and_write_stream(
                        p.stderr, stderr_output_file, sys.stderr
                    ),
                    self.read_and_write_stream(
                        p.stdout, stdout_output_file, sys.stdout
                    ),
                )
    ...
    async def read_and_write_stream(
        ...
        output_converter = (
            get_ansi_converter(output_stream) if self.convert_output else output_stream
        )
        ...
        is_progression_processing_enabled = (
            self.force_progression and output_stream.isatty() and "-v" not in self.args
        )
        ...
        try:
            ...
            yellow_print(
                ...
                "hints on how to resolve errors can be not accurate.".format(
                    e, output_stream.name.strip("<>")
                )
            )
...
def _strip_quotes(
    value: str, regexp: re.Pattern = re.compile(r"^\"(.*)\"$|^'(.*)'$|^(.*)$")
) -> Optional[str]:
    """
    ...
    return (
        [x for x in matching_values.groups() if x is not None][0].rstrip()
...
def ensure_build_directory(
    ...
    if args.generator is None:
        args.generator = (
            generator  # reuse the previously configured generator, if none was given
        )
    ...
    try:
        ...
        if os.path.normcase(python) != os.path.normcase(sys.executable):
            raise FatalError(
                ...
                "Run '{} fullclean' to start again.".format(
                    sys.executable, python, prog_name
                )
            )
    except KeyError:
        pass
...
def get_sdkconfig_filename(
    args: "PropertyDict", cache_cmdl: Optional[Dict] = None
) -> str:
    """
...
def _check_idf_target(
    ...
    if idf_target_from_env:
        ...
        if (
            idf_target_from_sdkconfig
            and idf_target_from_sdkconfig != idf_target_from_env
        ):
            raise FatalError(
                "Project sdkconfig '{cfg}' was generated for target '{t_conf}', but environment variable IDF_TARGET "
                "is set to '{t_env}'. Run '{prog} set-target {t_env}' to generate new sdkconfig file for target {t_env}.".format(
                    cfg=sdkconfig,
                    t_conf=idf_target_from_sdkconfig,
        ...
        if idf_target_from_cache and idf_target_from_cache != idf_target_from_env:
            raise FatalError(
                ...
                "Run '{prog} fullclean' to start again.".format(
                    t_env=idf_target_from_env,
                    t_cache=idf_target_from_cache,
                    prog=prog_name,
                )
            )

        if (
            idf_target_from_cache_cmdl
            and idf_target_from_cache_cmdl != idf_target_from_env
        ):
            raise FatalError(
        ...
        if (
            idf_target_from_cache
            and idf_target_from_cache != idf_target_from_cache_cmdl
        ):
            raise FatalError(
                "Target '{t_cmdl}' specified on command line is not consistent with "
                ...
                "CMakeCache.txt.".format(
                    t_cache=idf_target_from_cache,
                    t_cmdl=idf_target_from_cache_cmdl,
                    prog=prog_name,
                )
            )
        ...
        if (
            ...
            raise FatalError(
                "Project sdkconfig '{cfg}' was generated for target '{t_conf}', but CMakeCache.txt contains '{t_cache}'. "
                "To keep the setting in sdkconfig ({t_conf}) and re-generate CMakeCache.txt, run '{prog} fullclean'. "
                "To re-generate sdkconfig for '{t_cache}' target, run '{prog} set-target {t_cache}'.".format(
                    cfg=sdkconfig,
                    t_conf=idf_target_from_sdkconfig,
                    t_cache=idf_target_from_cache,
                    prog=prog_name,
