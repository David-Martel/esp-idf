...
class QemuTarget:
    ...
    install_package: (
        str  # name of the tools.json package from which to install the QEMU binary
    )
    qemu_args: str  # chip-specific arguments to pass to QEMU
    default_efuse: bytes  # default efuse values for the target
    boot_mode_arg: str = (
        ""  # additional arguments to pass to QEMU when booting in download mode
    )
    efuse_device: str = (
        ""  # efuse device name, if different from the target nvram.{target}.efuse
    )
...
def action_extensions(base_actions: Dict, project_path: str) -> Dict:
    ...
    def global_callback(ctx: Context, global_args: Dict, tasks: List) -> None:
        ...
        if have_qemu:
            if have_gdb and have_monitor:
                red_print(
                    "Cannot run qemu with both gdb and monitor in the same terminal"
                )
                raise SystemExit(1)
            if have_gdb:
    ...
    def qemu(
        ...
        flash_size = get_sdkconfig_value(
            project_desc["config_file"], "CONFIG_ESPTOOLPY_FLASHSIZE"
        )
        ...
        if flash_file:
            ...
            try:
                ...
                red_print(
                    f'The provided flash image file "{bin_path}" could not be found'
                )
                raise SystemExit(1)
        else:
        ...
        if not options.bg_mode:
            ...
            subprocess.run(qemu_args, check=False)
        else:
            ...
            if gdb and not options.wait_for_gdb:
                yellow_print(
                    'Waiting for GDB to connect. You can now run "idf.py gdb" in another terminal window.'
                )
