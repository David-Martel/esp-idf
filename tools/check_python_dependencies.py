...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ESP-IDF Python package dependency checker"
    )
    parser.add_argument(
        "--requirements",
        "-r",
        ...
        with open(const_path, encoding="utf-8") as f:
            for con in [
                ...
                if con.startswith("file://"):
                    ...
                    con.startswith("-e") and "#egg=" in con
    ...
    def version_check(requirement: Requirement) -> None:
        ...
        if not requirement.specifier.contains(version, prereleases=True):
            not_satisfied.append(
                f"Requirement '{requirement}' was not met. Installed version: {version}"
            )
    ...
    for req in new_req_list[:]:
        if not req.marker or req.marker.evaluate():
            try:
                ...
                not_satisfied.append(
                    f"Error while checking requirement '{req}'. Package was not found and is required by the application: {e}"
                )
            ...
            try:
                ...
                for name in requires(requirement.name) or []:
                    ...
                    for extra in extras:
                        ...
                        if not sub_req.marker or sub_req.marker.evaluate(
    ...
    if len(not_satisfied) > 0:
        ...
        if "IDF_PYTHON_ENV_PATH" in os.environ:
            ...
            print(
                'To install the missing packages, please run "{}"'.format(
                    install_script
                )
            )
        else:
