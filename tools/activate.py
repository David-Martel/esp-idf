...
idf_python_env_path, idf_python_export_path, virtualenv_python, idf_version = (
    idf_tools.get_python_env_path()
)
...
try:
    run(
        [
            virtualenv_python,
            os.path.join(idf_path, "tools", "export_utils", "activate_venv.py"),
        ]
        + sys.argv[1:],
        check=True,
        env=os.environ.copy(),
