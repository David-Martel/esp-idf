...
IDF_PIP_WHEELS_URL = os.environ.get(
    "IDF_PIP_WHEELS_URL", "https://dl.espressif.com/pypi"
)
...
def print_hints_on_download_error(err: str) -> None:
    ...
    if "CERTIFICATE" in err:
        ...
        info(
            f'Running "{sys.executable} -m pip install --upgrade certifi" can also resolve this issue in some cases.'
        )
...
class Platforms:
    ...
    def detect_linux_arm_platform(supposed_platform: Optional[str]) -> Optional[str]:
        ...
        """
        if supposed_platform not in (
            PLATFORM_LINUX_ARM32,
            PLATFORM_LINUX_ARMHF,
        ...
        with open(sys.executable, "rb") as f:
            ...
            if int.from_bytes(f.read(4), sys.byteorder) != int.from_bytes(
                b"\x7fELF", sys.byteorder
            ):
            ...
            if (
                e_machine == 183
            ):  # EM_AARCH64, https://github.com/ARM-software/abi-aa/blob/main/aaelf64/aaelf64.rst
                supposed_platform = PLATFORM_LINUX_ARM64
            elif (
                e_machine == 40
            ):  # EM_ARM, https://github.com/ARM-software/abi-aa/blob/main/aaelf32/aaelf32.rst
    ...
    def get(platform_alias: Optional[str]) -> str:
        ...
        if platform_alias in Platforms.UNSUPPORTED_PLATFORMS:
            raise ValueError(
                f"Platform '{platform_alias}' is not supported by ESP-IDF."
            )
        ...
        if not platform_name:
            raise ValueError(
                f"Support for platform '{platform_alias}' hasn't been added yet."
            )
...
DL_CERT_DICT = {
    "dl.espressif.com": DIGICERT_ROOT_G2_CERT,
...
def to_shell_specific_paths(paths_list: List[str]) -> List[str]:
    ...
    if sys.platform == "win32":
        paths_list = [
            p.replace("/", os.path.sep) if os.path.sep in p else p for p in paths_list
        ]
...
def download(url: str, destination: str) -> Union[None, Exception]:
    ...
    try:
        ...
        urlretrieve_ctx(
            url,
            destination,
            report_progress if not g.non_interactive else None,
            context=ctx,
        )
...
class IDFToolDownload(object):
    ...
    def __init__(
        self, platform_name: str, url: str, size: int, sha256: str, rename_dist: str
    ) -> None:
...
class IDFToolVersion(object):
    ...
    def __lt__(self, other: "IDFToolVersion") -> bool:
        if self.status != other.status:
            ...
            assert not (
                self.status == IDFToolVersion.STATUS_RECOMMENDED
    ...
    def add_download(
        ...
        """
        self.downloads[platform_name] = IDFToolDownload(
            platform_name, url, size, sha256, rename_dist
        )

    def get_download_for_platform(
        self, platform_name: Optional[str]
    ) -> Optional[IDFToolDownload]:
        """
    ...
    def compatible_with_platform(
        self, platform_name: Optional[str] = PYTHON_PLATFORM
    ) -> bool:
        """
...
class IDFTool(object):
    ...
    def get_version(
        ...
        return re.sub(
            self._current_options.version_regex,
            self._current_options.version_regex_replace,
            match.group(0),
        )  # type: ignore
    ...
    def check_binary_valid(self, version: str) -> bool:
        ...
        try:
            ...
            fatal(
                f"tool {self.name} version {version} is installed, but getting error: {e}"
            )
            return False
        if ver_str != version:
            # just print, state is still valid
            warn(
                f"tool {self.name} version {version} is installed, but reporting version {ver_str}"
            )
        return True
    ...
    def is_supported_for_any_of_targets(self, targets: List[str]) -> bool:
        ...
        return any(
            item in targets for item in supported_targets
        ) or supported_targets == ["all"]
    ...
    def find_installed_versions(self) -> None:
        ...
        for version, version_obj in self.versions.items():
            ...
            try:
                ...
                fatal(
                    f"tool {self.name} version {version} is installed, but cannot be run: {e}"
                )
                tool_error = True
            else:
                if ver_str != version:
                    warn(
                        f"tool {self.name} version {version} is installed, but has reported version {ver_str}"
                    )
                else:
                    self.versions_installed.append(version)
        if tool_error:
    ...
    def latest_installed_version(self) -> Optional[str]:
        ...
        for version in sorted(dirs, reverse=True):
            ...
            paths = [
                os.path.join(tool_path, version, *p)
                for p in self._current_options.export_paths
            ]
            try:
    ...
    def download(self, version: str) -> None:
        ...
        archive_name = (
            download_obj.rename_dist
            if download_obj.rename_dist
        ...
        for retry in range(DOWNLOAD_RETRY_COUNT):
            ...
            if not os.path.isfile(local_temp_path) or not self.check_download_file(
                download_obj, local_temp_path
            ):
    ...
    def install(self, version: str) -> None:
        ...
        archive_name = (
            download_obj.rename_dist
            if download_obj.rename_dist
            else os.path.basename(download_obj.url)
        )
        ...
        if self._current_options.strip_container_dirs:  # type: ignore
            do_strip_container_dirs(
                dest_dir, self._current_options.strip_container_dirs
            )  # type: ignore
        if not self.check_binary_valid(version):
            fatal(
                f"Failed to check the tool while installed. Removing directory {dest_dir}"
            )
            shutil.rmtree(dest_dir)
            raise SystemExit(1)
    ...
    def check_download_file(download_obj: IDFToolDownload, local_path: str) -> bool:
        ...
        if file_size != expected_size:
            warn(
                f"file size mismatch for {local_path}, expected {expected_size}, got {file_size}"
            )
            return False
        if file_sha256 != expected_sha256:
            warn(
                f"hash mismatch for {local_path}, expected {expected_sha256}, got {file_sha256}"
            )
            return False
        return True
    ...
    @classmethod
    def from_json(
        cls, tool_dict: Dict[str, Union[str, List[str], Dict[str, str]]]
    ) -> "IDFTool":
        """
        ...
        if type(version_cmd) is not list:
            raise RuntimeError(
                f"version_cmd for tool {tool_name} is not a list of strings"
            )

        version_regex = tool_dict.get("version_regex")
        if not isinstance(version_regex, str) or (not version_regex and is_executable):
            raise RuntimeError(
                f"version_regex for tool {tool_name} is not a non-empty string"
            )
        ...
        if version_regex_replace and not isinstance(version_regex_replace, str):
            raise RuntimeError(
                f"version_regex_replace for tool {tool_name} is not a string"
            )
        ...
        if strip_container_dirs and type(strip_container_dirs) is not int:
            raise RuntimeError(
                f"strip_container_dirs for tool {tool_name} is not an int"
            )
        ...
        if not isinstance(supported_targets, list):
            raise RuntimeError(
                f"supported_targets for tool {tool_name} is not a list of strings"
            )
        ...
        for index, override in enumerate(overrides_list):
            ...
            if type(platforms_list) is not list:
                raise RuntimeError(
                    "platforms for override %d of tool %s is not a list"
                    % (index, tool_name)
                )
            ...
            if install is not None and not isinstance(install, str):
                raise RuntimeError(
                    "install for override %d of tool %s is not a string"
                    % (index, tool_name)
                )
            ...
            if version_cmd is not None and type(version_cmd) is not list:
                raise RuntimeError(
                    "version_cmd for override %d of tool %s is not a list of strings"
                    % (index, tool_name)
                )

            version_regex = override.get("version_regex")  # type: ignore
            if version_regex is not None and (
                not isinstance(version_regex, str) or not version_regex
            ):
                raise RuntimeError(
                    "version_regex for override %d of tool %s is not a non-empty string"
                    % (index, tool_name)
                )

            version_regex_replace = override.get("version_regex_replace")  # type: ignore
            if version_regex_replace is not None and not isinstance(
                version_regex_replace, str
            ):
                raise RuntimeError(
                    "version_regex_replace for override %d of tool %s is not a string"
                    % (index, tool_name)
                )
            ...
            if export_paths is not None and type(export_paths) is not list:
                raise RuntimeError(
                    "export_paths for override %d of tool %s is not a list"
                    % (index, tool_name)
                )
            ...
            if export_vars is not None and type(export_vars) is not dict:
                raise RuntimeError(
                    "export_vars for override %d of tool %s is not a mapping"
                    % (index, tool_name)
                )
        ...
        for version_dict in versions:  # type: ignore
            ...
            if (
                not isinstance(version_status, str)
                and version_status not in IDFToolVersion.STATUS_VALUES
            ):
                raise RuntimeError(
                    "tool {} version {} status is not one of {}",
                    tool_name,
                    version,
            ...
            for platform_id, platform_dict in version_dict.items():  # type: ignore
                ...
                try:
                    ...
                    raise RuntimeError(
                        "invalid platform %s for tool %s version %s"
...
class IDFRecord:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return all(
            ...
            for x in ("version", "path", "features", "targets")
        )
...
class IDFEnv:
    ...
    def __init__(self) -> None:
        ...
        self.idf_installed: Dict[str, IDFRecord] = {
            active_idf_id: IDFRecord.get_active_idf_record()
        }
        ...
        if self.get_active_idf_record() != self.get_idf_env().get_active_idf_record():
            ...
            try:
                ...
                with open(idf_env_file_path, "w", encoding="utf-8") as w:
                    ...
                    json.dump(
                        dict(self), w, cls=IDFEnvEncoder, ensure_ascii=False, indent=4
                    )  # type: ignore
                ...
                raise OSError(
                    f"File {idf_env_file_path} is not accessible to write or corrupted. Required changes have not been saved"
                )
            ...
            with open(idf_env_file_path, "r", encoding="utf-8") as idf_env_file:
                ...
                try:
                    ...
                    for idf in idf_installed:
                        try:
                            idf_installed_verified[idf] = (
                                IDFRecord.get_idf_record_from_dict(idf_installed[idf])
                            )
                        except ValueError as err:
                            warn(
                                f'{err} "{idf}" found in {idf_env_file_path}, removing this record.'
                            )
                    ...
                    idf_env_obj.idf_installed = dict(
                        idf_env_obj.idf_installed, **idf_installed_verified
                    )
...
class ENVState:
    ...
    * idf_variables - loaded IDF variables from file
    ...
    def save(self) -> str:
        try:
            if self.deactivate_file_path and os.path.basename(
                self.deactivate_file_path
            ).endswith(f"idf_{str(os.getppid())}"):
                ...
                with tempfile.NamedTemporaryFile(
                    delete=False, suffix=f"idf_{str(os.getppid())}"
                ) as fp:
                    self.deactivate_file_path = fp.name
                    fp.write(
                        json.dumps(
                            self.idf_variables, ensure_ascii=False, indent=4
                        ).encode("utf-8")
                    )
        except (IOError, OSError):
            warn(
...
def expand_tools_arg(
    tools_spec: List[str], overall_tools: OrderedDict, targets: List[str]
) -> List[str]:
    """
    ...
    for tool_pattern in tools_spec:
        tools.extend(
            [
                k
                for k, _ in overall_tools.items()
                if fnmatch.fnmatch(k, tool_pattern) and k not in tools
            ]
        )
    ...
    if "required" in tools_spec:
        tools.extend(
            [
                k
                for k, v in overall_tools.items()
                if v.get_install_type() == IDFTool.INSTALL_ALWAYS and k not in tools
            ]
        )
        ...
        tools.extend(
            [
                k
                for k, v in overall_tools.items()
                if v.get_install_type() != IDFTool.INSTALL_NEVER and k not in tools
            ]
        )

    # Filtering by ESP_targets
    tools = [
        k for k in tools if overall_tools[k].is_supported_for_any_of_targets(targets)
    ]
    return tools
...
def parse_targets_arg(targets_str: str) -> List[str]:
    ...
    if targets == ["all"]:
        ...
        if invalid_targets:
            warn(
                f'Targets: "{", ".join(invalid_targets)}" are not supported. Only allowed options are: {", ".join(targets_from_tools_json)}.'
            )
            raise SystemExit(1)
        return targets
...
def feature_to_requirements_path(feature: str) -> str:
    ...
    """
    return os.path.join(
        g.idf_path, "tools", "requirements", f"requirements.{feature}.txt"
    )
...
def process_and_check_features(idf_env_obj: IDFEnv, features_str: str) -> List[str]:
    ...
    idf_env_obj.get_active_idf_record().update_features(
        tuple(new_features), tuple(remove_features)
    )
...
def filter_tools_info(
    idf_env_obj: IDFEnv, tools_info: Dict[str, IDFTool]
) -> Dict[str, IDFTool]:
    """
    ...
    if not targets:
        ...
        filtered_tools_spec = {
            ...
            if (
                v.get_install_type() == IDFTool.INSTALL_ALWAYS
                or v.get_install_type() == IDFTool.INSTALL_ON_REQUEST
            )
            and (
                any(item in targets for item in v.get_supported_targets())
                or v.get_supported_targets() == ["all"]
            )
        }
        return OrderedDict(filtered_tools_spec)


def add_variables_to_deactivate_file(
    args: List[str], new_idf_vars: Dict[str, Any]
) -> str:
    """
    ...
    if "PATH" in new_idf_vars:
        new_idf_vars["PATH"] = new_idf_vars["PATH"].split(":")[
            :-1
        ]  # PATH is stored as list of sub-paths without '$PATH'

    new_idf_vars["PATH"] = new_idf_vars.get("PATH", [])
    args_add_paths_extras = vars(args).get(
        "add_paths_extras"
    )  # remove mypy error with args
    new_idf_vars["PATH"] = (
        new_idf_vars["PATH"] + args_add_paths_extras.split(":")
        if args_add_paths_extras
        else new_idf_vars["PATH"]
    )
    ...
    if env_state_obj.idf_variables:
        ...
        new_idf_vars["PATH"] = list(
            set(new_idf_vars["PATH"] + exported_idf_vars.get("PATH", []))
        )  # remove duplicates
        env_state_obj.idf_variables = dict(
            exported_idf_vars, **new_idf_vars
        )  # merge two dicts
    else:
...
def print_deactivate_statement(args: List[str]) -> None:
    ...
    if not env_state_obj.idf_variables:
        warn(
            "No IDF variables to remove from environment found. Deactivation of previous esp-idf version was not successful."
        )
        return
    unset_vars = env_state_obj.idf_variables
    env_path: Optional[str] = os.getenv("PATH")
    if env_path:
        cleared_env_path = ":".join(
            [k for k in env_path.split(":") if k not in unset_vars["PATH"]]
        )
...
def different_idf_detected() -> bool:
    ...
    if all(
        s not in os.environ
        for s in ["IDF_PYTHON_ENV_PATH", "OPENOCD_SCRIPTS", "ESP_IDF_VERSION"]
    ):
        return False
...
def list_default(args):  # type: ignore
    ...
    for name, tool in tools_info.items():
        ...
        optional_str = (
            " (optional)"
            if tool.get_install_type() == IDFTool.INSTALL_ON_REQUEST
            else ""
        )
        ...
        versions_sorted = sorted(
            versions_for_platform.keys(), key=tool.versions.get, reverse=True
        )  # type: ignore
...
def list_outdated(args):  # type: ignore
    ...
    for name, tool in tools_info.items():
        ...
        if not version_installed:
            continue
        version_available = sorted(
            versions_for_platform.keys(), key=tool.versions.get, reverse=True
        )[0]
        if version_installed < version_available:
            info(
                f"{name}: version {version_installed} is outdated by {version_available}"
            )
...
def action_check(args):  # type: ignore
    ...
    for name, tool in tools_info.items():
        ...
        for version in tool.versions_installed:
            ...
            tool_found_somewhere = True
        if (
            not tool_found_somewhere
            and tool.get_install_type() == IDFTool.INSTALL_ALWAYS
        ):
            not_found_list.append(name)
    if not_found_list:
        fatal(
            f"The following required tools were not found: {' '.join(not_found_list)}"
        )
        raise SystemExit(1)
    if tool_error:
        raise SystemExit(1)
...
def handle_supported_or_deprecated_version(tool: IDFTool, tool_name: str) -> None:
    ...
    if version_obj.status == IDFToolVersion.STATUS_SUPPORTED:
        info(
            f"Using a supported version of tool {tool_name} found in PATH: {tool.version_in_path}.",
            f=sys.stderr,
        )
        info(
            f"However the recommended version is {tool.get_recommended_version()}.",
            f=sys.stderr,
        )
    elif version_obj.status == IDFToolVersion.STATUS_DEPRECATED:
        warn(
            f"using a deprecated version of tool {tool_name} found in PATH: {tool.version_in_path}"
        )


# The following function is used in process_tool which is a part of the action_export.
def handle_missing_versions(
    tool: IDFTool, tool_name: str, install_cmd: str, prefer_system_hint: str
) -> None:
    """
    ...
    if tool.version_in_path:
        if tool.version_in_path not in tool.versions:
            ...
            if args.prefer_system:  # type: ignore
                warn(
                    f"using an unsupported version of tool {tool.name} found in PATH: {tool.version_in_path}"
                )
                return tool_export_paths, tool_export_vars, tool_found
            else:
    ...
    if not tool.versions_installed:
        if tool.get_install_type() == IDFTool.INSTALL_ALWAYS:
            if os.getenv("IDF_SKIP_TOOLS_CHECK", "0") == "1":
                warn(
                    f"Tool {tool_name} is not installed and IDF_SKIP_TOOLS_CHECK is set. "
                    "This may cause build failures."
                )
            else:
                handle_missing_versions(
                    tool, tool_name, install_cmd, prefer_system_hint
                )
                tool_found = False
...
def check_python_venv_compatibility(idf_python_env_path: str, idf_version: str) -> None:
    try:
        with open(
            os.path.join(idf_python_env_path, VENV_VER_FILE), "r", encoding="utf-8"
        ) as f:
...
def action_export(args: Any) -> None:
    ...
    prefer_system_hint = (
        ...
        else f" To use it, run '{self_restart_cmd} export --prefer-system'"
    )
    install_cmd = (
        to_shell_specific_paths([IDF_TOOLS_INSTALL_CMD])[0]
        if IDF_TOOLS_INSTALL_CMD
        else f"{self_restart_cmd} install"
    )
    ...
    idf_python_env_path, idf_python_export_path, virtualenv_python, _ = (
        get_python_env_path()
    )
    if os.path.exists(virtualenv_python):
        ...
        if os.getenv("IDF_PYTHON_ENV_PATH") != idf_python_env_path:
            export_vars["IDF_PYTHON_ENV_PATH"] = to_shell_specific_paths(
                [idf_python_env_path]
            )[0]
        if (
            current_path and idf_python_export_path not in current_path
        ):  # getenv can return None
    ...
    if paths_to_export:
        export_vars["PATH"] = path_sep.join(
            to_shell_specific_paths(paths_to_export) + [old_path]
        )
        ...
        if sys.platform == "win32":
            ...
            try:
                if paths_to_check.index(r"\tools;") < paths_to_check.index(r"\idf-exe"):
                    warn(
                        "The PATH is not in correct order (idf-exe should be before esp-idf\\tools)"
                    )
            except ValueError:
                fatal(
                    f"Both of the directories (..\\idf-exe\\.. and ..\\tools) has to be in the PATH:\n\n{paths_to_check}\n"
                )

    if export_vars:
        # if not copy of export_vars is given to function, it brekas the formatting string for 'export_statements'
        deactivate_file_path = add_variables_to_deactivate_file(
            args, export_vars.copy()
        )
        export_vars[ENVState.env_key] = deactivate_file_path
        export_statements = export_sep.join(
            [export_format.format(k, v) for k, v in export_vars.items()]
        )
        print(export_statements)


def get_idf_download_url_apply_mirrors(
    args: Any = None, download_url: str = IDF_DL_URL
) -> str:
    """
    ...
    if mirror_prefix_map:
        for item in mirror_prefix_map:
            if URL_PREFIX_MAP_SEPARATOR not in item:
                warn(
                    f"invalid mirror-prefix-map item (missing '{URL_PREFIX_MAP_SEPARATOR}') {item}"
                )
                continue
            search, replace = item.split(URL_PREFIX_MAP_SEPARATOR, 1)
            replace = replace.replace(
                "\\", "\\\\"
            )  # On windows replace single \ with double \\
...
def apply_github_assets_option(idf_download_url: str) -> str:
    ...
    if "://" in github_assets:
        fatal(
            "IDF_GITHUB_ASSETS shouldn't include any URL qualifier, https:// is assumed"
        )
        raise SystemExit(1)
    ...
    new_url = re.sub(
        r"^https://github.com/", f"https://{github_assets}/", idf_download_url
    )
    if new_url != idf_download_url:
...
def action_download(args):  # type: ignore
    ...
    for tool_spec in tools_spec:
        ...
        _idf_tool_obj = tool_obj.versions[tool_version].get_download_for_platform(
            platform
        )
...
def action_install(args):  # type: ignore
    ...
    for tool_spec in tools_spec:
        ...
        if not tool_obj.compatible_with_platform():
            fatal(
                f"tool {tool_name} does not have versions compatible with platform {CURRENT_PLATFORM}"
            )
            raise SystemExit(1)
        if tool_version is not None and tool_version not in tool_obj.versions:
            fatal(f"unknown version for tool {tool_name}: {tool_version}")
...
def get_constraints(idf_version: str, online: bool = True) -> str:
    ...
    try:
        age = datetime.date.today() - datetime.date.fromtimestamp(
            os.path.getmtime(constraint_path)
        )
        if age < datetime.timedelta(days=1):
            info(
                f"Skipping the download of {constraint_path} because it was downloaded recently."
            )
            return constraint_path
    except OSError:
    ...
    if os.path.isfile(constraint_path):
        warn(
            "Failed to download, retry count has expired, using a previously downloaded version"
        )
        return constraint_path
    else:
        fatal("Failed to download, and retry count has expired")
...
def install_legacy_python_virtualenv(path: str) -> None:
    ...
    if not virtualenv_installed_via_pip:
        ...
        try:
            ...
            if major_ver < 20:
                warn(
                    f"Virtualenv version {virtualenv.__version__} is old, please consider upgrading it"
                )
    ...
    subprocess.check_call(
        [sys.executable, "-m", "virtualenv", *virtualenv_options, path],
        stdout=sys.stdout,
        stderr=sys.stderr,
    )
...
def action_install_python_env(args):  # type: ignore
    ...
    is_nix = (
        nix_store is not None
        and sys.base_prefix.startswith(nix_store)
        and sys.prefix.startswith(nix_store)
    )
    ...
    if os.path.exists(virtualenv_python):
        try:
            subprocess.check_call(
                [virtualenv_python, "--version"], stdout=sys.stdout, stderr=sys.stderr
            )
        ...
        try:
            subprocess.check_call(
                [virtualenv_python, "-m", "pip", "--version"],
                stdout=sys.stdout,
                stderr=sys.stderr,
            )
        except subprocess.CalledProcessError:
            warn(
                "pip is not available in the existing virtual environment, new virtual environment will be created."
            )
        ...
        if sys.platform != "win32":
            try:
                subprocess.check_call(
                    [virtualenv_python, "-c", "import curses"],
                    stdout=sys.stdout,
                    stderr=sys.stderr,
                )
            except subprocess.CalledProcessError:
                warn(
                    "curses can not be imported, new virtual environment will be created."
                )
                reinstall = True
    ...
    if os.path.exists(virtualenv_python):
        ...
        try:
            ...
            if not correct_env_path and re.search(
                PYTHON_VENV_DIR_TEMPLATE.format(r"\d+\.\d+", r"\d+\.\d+"),
        ...
        try:
            with open(
                os.path.join(idf_python_env_path, VENV_VER_FILE),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(idf_version)
            ...
            warn(
                f"The following issue occurred while generating the ESP-IDF version file in the Python environment: {e}. "
                "(Diagnostic information. It can be ignored.)"
            )
    ...
    if env_copy.get("PIP_USER") == "yes":
        warn(
            'Found PIP_USER="yes" in the environment. Disabling PIP_USER in this shell to install packages into a virtual environment.'
        )
...
def action_check_python_dependencies(args):  # type: ignore
    ...
    if not os.path.isfile(virtualenv_python):
        fatal(
            f'{virtualenv_python} doesn\'t exist! Please run the install script or "idf_tools.py install-python-env" in order to create it'
        )
        raise SystemExit(1)

    if use_constraints:
        constr_path = get_constraints(
            idf_version, online=False
        )  # keep offline for checking
    ...
    dep_check_cmd = [
        virtualenv_python,
        os.path.join(g.idf_path, "tools", "check_python_dependencies.py"),
    ]
    ...
    try:
        ret = subprocess.run(dep_check_cmd, check=False)
...
class ChecksumFileParser:
    ...
    def __init__(self, filename_prefix: str, url: str) -> None:
        ...
        sha256_file_tmp = os.path.join(
            g.idf_tools_path, "tools", "add-version.sha256.tmp"
        )
    ...
    def parseLine(self, regex: str, line: str) -> str:
        ...
        if not match:
            raise ChecksumParsingError(
                f'Can not parse line "{line}" with regex "{regex}"'
            )
...
def action_add_version(args: Any) -> None:
    ...
    filename_prefix = (
        args.dist_filename_prefix if len(args.dist_filename_prefix) else tool_name
    )
    checksum_info: ChecksumFileParser = (
        ChecksumFileParser(filename_prefix, args.checksum_file)
...
def action_uninstall(args: Any) -> None:
    ...
    for tool in installed_tools:
        tool_versions = (
            os.listdir(os.path.join(tools_path, tool))
            if os.path.isdir(os.path.join(tools_path, tool))
            else []
        )
        try:
            unused_versions = [
                x
                for x in tool_versions
                if x != tools_info[tool].get_recommended_version()
            ]
    ...
    if args.remove_archives:
        ...
        for tool_spec in tools_spec:
            ...
            if tool_version is not None:
                archive_version = tool_obj.versions[
                    tool_version
                ].get_download_for_platform(CURRENT_PLATFORM)
...
def action_validate(args):  # type: ignore
    ...
    with open(
        os.path.join(g.idf_path, TOOLS_FILE), "r", encoding="utf-8"
    ) as tools_file:
        tools_json = json.load(tools_file)

    with open(
        os.path.join(g.idf_path, TOOLS_SCHEMA_FILE), "r", encoding="utf-8"
    ) as schema_file:
...
def action_check_tool_supported(args: Any) -> None:
    ...
    try:
        ...
        fatal(
            f"Failed to check tool support: (name: {args.tool_name}, exec: {args.exec_path})"
        )
        fatal(f"{err}")
        raise SystemExit(1)
...
def main(argv: List[str]) -> None:
    ...
    parser.add_argument(
        "--quiet",
        help="Don't output diagnostic messages to stdout/stderr",
        action="store_true",
    )
    parser.add_argument(
        "--non-interactive",
        help="Don't output interactive messages and questions",
        action="store_true",
    )
    ...
    list_parser = subparsers.add_parser(
        "list", help="List tools and versions available"
    )
    list_parser.add_argument(
        "--outdated", help="Print only outdated installed tools", action="store_true"
    )
    subparsers.add_parser(
        "check", help="Print summary of tools installed or found in PATH"
    )
    export = subparsers.add_parser(
        "export", help="Output command for setting tool paths, suitable for shell"
    )
    export.add_argument(
        "--format",
    ...
    export.add_argument(
        "--add_paths_extras", help="Add idf-related path extras for deactivate option"
    )
    ...
    download = subparsers.add_parser(
        "download", help="Download the tools into the dist directory"
    )
    download.add_argument(
        "--platform",
        default=CURRENT_PLATFORM,
        help="Platform to download the tools for",
    )
    download.add_argument(
        "tools",
        metavar="TOOL",
    ...
    uninstall = subparsers.add_parser(
        "uninstall",
        help="Remove installed tools, that are not used by current version of ESP-IDF.",
    )
    uninstall.add_argument("--dry-run", help="Print unused tools.", action="store_true")
    uninstall.add_argument(
        "--remove-archives",
        help="Remove old archive versions and archives from unused tools.",
        action="store_true",
    )

    no_constraints_default = os.environ.get(
        "IDF_PYTHON_CHECK_CONSTRAINTS", ""
    ).lower() in ["0", "n", "no"]
    ...
    install_python_env = subparsers.add_parser(
        "install-python-env",
        help=(
            "Create Python virtual environment and install the required Python packages"
        ),
    )
    ...
    if IDF_MAINTAINER:
        add_version = subparsers.add_parser(
            "add-version", help="Add or update download info for a version"
        )
        add_version.add_argument("--output", help="Save new tools.json into this file")
        add_version.add_argument(
            "--tool", help="Tool name to set add a version for", required=True
        )
        add_version.add_argument("--version", help="Version identifier", required=True)
        add_version.add_argument(
            "--url-prefix",
            help="String to prepend to file names to obtain download URLs",
        )
        add_version.add_argument(
            "--override",
            action="store_true",
            help="Override tool versions with new data",
        )
        add_version.add_argument(
        ...
        add_version_files_group = add_version.add_mutually_exclusive_group(
            required=True
        )
        add_version_files_group.add_argument(
            "--checksum-file",
            help="URL or path to local file with checksum/size for artifacts",
        )
        add_version_files_group.add_argument(
            "--artifact-file", help="File names of the download artifacts", nargs="*"
        )

        rewrite = subparsers.add_parser(
            "rewrite",
            help="Load tools.json, validate, and save the result back into JSON",
        )
        rewrite.add_argument("--output", help="Save new tools.json into this file")

        subparsers.add_parser(
            "validate", help="Validate tools.json against schema file"
        )

        gen_doc = subparsers.add_parser(
            "gen-doc", help="Write the list of tools as a documentation page"
        )
        ...
        gen_doc.add_argument(
            "--heading-underline-char",
            help="Character to use when generating RST sections",
            default="~",
        )
    ...
    if os.environ.get("IDF_TOOLS_VERSION_HELPER"):
        ...
        check_tool_supported.add_argument(
            "--tool-name", required=True, help="Tool name (from tools.json)"
        )
        check_tool_supported.add_argument(
            "--exec-path", required=True, help="Full path to executable under the test"
        )

        get_tool_supported_versions = subparsers.add_parser(
            "get-tool-supported-versions",
            help="Prints a list of tool's supported versions",
        )
        get_tool_supported_versions.add_argument(
            "--tool-name", required=True, help="Tool name (from tools.json)"
        )
    ...
    g.idf_path = (
        ...
        or os.path.realpath(os.path.join(os.path.dirname(__file__), ".."))
    )
    os.environ["IDF_PATH"] = g.idf_path

    g.idf_tools_path = os.environ.get("IDF_TOOLS_PATH") or os.path.expanduser(
    ...
    if sys.version_info.major == 2:
        try:
            ...
            fatal(
                f"IDF_TOOLS_PATH contains non-ASCII characters: {g.idf_tools_path}"
