...
class BLE_Bleak_Client:
    ...
    async def connect(self, devname, iface, chrc_names, fallback_srv_uuid):
        ...
        try:
            ...
            if str(e) == "[org.bluez.Error.NotReady] Resource Not Ready":
                raise RuntimeError(
                    "Bluetooth is not ready. Maybe try `bluetoothctl power on`?"
                )
            raise
        ...
        if self.devname is None:
            ...
            while True:
                ...
                print("{0: >4} {1: <33} {2: <12}".format("S.N.", "Name", "Address"))
                for i, _ in enumerate(devices):
                    print(
                        "[{0: >2}] {1: <33} {2: <12}".format(
                            i + 1,
                            devices[i][0].name or "Unknown",
                            devices[i][0].address,
                        )
                    )
    ...
    async def send_data(self, characteristic_uuid, data):
        await self.device.write_gatt_char(
            characteristic_uuid, bytearray(data.encode("latin-1")), True
        )
...
class BLE_Console_Client:
    async def connect(self, devname, iface, chrc_names, fallback_srv_uuid):
        ...
        print(
            "BLECLI >> Please connect to BLE device `"
            + devname
            + "` manually using your tool of choice"
        )
        ...
        resp = input(
            "BLECLI >> Is the service UUID '"
            + fallback_srv_uuid
            + "' listed among available attributes? [y/n] "
        )
        if resp != "Y" and resp != "y":
            return False
        return True
    ...
    def has_characteristic(self, uuid):
        resp = input(
            "BLECLI >> Is the characteristic UUID '"
            + uuid
            + "' listed among available attributes? [y/n] "
        )
        if resp != "Y" and resp != "y":
    ...
    async def send_data(self, characteristic_uuid, data):
        print(
            "BLECLI >> Write following data to characteristic with UUID '"
            + characteristic_uuid
            + "' :"
        )
