...
def scan_start_request(
    security_ctx, blocking=True, passive=False, group_channels=5, period_ms=120
):
    ...
    print_verbose(
        security_ctx, f"Client -> Device (Encrypted CmdScanStart): 0x{enc_cmd.hex()}"
    )
...
def scan_status_request(security_ctx):
    ...
    enc_cmd = security_ctx.encrypt_data(cmd.SerializeToString())
    print_verbose(
        security_ctx, f"Client -> Device (Encrypted CmdScanStatus): 0x{enc_cmd.hex()}"
    )
    return enc_cmd.decode("latin-1")
...
def scan_status_response(security_ctx, response_data):
    ...
    if resp.status != 0:
        raise RuntimeError
    return {
        "finished": resp.resp_scan_status.scan_finished,
        "count": resp.resp_scan_status.result_count,
    }
...
def scan_result_request(security_ctx, index, count):
    ...
    enc_cmd = security_ctx.encrypt_data(cmd.SerializeToString())
    print_verbose(
        security_ctx, f"Client -> Device (Encrypted CmdScanResult): 0x{enc_cmd.hex()}"
    )
    return enc_cmd.decode("latin-1")
...
def scan_result_response(security_ctx, response_data):
    ...
    for entry in resp.resp_scan_result.entries:
        ...
        print_verbose(
            security_ctx, f"ScanResult Channel : {str(results[-1]['channel'])}"
        )
