...
class TestMkDFU(unittest.TestCase):
    def common_test(
        self, json_input=None, file_args=[], output_to_compare=None, part_size=None
    ):
        """
        ...
        if output_to_compare:
            self.assertTrue(
                filecmp.cmp(f_out.name, os.path.join(current_dir, output_to_compare)),
...
class TestHelloWorldExample(TestMkDFU):
    ...
    def test_with_json(self):
        with tempfile.NamedTemporaryFile(
            mode="w", dir=os.path.join(current_dir, "1"), delete=False
        ) as f:
    ...
    def test_without_json(self):
        self.common_test(
            file_args=[(0x1000, "1/1.bin"), (0x8000, "1/2.bin"), (0x10000, "1/3.bin")],
...
class TestSplit(TestMkDFU):
    ...
    def test_split(self):
        ...
        self.common_test(
            file_args=[(0, f.name)], part_size=5, output_to_compare="2/dfu.bin"
        )
