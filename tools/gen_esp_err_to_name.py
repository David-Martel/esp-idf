...
err_dict = collections.defaultdict(
    list
)  # identified errors are stored here; mapped by the error code
...
def process(line: str, idf_path: str, include_as: Any) -> None:
    ...
    if m:
        ...
        todo_str = todo_str[
            : m.start()
        ].strip()  # keep just the part before the comment
    ...
    try:
        ...
        unproc_list.append(
            ErrItem(words[1], idf_path, include_as, comment, related, num)
        )
...
def process_remaining_errors() -> None:
    ...
    for item in unproc_list:
        if item.rel_str in rev_err_dict:
            ...
            err_dict[num].append(
                ErrItem(item.name, item.file, item.include_as, item.comment)
            )
...
def path_to_include(path: str) -> str:
    ...
    try:
        ...
        return os.sep.join(
            spl_path[i + 1 :]
        )  # subdirectories and filename in "include"
...
def generate_c_output(fin: TextIO, fout: TextIO) -> None:
    ...
    max_width = (
        max_string_width() + 17 + 1
    )  # length of "    ERR_TBL_IT()," with spaces is 17
    max_decdig = max(len(str(k)) for k in err_dict)

    for line in fin:
        if re.match(r"@COMMENT@", line):
            fout.write(
                "//Do not edit this file because it is autogenerated by "
            ...
            for k in sorted(err_dict.keys()):
                ...
                for e in err_dict[k]:
                    if e.file != last_file:
                        ...
                        fout.write("    // %s\n" % last_file)
                    table_line = (
                        ("    ERR_TBL_IT(" + e.name + "), ").ljust(max_width)
                        + "/* "
                        + str(k).rjust(max_decdig)
                    )
                    fout.write("#   ifdef      %s\n" % e.name)
                    ...
                    if (
                        k > 0
                    ):  # negative number and zero should be only ESP_FAIL and ESP_OK
                        hexnum = " 0x%x" % k
...
def main() -> None:
    if "IDF_PATH" in os.environ:
        ...
        idf_path = os.path.realpath(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
        )

    parser = argparse.ArgumentParser(
        description="ESP32 esp_err_to_name lookup generator for esp_err_t"
    )
    parser.add_argument(
        "--c_input",
    ...
    parser.add_argument(
        "--rst_output", help="Generate .rst output and save it into this file"
    )
    args = parser.parse_args()

    include_as_pattern = re.compile(
        r'\s*//\s*{}: [^"]* "([^"]+)"'.format(os.path.basename(__file__))
    )
    define_pattern = re.compile(r"\s*#define\s+(ESP_ERR_|ESP_OK|ESP_FAIL)")

    for root, dirnames, filenames in os.walk(
        os.path.join(idf_path, "components"), topdown=True
    ):
        ...
        for filename in fnmatch.filter(filenames, "*.[ch]"):
            ...
            with open(full_path, encoding="utf-8") as f:
                try:
                    ...
                    raise ValueError(
                        "The encoding of {} is not Unicode.".format(path_in_idf)
                    )
    ...
    if args.rst_output is not None:
        ...
        with (
            open(args.c_input, "r", encoding="utf-8") as fin,
            open(args.c_output, "w", encoding="utf-8") as fout,
        ):
