...
def test_target_from_environment_cmake(default_idf_env: EnvDict) -> None:
    ...
    assert file_contains(
        "build/CMakeCache.txt", "IDF_TARGET:STRING={}".format(ESP32S2_TARGET)
    )


def test_target_from_environment_idf_py(
    idf_py: IdfPyFunc, default_idf_env: EnvDict, test_app_copy: Path
) -> None:
    def reconfigure_and_check_return_values(
        errmsg: str, opts: Optional[List[str]] = None
    ) -> None:
        opts = opts or []
    ...
    reconfigure_and_check_return_values(
        "Project sdkconfig '{}' was generated for target '{}', but environment "
        "variable IDF_TARGET is set to '{}'.".format(
            cfg_path, ESP32S2_TARGET, ESP32_TARGET
        )
    )

    logging.info(
        "idf.py fails if IDF_TARGET settings in CMakeCache.txt don't match the environment"
    )
    (test_app_copy / "sdkconfig").write_text(
        'CONFIG_IDF_TARGET="{}"'.format(ESP32_TARGET)
    )
    reconfigure_and_check_return_values(
        "Target settings are not consistent: '{}' in the environment, "
        "'{}' in CMakeCache.txt.".format(ESP32_TARGET, ESP32S2_TARGET)
    )

    logging.info(
        "idf.py fails if IDF_TARGET settings in CMakeCache.txt don't match the sdkconfig"
    )
    default_idf_env.pop("IDF_TARGET")
    reconfigure_and_check_return_values(
        "Project sdkconfig '{}' was generated for target '{}', but CMakeCache.txt "
        "contains '{}'.".format(cfg_path, ESP32_TARGET, ESP32S2_TARGET)
    )

    logging.info(
        "idf.py fails if IDF_TARGET is set differently in environment and with -D option"
    )
    (test_app_copy / "sdkconfig").write_text(
        'CONFIG_IDF_TARGET="{}"'.format(ESP32S2_TARGET)
    )
    default_idf_env.update({"IDF_TARGET": ESP32S2_TARGET})
    reconfigure_and_check_return_values(
        "Target '{}' specified on command line is not consistent with target '{}' "
        "in the environment.".format(ESP32_TARGET, ESP32S2_TARGET),
        ["-D", "IDF_TARGET={}".format(ESP32_TARGET)],
    )

    logging.info(
        "idf.py fails if IDF_TARGET is set differently in CMakeCache.txt and with -D option"
    )
    default_idf_env.pop("IDF_TARGET")
    reconfigure_and_check_return_values(
        "Target '{}' specified on command line is not consistent with "
...
def test_target_consistency_cmake(
    default_idf_env: EnvDict, test_app_copy: Path
) -> None:
    def reconfigure_and_check_return_values(
        errmsg: str, opts: Optional[List[str]] = None
    ) -> None:
    ...
    default_idf_env.update({"IDF_TARGET": ESP32S2_TARGET})
    reconfigure_and_check_return_values(
        f"IDF_TARGET '{ESP32_TARGET}' in CMake cache does not "
    ...
    reconfigure_and_check_return_values(
        "IDF_TOOLCHAIN 'gcc' in CMake cache does not match "
    ...
    reconfigure_and_check_return_values(
        "CMAKE_TOOLCHAIN_FILE 'toolchain-esp32' does not "
        f"match currently selected IDF_TARGET '{ESP32S2_TARGET}'",
...
def test_target_precedence(
    ...
    assert file_contains(
        "sdkconfig", "CONFIG_IDF_TARGET_{}=y".format(ESP32_TARGET.upper())
    )
    assert file_contains(
        "build/CMakeCache.txt", "IDF_TARGET:STRING={}".format(ESP32_TARGET)
    )
...
@pytest.mark.usefixtures("test_app_copy")
def test_target_using_settarget_parameter(
...
def test_target_using_sdkconfig(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    assert file_contains("sdkconfig", 'CONFIG_IDF_TARGET="{}"'.format(ESP32S2_TARGET))
    assert file_contains(
        "sdkconfig", "CONFIG_IDF_TARGET_{}=y".format(ESP32S2_TARGET.upper())
    )
...
def test_target_guessing(
    ...
    (test_app_copy / "sdkconfig2").write_text(
        'CONFIG_IDF_TARGET="{}"'.format(ESP32S2_TARGET)
    )
    clean_app(test_app_copy)
    ...
    (test_app_copy / "sdkconfig3").write_text(
        'CONFIG_IDF_TARGET="{}"'.format(ESP32S2_TARGET)
    )
    (test_app_copy / "sdkconfig4").write_text(
        'CONFIG_IDF_TARGET="{}"'.format(ESP32S3_TARGET)
    )
    clean_app(test_app_copy)
    ...
    assert file_contains(
        "build/CMakeCache.txt", "IDF_TARGET:STRING={}".format(ESP32S3_TARGET)
    )

    logging.info("Can guess target from custom sdkconfig")
    (test_app_copy / "sdkconfig5").write_text(
        'CONFIG_IDF_TARGET="{}"'.format(ESP32C3_TARGET)
    )
    clean_app(test_app_copy)
    idf_py(
        "-D",
        ...
        "-D",
        "SDKCONFIG_DEFAULTS=sdkconfig4;sdkconfig3",
        "reconfigure",
    )
    assert file_contains("sdkconfig5", 'CONFIG_IDF_TARGET="{}"'.format(ESP32C3_TARGET))
    assert file_contains(
        "build/CMakeCache.txt", "IDF_TARGET:STRING={}".format(ESP32C3_TARGET)
    )
