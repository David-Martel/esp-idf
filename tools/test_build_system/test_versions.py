...
def test_versions_get_version_from_git_describe(
    idf_py: IdfPyFunc, test_git_template_app: Path, env: typing.Optional[EnvDict] = None
) -> None:
    ...
    git_ret = subprocess.run(
        ...
        check=False, cwd=test_git_template_app,
        env=env_dict,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )
    assert (
        f'App "app-template" version: {git_ret.stdout.decode("utf-8")}'
        in idf_ret.stdout
    )
...
#
#############################################################################################
def test_versions_get_version_from_version_arg(
    idf_py: IdfPyFunc, test_git_template_app: Path
) -> None:
    logging.info(
        "Verify that the VERSION argument in project() is correctly parsed by cmake"
    )
...
#
#############################################################################################
def test_versions_get_version_from_version_file(
    idf_py: IdfPyFunc, test_git_template_app: Path
) -> None:
...
#
#############################################################################################
def test_versions_get_version_from_top_level_cmake(
    idf_py: IdfPyFunc, test_git_template_app: Path
) -> None:
    logging.info(
        "Verify that the version of app can be set from PROJECT_VER in CMakeLists.txt"
    )
    replace_in_file(
        (test_git_template_app / "CMakeLists.txt"),
        "project(app-template)",
    ...
    assert (
        'App "app-template" version: project_version_from_CMakeLists' in idf_ret.stdout
    )
...
#
#############################################################################################
def test_versions_get_version_from_kconfig_option(
    idf_py: IdfPyFunc, test_git_template_app: Path
) -> None:
