...
def create_idf_components(request: pytest.FixtureRequest) -> Generator:
    ...
    if mark is None:
        ...
        for name in mark.args[0]:
            ...
            (idf_path / "components" / name / "CMakeLists.txt").write_text(
...
def test_component_extra_dirs(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    replace_in_file(
        ...
        "set(EXTRA_COMPONENT_DIRS {})".format(
            Path("different_main", "main").as_posix()
        ),
    )
...
def test_component_names_contain_spaces(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    (test_app_copy / "extra component" / "CMakeLists.txt").write_text(
        "idf_component_register"
    )
...
def test_component_subdirs_not_added_to_component_dirs(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    logging.info(
    ...
    (test_app_copy / "main" / "test" / "CMakeLists.txt").write_text(
        "idf_component_register()"
    )
    idf_py("reconfigure")
    data = json.load(open(test_app_copy / "build" / "project_description.json", "r"))
    assert str((test_app_copy / "main" / "test").as_posix()) not in data.get(
        "build_component_paths"
    )
    assert str((test_app_copy / "main").as_posix()) in data.get("build_component_paths")


def test_component_sibling_dirs_not_added_to_component_dirs(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    logging.info(
    ...
    (mycomponents_subdir / "mycomponent" / "CMakeLists.txt").write_text(
        "idf_component_register()"
    )

    # first test by adding single component directory to EXTRA_COMPONENT_DIRS
    (mycomponents_subdir / "esp32").mkdir(parents=True)
    (mycomponents_subdir / "esp32" / "CMakeLists.txt").write_text(
        "idf_component_register()"
    )
    idf_py(
        "-DEXTRA_COMPONENT_DIRS={}".format(str(mycomponents_subdir / "mycomponent")),
        "reconfigure",
    )
    data = json.load(open(test_app_copy / "build" / "project_description.json", "r"))
    assert str((mycomponents_subdir / "esp32").as_posix()) not in data.get(
        "build_component_paths"
    )
    assert str((mycomponents_subdir / "mycomponent").as_posix()) in data.get(
        "build_component_paths"
    )
    ...
    data = json.load(open(test_app_copy / "build" / "project_description.json", "r"))
    assert str((test_app_copy / "esp32").as_posix()) not in data.get(
        "build_component_paths"
    )
    assert str((mycomponents_subdir / "mycomponent").as_posix()) in data.get(
        "build_component_paths"
...
def test_component_properties_are_set(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    ret = idf_py("reconfigure")
    assert (
        "SRCS:{}".format((test_app_copy / "main" / "build_test_app.c").as_posix())
        in ret.stdout
    ), "Component properties should be set"


def test_get_property_for_unknown_component(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
...
def test_set_property_for_unknown_component(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
...
def test_component_overridden_dir(
    ...
    assert (
        "overridden_dir:{}".format((idf_path / "components" / "hal").as_posix())
        in ret.stdout
    ...
    ret = idf_py("reconfigure", check=False)
    assert (
        "kconfig:{}".format((idf_path / "components" / "hal").as_posix()) in ret.stdout
    ), "Failed to verify original `main` directory"


def test_project_components_overrides_extra_components(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    logging.info(
        "Project components override components defined in EXTRA_COMPONENT_DIRS"
    )
    (test_app_copy / "extra_dir" / "my_component").mkdir(parents=True)
    (test_app_copy / "extra_dir" / "my_component" / "CMakeLists.txt").write_text(
        "idf_component_register()"
    )
    replace_in_file(
        test_app_copy / "CMakeLists.txt",
    ...
    with open(test_app_copy / "build" / "project_description.json", "r") as f:
        data = json.load(f)
    assert str((test_app_copy / "extra_dir" / "my_component").as_posix()) in data.get(
        "build_component_paths"
    )

    (test_app_copy / "components" / "my_component").mkdir(parents=True)
    (test_app_copy / "components" / "my_component" / "CMakeLists.txt").write_text(
        "idf_component_register()"
    )
    idf_py("reconfigure")
    with open(test_app_copy / "build" / "project_description.json", "r") as f:
        data = json.load(f)
    assert str((test_app_copy / "components" / "my_component").as_posix()) in data.get(
        "build_component_paths"
    )
    assert str(
        (test_app_copy / "extra_dir" / "my_component").as_posix()
    ) not in data.get("build_component_paths")


def test_extra_components_overrides_managed_components(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    logging.info(
        "components defined in EXTRA_COMPONENT_DIRS override managed components"
    )
    ...
    with open(test_app_copy / "build" / "project_description.json", "r") as f:
        data = json.load(f)
    assert str(
        (test_app_copy / "managed_components" / "example__cmp").as_posix()
    ) in data.get("build_component_paths")

    (test_app_copy / "extra_dir" / "cmp").mkdir(parents=True)
    (test_app_copy / "extra_dir" / "cmp" / "CMakeLists.txt").write_text(
        "idf_component_register()"
    )
    replace_in_file(
        test_app_copy / "CMakeLists.txt",
    ...
    with open(test_app_copy / "build" / "project_description.json", "r") as f:
        data = json.load(f)
    assert str((test_app_copy / "extra_dir" / "cmp").as_posix()) in data.get(
        "build_component_paths"
    )
    assert str(
        (test_app_copy / "managed_components" / "example__cmp").as_posix()
...
def test_managed_components_overrides_idf_components(
    ...
    logging.info(
        "Managed components override components defined in IDF_PATH/components"
    )
...
def test_manifest_local_source_overrides_extra_components(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    (test_app_copy / ".." / "extra_dir" / "cmp").mkdir(parents=True)
    (test_app_copy / ".." / "extra_dir" / "cmp" / "CMakeLists.txt").write_text(
    ...
    with open(test_app_copy / "build" / "project_description.json", "r") as f:
        data = json.load(f)
    assert str(
        (test_app_copy / ".." / "extra_dir" / "cmp").resolve().as_posix()
    ) in data.get("build_component_paths")

    (test_app_copy / ".." / "cmp").mkdir(parents=True)
    (test_app_copy / ".." / "cmp" / "CMakeLists.txt").write_text(
        "idf_component_register()"
    )
...
dependencies:
    ...
    with open(test_app_copy / "build" / "project_description.json", "r") as f:
        data = json.load(f)
    assert str((test_app_copy / ".." / "cmp").resolve().as_posix()) in data.get(
        "build_component_paths"
    )
    assert str(
        (test_app_copy / ".." / "extra_dir" / "cmp").resolve().as_posix()
...
def test_exclude_components_not_passed(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    (test_app_copy / "components" / "to_be_excluded" / "idf_component.yml").write_text(
        "invalid syntax.."
    )
    ret = idf_py("reconfigure", check=False)
    assert ret.returncode == 2, (
        "Reconfigure should have failed due to invalid syntax in idf_component.yml"
    )
...
def test_unknown_component_error(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    ret = idf_py("reconfigure", check=False)
    assert (
        "Failed to resolve component 'unknown' required by component 'main'"
        in ret.stderr
    )
