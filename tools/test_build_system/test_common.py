...
def test_idf_build_with_env_var_sdkconfig_defaults(
    test_app_copy: Path, default_idf_env: EnvDict
) -> None:
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="Windows does not support executing bash script"
)
...
@pytest.mark.skipif(
    sys.platform != "win32", reason="Linux does not support executing .exe files"
)
...
def test_toolchain_prefix_in_description_file(
...
def test_subcommands_with_options(idf_py: IdfPyFunc, default_idf_env: EnvDict) -> None:
    ...
    try:
        (idf_path / "tools" / "idf_monitor.py").write_text(
...
def test_fallback_to_build_system_target(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    ...
    append_to_file(
        ...
        'add_custom_target(custom_target COMMAND ${{CMAKE_COMMAND}} -E echo "{}")'.format(
            msg
        ),
    )
...
def test_create_component_project(idf_copy: Path) -> None:
    ...
    run_idf_py(
        "-C", "projects", "create-project", "temp_test_project", workdir=idf_copy
    )
    run_idf_py(
        "-C", "components", "create-component", "temp_test_component", workdir=idf_copy
    )
    replace_in_file(
...
def test_create_project(idf_py: IdfPyFunc, idf_copy: Path) -> None:
    ...
    ret = idf_py(
        "create-project",
        "--path",
        str(idf_copy / "example_proj_file"),
        "temp_test_project",
        check=False,
    )
...
@pytest.mark.skipif(
    sys.platform == "darwin",
    reason="macos runner is a shell executor, it would break the file system",
)
def test_create_project_with_idf_readonly(idf_copy: Path) -> None:
    ...
    logging.info(
        "Check that command for creating new project will success if the IDF itself is readonly."
    )
...
def test_docs_command(idf_py: IdfPyFunc) -> None:
    ...
    assert (
        "https://docs.espressif.com/projects/esp-idf/en/v4.2.1/esp32/get-started"
        in ret.stdout
    )
...
def test_deprecation_warning(idf_py: IdfPyFunc) -> None:
    ...
    # click warning
    assert (
        'Error: Command "post_debug" is deprecated since v4.4 and was removed in v5.0.'
        in ret.stderr
    )
...
def test_save_defconfig_check(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    (test_app_copy / "sdkconfig").write_text(
        "\n".join(
            ["CONFIG_COMPILER_OPTIMIZATION_SIZE=y", "CONFIG_ESPTOOLPY_FLASHFREQ_80M=y"]
        )
    )
    idf_py("save-defconfig")
    assert not file_contains(
        test_app_copy / "sdkconfig.defaults", "CONFIG_IDF_TARGET"
    ), "CONFIG_IDF_TARGET should not be in sdkconfig.defaults"
    assert file_contains(
        test_app_copy / "sdkconfig.defaults", "CONFIG_COMPILER_OPTIMIZATION_SIZE=y"
    ), "Missing CONFIG_COMPILER_OPTIMIZATION_SIZE=y in sdkconfig.defaults"
    assert file_contains(
        test_app_copy / "sdkconfig.defaults", "CONFIG_ESPTOOLPY_FLASHFREQ_80M=y"
    ...
    idf_py("save-defconfig")
    assert file_contains(
        test_app_copy / "sdkconfig.defaults", 'CONFIG_IDF_TARGET="esp32c3"'
    ), 'Missing CONFIG_IDF_TARGET="esp32c3" in sdkconfig.defaults'
    assert file_contains(
        test_app_copy / "sdkconfig.defaults", "CONFIG_PARTITION_TABLE_OFFSET=0x8001"
