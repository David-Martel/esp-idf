...
def pytest_runtest_makereport(
    item: typing.Any, call: typing.Any
) -> typing.Generator[None, pytest.TestReport, None]:  # pylint: disable=unused-argument
...
def should_clean_test_dir(request: FixtureRequest) -> bool:
    ...
    return getattr(request.node, "passed", False) or request.config.getoption(
        "cleanup_idf_copy", False
    )
...
def _session_work_dir(
    request: FixtureRequest,
) -> typing.Generator[typing.Tuple[Path, bool], None, None]:
...
@pytest.fixture
def test_app_copy(
    func_work_dir: Path, request: FixtureRequest
) -> typing.Generator[Path, None, None]:
    ...
    if should_clean_test_dir(request):
        logging.debug(
            "cleaning up work directory after a successful test: {}".format(path_to)
        )
...
def idf_py(default_idf_env: EnvDict) -> IdfPyFunc:
    def result(
        *args: str, check: bool = True, input_str: typing.Optional[str] = None
