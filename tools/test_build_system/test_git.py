...
def run_git_cmd(
    *args: str, workdir: Path, env: typing.Optional[EnvDict] = None
    ...
    return subprocess.run(
        cmd, check=False, cwd=workdir, env=env_dict, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def test_git_custom_tag() -> None:
    try:
        env_dict = dict(**os.environ)
        idf_build_test_app_path = Path(
            env_dict["IDF_PATH"], "tools", "test_build_system", "build_test_app"
        )
        ...
        run_git_cmd(
            ...
            "-m",
            "mytag",
            workdir=idf_build_test_app_path,
            env=env_dict,
        )
        idf_ret = run_idf_py("reconfigure", workdir=idf_build_test_app_path)
        assert 'App "build_test_app" version: mytag' in idf_ret.stdout, (
            "Project version should be the custom tag"
        )
...
def test_support_git_worktree(test_git_template_app: Path) -> None:
    ...
    run_git_cmd(
        "worktree",
        "add",
        "../esp-idf-worktree-app",
        "test_build_system",
        workdir=test_git_template_app,
    )
    try:
        ...
        idf_ret_worktree_app = run_idf_py(
            ...
            workdir=os.path.join(
                os.path.dirname(test_git_template_app), "esp-idf-worktree-app"
            ),
        )
        assert (
            re.search(r"-- App \"app-template\".*", idf_ret_template_app.stdout).group()  # type: ignore
            == re.search(
                r"-- App \"app-template\".*", idf_ret_worktree_app.stdout
            ).group()
        )  # type: ignore
    finally:
        run_git_cmd(
