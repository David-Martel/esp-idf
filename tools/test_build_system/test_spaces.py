...
import pytest
from test_build_system_helpers import IdfPyFunc, run_idf_py
...
def test_spaces_bundle1(idf_copy: Path) -> None:
    ...
    # test build ulp_fsm
    run_idf_py(
        "build", workdir=(idf_copy / "examples" / "system" / "ulp" / "ulp_fsm" / "ulp")
    )
    # test build ulp_riscv
    run_idf_py(
        "-DIDF_TARGET=esp32s2",
        "build",
        workdir=(idf_copy / "examples" / "system" / "ulp" / "ulp_riscv" / "gpio"),
    )
...
def test_spaces_bundle2(idf_copy: Path) -> None:
    ...
    run_idf_py(
        "build", workdir=(idf_copy / "examples" / "security" / "flash_encryption")
    )
    # test_x509_cert_bundle
    run_idf_py(
        "build", workdir=(idf_copy / "examples" / "protocols" / "https_x509_bundle")
    )
...
def test_spaces_bundle3(idf_copy: Path) -> None:
    ...
    # test secure_boot_v1
    run_idf_py(
        "-DSDKCONFIG_DEFAULTS=sdkconfig.defaults;sdkconfig.ci.00",
        "build",
    ...
    # test secure_boot_v2
    run_idf_py(
        "-DSDKCONFIG_DEFAULTS=sdkconfig.defaults;sdkconfig.ci.01",
        "build",
        workdir=secure_boot_app_path,
    )
    clean_app_dir(secure_boot_app_path)
    # test app_signing
    run_idf_py(
        "-DSDKCONFIG_DEFAULTS=sdkconfig.defaults;sdkconfig.ci.02",
        "build",
        workdir=secure_boot_app_path,
    )
    clean_app_dir(secure_boot_app_path)
...
def test_install_export_unix(idf_copy: Path) -> None:
    ...
    subprocess.check_call(
        export_cmd, env=env, shell=True, cwd=idf_copy, executable="/bin/bash"
    )
