...
def test_sdkconfig_contains_all_files(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    (test_app_copy / "sdkconfig.defaults.esp32").write_text(
...
def test_sdkconfig_multiple_default_files(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    logging.info("should be able to specify multiple sdkconfig default files")
    (test_app_copy / "sdkconfig.defaults1").write_text(
        "CONFIG_PARTITION_TABLE_OFFSET=0x10000"
    )
    (test_app_copy / "sdkconfig.defaults2").write_text(
        "CONFIG_PARTITION_TABLE_TWO_OTA=y"
    )
    idf_py(
        "-DSDKCONFIG_DEFAULTS=sdkconfig.defaults1;sdkconfig.defaults2", "reconfigure"
    )
    assert all(
        [
            file_contains((test_app_copy / "sdkconfig"), x)
...
def test_keep_idf_init_version(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    logging.info(
        "sdkconfig should contain initial IDF version, even after regenerating the config"
    )
    (test_app_copy / "sdkconfig").write_text('CONFIG_IDF_INIT_VERSION="1.2.3"')
    idf_py("reconfigure")
    assert file_contains(
        (test_app_copy / "sdkconfig"), 'CONFIG_IDF_INIT_VERSION="1.2.3"'
    )
...
def test_empty_idf_init_version(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    assert file_contains(
        (test_app_copy / "sdkconfig"), f'CONFIG_IDF_INIT_VERSION="{version}"'
    )
