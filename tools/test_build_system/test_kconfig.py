...
def test_kconfig_deprecated_options(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    with backup_required_files(test_app_copy):
        ...
        (idf_path / "sdkconfig.rename").write_text(
            "CONFIG_TEST_OLD_OPTION CONFIG_TEST_NEW_OPTION"
        )
        append_to_file(
            (idf_path / "Kconfig"),
            textwrap.dedent("""
        ...
        assert all(
            [
                file_contains((test_app_copy / "sdkconfig"), x)
                for x in ["CONFIG_TEST_OLD_OPTION=y", "CONFIG_TEST_NEW_OPTION=y"]
            ]
        )
        assert all(
            [
                file_contains((test_app_copy / "build" / "config" / "sdkconfig.h"), x)
                for x in [
                    "#define CONFIG_TEST_NEW_OPTION 1",
        ...
        assert all(
            [
                file_contains(
                    (test_app_copy / "build" / "config" / "sdkconfig.cmake"), x
                )
                for x in [
                    'set(CONFIG_TEST_OLD_OPTION "y")',
    ...
    assert all(
        [
            file_contains((test_app_copy / "sdkconfig"), x)
            for x in ["CONFIG_TEST_OLD_OPTION=7", "CONFIG_TEST_NEW_OPTION=7"]
        ]
    )
...
def test_kconfig_multiple_and_target_specific_options(
    ...
    with backup_required_files(test_app_copy):
        logging.info(
            "Can have multiple deprecated Kconfig options map to a single new option"
        )
        (idf_path / "sdkconfig.rename").write_text("")
        idf_py("reconfigure")
        append_to_file((test_app_copy / "sdkconfig"), "CONFIG_TEST_NEW_OPTION=y")
        append_to_file(
            (idf_path / "sdkconfig.rename"),
            "\n".join(
                [
                    "CONFIG_TEST_OLD_OPTION_1 CONFIG_TEST_NEW_OPTION",
        ...
        assert all(
            [
                file_contains((test_app_copy / "sdkconfig"), x)
                for x in ["CONFIG_TEST_OLD_OPTION_1=y", "CONFIG_TEST_OLD_OPTION_2=y"]
            ]
        )
        assert all(
            [
                file_contains((test_app_copy / "build" / "config" / "sdkconfig.h"), x)
                for x in [
                    "#define CONFIG_TEST_OLD_OPTION_1 CONFIG_TEST_NEW_OPTION",
        ...
        assert all(
            [
                ...
                for x in [
                    'set(CONFIG_TEST_OLD_OPTION_1 "y")',
    ...
    append_to_file(
        (idf_path / "components" / "esp_system" / "sdkconfig.rename.esp32s2"),
        "CONFIG_TEST_OLD_OPTION CONFIG_TEST_NEW_OPTION",
    )
    append_to_file(
        (idf_path / "Kconfig"),
    ...
    assert all(
        [
            file_contains((test_app_copy / "sdkconfig"), x)
            for x in ["CONFIG_TEST_NEW_OPTION=y", "CONFIG_TEST_OLD_OPTION=y"]
        ]
    )
