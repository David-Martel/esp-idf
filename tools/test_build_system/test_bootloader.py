...
def test_bootloader_custom_overrides_original(
    test_app_copy: Path, idf_py: IdfPyFunc, default_idf_env: EnvDict
) -> None:
    ...
    shutil.copytree(
        idf_path / "components" / "esp_bootloader_format",
        test_app_copy / "components" / "esp_bootloader_format",
    )
    idf_py("bootloader")
    assert file_contains(
        test_app_copy / "build" / "bootloader" / "compile_commands.json",
        (
            test_app_copy
            / "components"
            / "bootloader"
...
def test_bootloader_custom_ignores_extra_component(
    test_app_copy: Path, idf_py: IdfPyFunc, default_idf_env: EnvDict
) -> None:
    ...
    default_message = bytes(
        "Custom bootloader message defined in the KConfig file.", "ascii"
    )
    assert bin_file_contains(
        test_app_copy / "build" / "bootloader" / "bootloader.bin", default_message
    )
    # Clean the project
    idf_py("fullclean")
    ...
    assert not bin_file_contains(
        test_app_copy / "build" / "bootloader" / "bootloader.bin", default_message
    )


def test_bootloader_correctly_set_image_header(
    ...
    idf_py("bootloader")
    assert (
        get_two_header_bytes(test_app_copy / "build" / "bootloader" / "bootloader.bin")
        == "0210"
    )
