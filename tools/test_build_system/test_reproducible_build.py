...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "blink", marks=[pytest.mark.test_app_copy("examples/get-started/blink")]
        ),
        pytest.param(
            "blecent",
            marks=[pytest.mark.test_app_copy("examples/bluetooth/nimble/blecent")],
        ),
    ],
)
def test_reproducible_builds(
    app_name: str, idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    ...
    strings_output = subprocess.check_output(
        ["xtensa-esp32-elf-strings", str(elf_file)], encoding="utf-8"
    )
    idf_path = os.environ["IDF_PATH"]
    assert str(idf_path) not in strings_output, f"{idf_path} found in {elf_file}"
    assert str(test_app_copy) not in strings_output, (
        f"{test_app_copy} found in {elf_file}"
    )
    ...
    assert str(toolchain_path) not in strings_output, (
        f"{toolchain_path} found in {elf_file}"
    )
    ...
    for artifact in artifacts_to_check:
        ...
        assert not bin_files_differ(path_first, path_second), (
            f"{path_first} and {path_second} differ"
        )
    ...
    assert "No such file or directory" not in gdb_output, (
        f"GDB failed to find app_main in {elf_file}:\n{gdb_output}"
    )
