...
def test_build_custom_cmake_project(test_app_copy: Path) -> None:
    ...
    for target in ["esp32", "esp32s3", "esp32c6", "esp32h2"]:
        logging.info(
            f"Test build ESP-IDF as a library to a custom CMake projects for {target}"
        )
        ...
        run_cmake_and_build(
            ...
            "-DCMAKE_TOOLCHAIN_FILE={}".format(
                idf_path / "tools" / "cmake" / f"toolchain-{target}.cmake"
            ),
            f"-DTARGET={target}",
        )
        assert file_contains(
            (test_app_copy / "build" / "compile_commands.json"), '"command"'
        )
...
def test_build_cmake_library_psram_workaround(test_app_copy: Path) -> None:
    ...
    with open(
        (test_app_copy / "build" / "compile_commands.json"), "r", encoding="utf-8"
    ) as f:
        data = f.read()
...
def test_build_cmake_library_psram_strategies(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
    for strategy in ["DUPLDST", "NOPS", "MEMW"]:
        logging.info(
            f"Test for external libraries in custom CMake projects with PSRAM strategy {strategy}"
        )
        (test_app_copy / "sdkconfig.defaults").write_text(
            "\n".join(
                [
        ...
        with open(
            ...
            for r in res:
                assert f"mfix-esp32-psram-cache-strategy={strategy.lower()}" in r, (
                    "All commands in compile_commands.json "
...
def test_defaults_unspecified_build_args(idf_copy: Path) -> None:
    ...
    ret = run_cmake(
        ...
        "Ninja",
        "-DCMAKE_TOOLCHAIN_FILE={}".format(
            str(idf_copy / "tools" / "cmake" / "toolchain-esp32.cmake")
        ),
        "-DTARGET=esp32",
