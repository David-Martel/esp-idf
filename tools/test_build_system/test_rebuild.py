...
def rebuild_and_check(
    ...
    should_not_be_rebuilt: Union[str, List[str]],
) -> None:
    """
...
def test_rebuild_source_files(idf_py: IdfPyFunc) -> None:
    ...
    replace_in_file(
        "sdkconfig", "# CONFIG_FREERTOS_UNICORE is not set", "CONFIG_FREERTOS_UNICORE=y"
    )
    rebuild_and_check(
        idf_py, APP_BINS + BOOTLOADER_BINS + obj_files + sdkconfig_files, PARTITION_BIN
    )
    ...
    rebuild_and_check(
        idf_py, APP_BINS + obj_files, PARTITION_BIN + BOOTLOADER_BINS + sdkconfig_files
    )
