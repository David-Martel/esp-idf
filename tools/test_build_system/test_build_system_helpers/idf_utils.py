...
def get_idf_build_env(idf_path: str) -> EnvDict:
    ...
    idf_tool_py_env = {
        ...
        for key, value in [line.split("=") for line in keys_values.splitlines()]
    }
...
def run_idf_py(
    *args: str,
    ...
    if idf_path is None:
        ...
        if not idf_path:
            raise ValueError(
                "IDF_PATH must be set in the env array if idf_path argument is not set"
            )
    if python is None:
    ...
    try:
        ...
        logging.error(
            "The following idf.py command has failed: {}".format(" ".join(cmd))
        )
...
def run_cmake(
    *cmake_args: str,
    ...
    try:
        ...
        logging.error(
            "The following cmake command has failed: {}".format(" ".join(cmd))
        )
        logging.error("Working directory: {}".format(workdir))
        logging.error("Stdout: {}".format(e.stdout))
        logging.error("Stderr: {}".format(e.stderr))
