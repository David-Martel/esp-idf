...
class Snapshot:
    ...
    def assert_same(self, other: "Snapshot") -> None:
        ...
        for my_info, other_info in zip(self.info, other.info):
            assert my_info[0] == other_info[0], (
                f"Snapshots mismatch, expected file {my_info[0]}, found {other_info[0]}"
            )
            assert my_info[1] == other_info[1], (
                f"File {my_info[0]} timestamp has changed! diff={(other_info[1] - my_info[1]) / 10**9}"
            )
    ...
    def assert_different(self, other: "Snapshot") -> None:
        ...
        for my_info, other_info in zip(self.info, other.info):
            ...
            assert my_info[1] != other_info[1], (
                f"File {my_info[0]} timestamp has not changed! ({my_info[1]})"
            )


def get_snapshot(
    glob_patterns: Union[str, List[str]],
    ...
    if isinstance(exclude_patterns, str):
        ...
        raise ValueError(
            f"Unexpcted type of exclude_patterns: ({type(exclude_patterns)}"
        )

    # whether the path found by glob.glob should be excluded?
    def should_exclude(filename: str) -> bool:
        return os.path.isdir(filename) or any(
            (fnmatch.fnmatch(filename, pattern) for pattern in exclude_patterns_list)
        )

    files_to_snapshot: List[str] = []
    for pattern in glob_patterns:
        found = filter(
            lambda f: not should_exclude(f), glob.glob(pattern, recursive=True)
        )
        if not found:
