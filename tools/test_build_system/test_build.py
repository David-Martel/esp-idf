...
def test_build_alternative_directories(
    idf_py: IdfPyFunc, func_work_dir: Path, test_app_copy: Path
) -> None:
    ...
    assert os.listdir(build_subdir_inside_build_dir) != [], (
        "No files found in new build directory!"
    )
...
def test_build_with_cmake_and_idf_path_unset(
    ...
    kconfig_file.write_text(
        'source "$IDF_PATH/examples/wifi/getting_started/station/main/Kconfig.projbuild"'
    )
    run_cmake_and_build("-G", "Ninja", "..", "-DIDF_PATH={}".format(idf_path), env=env)
    assert_built(BOOTLOADER_BINS + APP_BINS + PARTITION_BIN)
    ...
    run_cmake_and_build(
        "-G", "Ninja", "-D", "ci_idf_path={}".format(idf_path), "..", env=env
    )
    assert_built(BOOTLOADER_BINS + APP_BINS + PARTITION_BIN)
...
def test_build_skdconfig_phy_init_data(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    idf_py("build")
    assert_built(
        BOOTLOADER_BINS + APP_BINS + PARTITION_BIN + ["build/phy_init_data.bin"]
    )


def test_build_compiler_flag_in_source_file(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
...
def test_build_with_sdkconfig_build_abspath(
    idf_py: IdfPyFunc, test_app_copy: Path
) -> None:
...
def test_build_dfu(idf_py: IdfPyFunc) -> None:
    ...
    assert (
        'command "dfu" is not known to idf.py and is not a Ninja target' in ret.stderr
    ), "DFU build should fail for default chip target"
    idf_py("set-target", "esp32s2")
    ret = idf_py("dfu")
    assert (
        'build/dfu.bin" has been written. You may proceed with DFU flashing.'
        in ret.stdout
...
def test_build_uf2(idf_py: IdfPyFunc) -> None:
    ...
    ret = idf_py("uf2-app")
    assert (
        "build/uf2-app.bin, ready to be flashed with any ESP USB Bridge" in ret.stdout
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="Windows does not support stat commands"
)
def test_build_with_crlf_files(
    idf_py: IdfPyFunc, test_app_copy: Path, idf_copy: Path
) -> None:
...
def test_build_cmake_executable_suffix(idf_py: IdfPyFunc, test_app_copy: Path) -> None:
    ...
    append_to_file(
        (test_app_copy / "CMakeLists.txt"), 'set(CMAKE_EXECUTABLE_SUFFIX_CXX ".ext")'
    )
    ret = idf_py("build")
    assert "Project build complete" in ret.stdout, (
        "Build with CMAKE_EXECUTABLE_SUFFIX set failed"
    )
