...
def action_write(args):  # type: (typing.Mapping[str, typing.Any]) -> None
    ...
    print(
        "Adding flash chip parameters file with flash_size = {}".format(
            args["flash_size"]
        )
    )
    ...
    writer.finish()
    print(
        '"{}" has been written. You may proceed with DFU flashing.'.format(
            args["output_file"].name
        )
    )
    if args["part_size"] % (4 * 1024) != 0:
        print(
            "WARNING: Partition size of DFU is not multiple of 4k (4096). You might get unexpected behavior."
        )
...
def main():  # type: () -> None
    ...
    write_parser.add_argument(
        "files", metavar="<address> <file>", help="Add <file> at <address>", nargs="*"
    )
    write_parser.add_argument(
        "-fs",
        "--flash-size",
    ...
    if args.files:
        files += [
            (int(addr, 0), check_file(f_name))
            for addr, f_name in zip(args.files[::2], args.files[1::2])
        ]
    ...
    if args.json:
        ...
        def process_json_file(path):  # type: (str) -> str
            ...
            """
            return check_file(
                os.path.relpath(os.path.join(json_dir, path), start=os.curdir)
            )

        with open(args.json, encoding="utf-8") as f:
            files += [
                (int(addr, 0), process_json_file(f_name))
                for addr, f_name in json.load(f)["flash_files"].items()
            ]
