...
def strip_blank_lines(input_filename, output_filename):
    with (
        open(input_filename, "r", encoding="utf-8") as read_from,
        open(output_filename, "w", newline="", encoding="utf-8") as write_to,
    ):
    ...
    with open(input_config_file, "r", encoding="utf-8") as config_file:
        ...
        for line_num, line in enumerate(config_file_reader, start=1):
            if line[1] not in valid_datatypes:
                raise SystemExit(
                    "Error: config file: %s has invalid datatype at line no:%s\n`"
                    % (str(input_config_file), str(line_num))
                )
            if "namespace" not in line and line[2] not in valid_encodings:
                raise SystemExit(
                    "Error: config file: %s has invalid encoding at line no:%s\n`"
                    % (str(input_config_file), str(line_num))
                )
    ...
    with open(input_config_file, "r", encoding="utf-8") as config_file:
        ...
        for line_num, line in enumerate(config_file_reader, start=1):
            if len(line) != 3 and line[0] not in keys_repeat:
                raise SystemExit(
                    "Error: data missing in config file at line no:%s <format needed:key,type,encoding>\n"
                    % str(line_num)
                )


def verify_data_in_file(
    input_config_file,
    input_values_file,
    config_file_keys,
    keys_in_values_file,
...
def add_config_data_per_namespace(input_config_file):
    ...
    with open(input_config_file, "r", encoding="utf-8") as csv_config_file:
        ...
        if (not config_data_to_write) or (
            config_data_to_write and config_data_per_namespace
        ):
            config_data_to_write.append(config_data_per_namespace)
...
def set_repeat_value(total_keys_repeat, keys, csv_file, target_filename):
    with (
        open(csv_file, "r", encoding="utf-8") as read_from,
        open(target_filename, "w", newline="", encoding="utf-8") as write_to,
    ):
...
def create_intermediate_csv(args, keys_in_values_file, keys_repeat, is_encr=False):
    ...
    try:
        ...
        if keys_repeat:
            target_values_file = set_repeat_value(
                keys_repeat, keys, args.values, target_filename
            )
        else:
        ...
        with open(target_values_file, "r", encoding="utf-8") as csv_values_file:
            ...
            for values_data_line in values_file_reader:
                ...
                file_identifier_value = get_fileid_val(
                    args.fileid, key_value_data, file_identifier_value
                )
                ...
                if args.prefix_num:
                    ...
                    csv_filename = (
                        args.prefix + "-" + file_identifier_value + "." + "csv"
                    )
                    bin_filename = (
                        args.prefix + "-" + file_identifier_value + "." + "bin"
                    )
                    if args.keygen:
                        args.keyfile = (
                            "keys-" + args.prefix + "-" + file_identifier_value
                        )
                ...
                if os.path.isfile(output_csv_file):
                    raise SystemExit(
                        "Target csv file: %s already exists.`" % output_csv_file
                    )
                ...
                if os.path.isfile(output_bin_file):
                    raise SystemExit(
                        "Target binary file: %s already exists.`" % output_bin_file
                    )
...
def verify_file_format(args):
    ...
    if conf_extension != ".csv":
        raise SystemExit(
            "Error: config file: %s does not have the .csv extension." % args.conf
        )
    values_name, values_extension = os.path.splitext(args.values)
    if values_extension != ".csv":
        raise SystemExit(
            "Error: values file: %s does not have the .csv extension." % args.values
        )
...
def generate(args):
    ...
    verify_data_in_file(
        args.conf, args.values, keys_in_config_file, keys_in_values_file, keys_repeat
    )
    ...
    create_intermediate_csv(
        args, keys_in_values_file, keys_repeat, is_encr=encryption_enabled
    )
...
def main():
    try:
        ...
        parser_gen.add_argument(
            "conf", default=None, help="Path to configuration csv file to parse"
        )
        parser_gen.add_argument(
            "values", default=None, help="Path to values csv file to parse"
        )
        parser_gen.add_argument(
            "prefix", default=None, help="Unique name for each output filename prefix"
        )
        parser_gen.add_argument(
            "size",
            default=None,
        ...
        parser_gen.add_argument("--keyfile", default=None, help=argparse.SUPPRESS)
        parser_gen.add_argument(
            "--inputkey",
            default=None,
        ...
        parser_gen.add_argument(
            "--kp_hmac_keyfile", default=None, help="Path to output HMAC key file"
        )
        parser_gen.add_argument(
            "--kp_hmac_inputkey",
            default=None,
        ...
        parser_gen.add_argument("--input", default=None, help=argparse.SUPPRESS)
        parser_gen.add_argument("--output", default=None, help=argparse.SUPPRESS)
        ...
        parser_gen_key.add_argument(
            "--keyfile", default=None, help="Path to output encryption keys file"
        )
        parser_gen_key.add_argument(
            "--outdir",
            default=os.getcwd(),
