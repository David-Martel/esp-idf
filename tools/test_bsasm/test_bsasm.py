...
class TestAssembler(unittest.TestCase):
    ...
    def compare(self, out: Any, js: Any, base: str) -> None:
        self.assertEqual(
            type(out),
            type(js),
            " Diverging types between json and decoded obj: " + base,
        )
        if type(js) == dict:
            for k in js:
                self.assertTrue(
                    k in out, " Key not found in decoded output: " + base + "." + k
                )
            ...
            self.assertEqual(
                js, out, f"Items different: {base} (json {js} decoded {out})"
            )
    ...
    def test_examples(self) -> None:
        ...
        for f in testfiles:
            ...
            p = subprocess.run(args, check=False, timeout=10)
