...
def get_component_git_files() -> Set[str]:
    output = subprocess.check_output(
        ["git", "ls-files", "components/"], cwd=IDF_PATH
    ).decode()
...
def get_component_files_from_patterns(patterns: List[str]) -> Set[str]:
    ...
    for pattern in patterns:
        ...
        try:
            _files = set(
                glob.iglob(
                    pattern, root_dir=IDF_PATH, recursive=True, include_hidden=True
                )
            )  # type: ignore
        except TypeError:
...
def main() -> None:
    with open(RULES_FILE, "r") as f:
        ...
        patterns_components = [
            p
            for p in rules[".patterns-build_components"]
            if p.startswith("components/")
        ]
        submodules = [
            p for p in rules[".patterns-submodule"] if p.startswith("components/")
        ]
        # resolve components in patterns_components
        patterns_components = [
            f"{p}/**/*" if p in submodules else p for p in patterns_components
        ]
    ...
    components_build_files = get_all_components_files().difference(
        components_test_files
    )

    if check_test_files := components_test_files & rules_files:
        # rules should not include test files
        print(
            f"Found test files should not in .patterns-build_components: {check_test_files.pop()}"
        )
        exit(1)
    if check_difference := components_build_files.difference(rules_files):
        # rules should include all build files
        print(
            f"Found components files should in .patterns-build_components but not: {check_difference.pop()}"
        )
        exit(1)
    if check_difference := rules_files.difference(components_build_files):
        print(
            f"Found components files should not in .patterns-build_components: {check_difference.pop()}"
        )
        exit(1)
