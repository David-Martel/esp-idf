...
def action_validate(
    req_obj: Any,
) -> None:  # "Any" because we are checking this in this script
    """
...
def action_check_directory(
    ...
    for file_name in os.listdir(REQ_DIR):
        ...
        if m:
            if m.group(1) not in features:
                raise RuntimeError(
                    f"Cannot find a feature for {file_name} in {JSON_PATH}"
                )
            features_found.add(m.group(1))
        else:
            raise RuntimeError(
                f"{file_name} in {REQ_DIR} doesn't match the expected name"
            )
    ...
    if len(features_not_found) > 0:
        raise RuntimeError(
            f"There are no requirements file in {REQ_DIR} for {features_not_found}"
        )
