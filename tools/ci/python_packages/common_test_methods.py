...
try:
    ...
    netifaces = MagicMock()
    logging.warning(
        "netifaces is not installed. Please install it to get network interface information."
    )
...
def get_host_ip6_by_dest_ip(dest_ip: str, interface: str) -> str:
    addr_info = socket.getaddrinfo(
        f"{dest_ip}%{interface}", 80, socket.AF_INET6, socket.SOCK_DGRAM
    )
