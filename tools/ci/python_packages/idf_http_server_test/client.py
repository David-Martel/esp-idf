...
def test_get_handler(ip, port, verbosity=False):
    ...
    if not (
        test_val("Status code mismatch", 200, resp.status)
        and test_val(
            "Response mismatch", "Custom-Value-1", resp.getheader("Custom-Header-1")
        )
        and test_val(
            "Response mismatch", "Custom-Value-2", resp.getheader("Custom-Header-2")
        )
        and test_val("Response mismatch", "Hello World!", resp_data)
    ):
        return False
...
def test_put_handler(ip, port, verbosity=False):
    ...
    try:
        ...
        if not test_val(
            "Response mismatch", "Nothing matches the given URI", resp_data
        ):
            raise AssertionError
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "msg", metavar="message", type=str, help="Message to be sent to server"
    )
    ...
    if not (
        ...
        and test_put_handler(ip, port, True)
        and test_post_handler(ip, port, msg, True)
    ):
        logging.info("Failed!")
