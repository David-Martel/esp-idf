...
class TestResult(object):
    ...
    PC_BANDWIDTH_LOG_PATTERN = re.compile(
        r"(\d+\.\d+)\s*-\s*(\d+.\d+)\s+sec\s+[\d.]+\s+MBytes\s+([\d.]+)\s+Mbits\/sec"
    )
    DUT_BANDWIDTH_LOG_PATTERN = re.compile(
        r"([\d.]+)\s*-\s*([\d.]+)\s+sec\s+([\d.]+)\s+Mbits/sec"
    )
    ...
    def _save_result(
        self, throughput: float, ap_ssid: str, att: int, rssi: int, heap_size: str
    ) -> None:
        """
        ...
        * record the min heap size.
        ...
        def record_throughput(database: dict, key_value: int) -> None:
            try:
                ...
                database[ap_ssid][key_value] = max(database[ap_ssid][key_value], throughput)
            except KeyError:
        ...
        self.heap_size = min(self.heap_size, int(heap_size))

    def add_result(
        self, raw_data: str, ap_ssid: str, att: int, rssi: int, heap_size: str
    ) -> float:
        """
        ...
        for result in result_list:
            ...
            if (
                throughput == 0
                and rssi > self.ZERO_POINT_THRESHOLD
    ...
    def draw_throughput_figure(self, path: str, ap_ssid: str, draw_type: str) -> str:
        ...
        if isinstance(ap_ssid, list):
            file_name = "ThroughputVs{}_{}_{}.html".format(
                type_name, self.proto, self.direction
            )
        else:
            file_name = "ThroughputVs{}_{}_{}.html".format(
                type_name, self.proto, self.direction
    ...
    def get_best_throughput(self) -> Any:
        ...
        best_for_aps = [
            max(self.throughput_by_att[ap_ssid].values())
    ...
    def __str__(self) -> str:
        ...
        if self.throughput_by_att:
            ...
            for ap_ssid in self.throughput_by_att:
                ret += "[{}]: {:.02f} Mbps\r\n".format(
                    ap_ssid, max(self.throughput_by_att[ap_ssid].values())
                )
...
class IperfTestUtility(object):
    ...
    def setup(self) -> Tuple[str, int]:
        ...
        dut_ip = self.dut.expect(
            r"sta ip: ([\d.]+), mask: ([\d.]+), gw: ([\d.]+)"
        ).group(1)
        return dut_ip, rssi

    def _save_test_result(
        self, test_case: str, raw_data: str, att: int, rssi: int, heap_size: int
    ) -> Any:
        return self.test_result[test_case].add_result(
            raw_data, self.ap_ssid, att, rssi, heap_size
        )

    def _test_once(
        self, proto: str, direction: str, bw_limit: int
    ) -> Tuple[str, int, int]:
        ...
        if direction == "tx":
            with open(PC_IPERF_TEMP_LOG_FILE, "w") as f:
                if proto == "tcp":
                    ...
                    if bw_limit > 0:
                        self.dut.write(
                            "iperf -c {} -i 1 -t {} -b {}".format(
                                self.pc_nic_ip, TEST_TIME, bw_limit
                            )
                        )
                    else:
                        self.dut.write(
                            "iperf -c {} -i 1 -t {}".format(self.pc_nic_ip, TEST_TIME)
                        )
                else:
                    process = subprocess.Popen(
                        [
                    ...
                    if bw_limit > 0:
                        self.dut.write(
                            "iperf -c {} -u -i 1 -t {} -b {}".format(
                                self.pc_nic_ip, TEST_TIME, bw_limit
                            )
                        )
                    else:
                        self.dut.write(
                            "iperf -c {} -u -i 1 -t {}".format(
                                self.pc_nic_ip, TEST_TIME
                            )
                        )
            ...
            with open(PC_IPERF_TEMP_LOG_FILE, "w") as f:
                if proto == "tcp":
                    ...
                    if bw_limit > 0:
                        ...
                        process = subprocess.Popen(
                            ["iperf", "-c", dut_ip, "-t", str(TEST_TIME), "-f", "m"],
                            stdout=f,
                            stderr=f,
                        )
                    for _ in range(TEST_TIMEOUT):
            ...
            server_raw_data = self.dut.expect(pexpect.TIMEOUT, timeout=5).decode(
                "utf-8"
            )
            with open(PC_IPERF_TEMP_LOG_FILE, "r") as f:
                pc_raw_data = f.read()
    ...
    def run_test(
        self, proto: str, direction: str, atten_val: int, bw_limit: int
    ) -> None:
        """
        ...
        try:
            ...
            self._save_test_result(
                "{}_{}".format(proto, direction), "", atten_val, rssi, heap_size
            )
    ...
    def run_all_cases(self, atten_val: int, bw_limit: int) -> None:
        ...
        if self.fail_to_scan > 10:
            logging.info(
                "Fail to scan AP for more than 10 times. Lowest RSSI scanned is {}".format(
                    self.lowest_rssi_scanned
                )
            )
    ...
    def wait_ap_power_on(self) -> bool:
        ...
        for _ in range(WAIT_AP_POWER_ON_TIMEOUT // SCAN_TIMEOUT):
            try:
                ...
                self.dut.expect(
                    r"\[{}]\[rssi=(-\d+)]".format(self.ap_ssid), timeout=SCAN_TIMEOUT
                )
                ret = True
                break
            except pexpect.TIMEOUT:
