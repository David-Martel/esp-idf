...
class wpa_cli:
    def __init__(self, iface, reset_on_exit=False):
        ...
        try:
            ...
            service = dbus.Interface(
                bus.get_object("fi.w1.wpa_supplicant1", "/fi/w1/wpa_supplicant1"),
                "fi.w1.wpa_supplicant1",
            )
            iface_path = service.GetInterface(self.iface_name)
            self.iface_obj = bus.get_object("fi.w1.wpa_supplicant1", iface_path)
            self.iface_ifc = dbus.Interface(
                self.iface_obj, "fi.w1.wpa_supplicant1.Interface"
            )
            self.iface_props = dbus.Interface(
                self.iface_obj, "org.freedesktop.DBus.Properties"
            )
    ...
    def connect(self, ssid, password):
        try:
            ...
            print(
                "Pre-connect state is %s, IP is %s"
                % (self._get_iface_property("State"), get_wiface_IPv4(self.iface_name))
            )

            self.new_network = self.iface_ifc.AddNetwork(
            ...
            while retry > 0:
                ...
                print(
                    "wpa iface state %s (scanning %s)"
                    % (state, bool(self._get_iface_property("Scanning")))
                )
