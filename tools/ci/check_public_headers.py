...
class HeaderFailedContainsStaticAssert(HeaderFailed):
    def __str__(self) -> str:
        return (
            "Header uses _Static_assert or static_assert instead of ESP_STATIC_ASSERT"
        )


#   Creates a temp file and returns both output as a string and a file name
#
def exec_cmd_to_temp_file(
    what: List, suffix: str = ""
...
def exec_cmd(
    what: List,
    out_file: Union["tempfile._TemporaryFileWrapper[bytes]", int] = subprocess.PIPE,
) -> Tuple[int, str, str, str]:
    p = subprocess.Popen(
        what, stdin=subprocess.PIPE, stdout=out_file, stderr=subprocess.PIPE
    )
...
class PublicHeaderChecker:
    ...
    def __init__(
        self, verbose: bool = False, jobs: int = 1, prefix: Optional[str] = None
    ) -> None:
        ...
        self.error_orphan_kconfig = re.compile(
            r"#error CONFIG_VARS_USED_WHILE_SDKCONFIG_NOT_INCLUDED"
        )
        ...
        self.auto_soc_header = re.compile(
            r"components/soc/esp[a-z0-9_]+(?:/\w+)?/(include|register)/(soc|modem)/[a-zA-Z0-9_]+.h"
        )
        self.assembly_nocode = (
            r"^\s*(\.file|\.text|\.ident|\.option|\.attribute|(\.section)?).*$"
        )
    ...
    def compile_one_header_with(
        self, compiler: str, std_flags: str, header: str
    ) -> None:
        rc, out, err, cmd = exec_cmd(
            [compiler, std_flags, "-S", "-o-", "-include", header, self.main_c]
    ...
    def preprocess_one_header(self, header: str, num: int) -> None:
        ...
        rc, out, err, _ = exec_cmd(
            [self.gcc, "-fpreprocessed", "-dD", "-P", "-E", header]
            + self.include_dir_flags
        )
        ...
        if (
            grp
            and not re.search(self.defines_assert, out)
            and not re.search(self.auto_soc_header, normalized_path)
        ):
            self.log(
                "{}: FAILED: contains {}. Please use ESP_STATIC_ASSERT".format(
                    header, grp.group(1)
                ),
                True,
            )
        ...
        try:
            ...
            rc, cpp_out, err, cpp_out_file, cmd = exec_cmd_to_temp_file(
                [self.gpp, self.stdcpp] + all_compilation_flags
            )
            if rc != 0:
                if re.search(self.error_macro, err):
                    if re.search(self.error_orphan_kconfig, err):
                        self.log(
                            "{}: CONFIG_VARS_USED_WHILE_SDKCONFIG_NOT_INCLUDED".format(
                                header
                            ),
                            True,
                        )
            ...
            rc, _, err, c_out_file, _ = exec_cmd_to_temp_file(
                [self.gcc, self.stdc] + all_compilation_flags
            )
            if rc != 0:
                self.log(
                    "{} FAILED: compilation in C (while C++ compilation passes)".format(
                        header
                    )
                )
                raise HeaderFailedPreprocessError()
            ...
            if not diff or diff.isspace():
                if not cpp_out or cpp_out.isspace():
                    ...
                    return
                self.log(
                    "{} FAILED C and C++ preprocessor output is the same!".format(
                        header
                    ),
                    True,
                )
            ...
            if re.search(self.extern_c, diff):
                ...
                if re.search(self.extern_c, out):
                    self.log(
                        "{} extern C present in the actual header, too - OK".format(
                            header
                        )
                    )
                    return
                ...
                if without_macros.isspace():
                    self.log(
                        "{} Header doesn't need extern-C, it's all just macros - OK".format(
                            header
                        )
                    )
                    return
                ...
                without_macros = re.sub(
                    macros_calls, "", without_macros, flags=re.DOTALL
                )
                if without_macros.isspace():
                    self.log(
                        "{} Header doesn't need extern-C, it's all macros definitions and calls - OK".format(
                            header
                        )
                    )
                    return
    ...
    def list_public_headers(
        ...
        def get_std(json: List, extension: str) -> str:
            ...
            command = [
                c
                for c in j
                if c["file"].endswith("." + extension) and "-std=" in c["command"]
            ][0]
            return str(
                [s for s in command["command"].split() if "std=" in s][0]
            )  # grab the std flag
        ...
        for file_name in all_include_files:
            ...
            if any(
                [os.path.commonprefix([d, rel_path_file]) == d for d in ignore_dirs]
            ):
...
def check_all_headers() -> None:
    ...
    * Copy, paste and execute the compilation commands to reproduce build errors (script prints out
    ...
    parser.add_argument(
        "--verbose", "-v", help="enables verbose mode", action="store_true"
    )
    parser.add_argument(
        "--jobs", "-j", help="number of jobs to run checker", default=1, type=int
    )
    parser.add_argument(
        "--prefix", "-p", help="compiler prefix", default="xtensa-esp32-elf-", type=str
    )
    parser.add_argument(
        "--exclude-file",
        "-e",
        help="exception file",
        default="check_public_headers_exceptions.txt",
        type=str,
    )
    parser.add_argument(
        "--only-dir",
        "-d",
        help="reduce the analysis to this directory only",
        default=None,
        type=str,
    )
    ...
    with PublicHeaderChecker(args.verbose, args.jobs, args.prefix) as header_check:
        header_check.list_public_headers(
            ignore_dirs, ignore_files, only_dir=args.only_dir
        )
        try:
            header_check.join()
