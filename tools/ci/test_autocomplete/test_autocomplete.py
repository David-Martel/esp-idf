...
Test = collections.namedtuple(
    "Test", ["name", "shell", "term", "prefix", "pattern", "ext"]
)

IDF_PATH = os.environ["IDF_PATH"]

TESTS = (
    Test("base", "fish", "vt100", "", "all.*app.*app-flash.*bootloader.*", "fish"),
    Test(
        "base",
        "bash",
        "xterm-256color",
    ...
    Test("file", "fish", "vt100", f"@{IDF_PATH}/README", ".*README.*md.*", "fish"),
    Test(
        "file", "bash", "xterm-256color", f"@{IDF_PATH}/README", ".*README.*md.*", "sh"
    ),
    Test("file", "zsh", "", f"@{IDF_PATH}/README", ".*README.*md.*", "sh"),
)
...
def get_fail_msg(pproc: pexpect.spawn, msg: str, index: int) -> str:
    ...
    return '{} ({}) buffer: "{}"'.format(
        msg, "EOF - child has exited" if index == 1 else "TIMEOUT", buf
    )
...
class UTTest(unittest.TestCase):
    def test_shell(self) -> None:
        ...
        for test in TESTS:
            with self.subTest():
                with open(
                    os.path.join(IDF_PATH, f"{test.shell}.{test.name}.out"), "wb"
                ) as o:
                    env["TERM"] = test.term
                    with pexpect.spawn(
                        f"{test.shell} -i", env=env, logfile=o, timeout=200
                    ) as pproc:
                        if test.shell == "bash":
                        ...
                        self.assertEqual(
                            i, 0, get_fail_msg(pproc, "Export was not successful!", i)
                        )
                        ...
                        self.assertEqual(
                            ...
                            get_fail_msg(
                                pproc,
                                f"Autocompletion for idf.py failed in {test.shell}!",
                                i,
                            ),
                        )
