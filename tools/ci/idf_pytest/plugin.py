...
class IdfPytestEmbedded:
    ...
    def item_to_pytest_case(self, item: Function) -> t.Optional[PytestCase]:
        ...
        app_paths = to_list(
            parse_multi_dut_args(
                count, self.get_param(item, "app_path", os.path.dirname(item.path))
            )
        )
        configs = to_list(
            parse_multi_dut_args(
                count, self.get_param(item, "config", DEFAULT_SDKCONFIG)
            )
        )
        ...
        if count > 1 and targets == [None] * count:
            ...
            try:
                targets = to_list(
                    parse_multi_dut_args(count, "|".join(self.target))
                )  # check later while collecting
        ...
        return PytestCase(
            apps=[
                PytestApp(app_paths[i], targets[i], configs[i]) for i in range(count)
            ],
            item=item,
    ...
    def pytest_collectstart(self) -> None:
        ...
        if (
            not os.getenv("CI_JOB_ID")
            or os.getenv("PYTEST_IGNORE_COLLECT_IMPORT_ERROR") == "1"
        ):
            ...
            for p in [
                ...
                try:
                    ...
                    logging.warning(
                        f"Optional package {p} is not installed, mocking it while collecting..."
                    )
    ...
    def pytest_collection_modifyitems(self, items: t.List[Function]) -> None:
        ...
        if os.getenv("INCLUDE_NIGHTLY_RUN") == "1":
            ...
            items[:] = [
                _item
                for _item in items
                if _item.get_closest_marker("nightly_run") is not None
            ]
        else:
            ...
            items[:] = [
                _item
                for _item in items
                if _item.get_closest_marker("nightly_run") is None
            ]
        ...
        for item in items:
            ...
            if (
                case.is_single_dut_test_case
                and _item_target_param
                and _item_target_param not in case.all_markers
            ):
        ...
        if self.collect_mode == CollectMode.SINGLE_SPECIFIC:
            ...
            for item in items:
                ...
                if (
                    case.is_single_dut_test_case
                    and self.target[0] in case.target_markers
                ):
                    filtered_items.append(item)

                # multi-dut ones and in single_target_duplicate_mode
                elif (
                    self._single_target_duplicate_mode
                    and not case.is_single_dut_test_case
                ):
                    # ignore those test cases with `target` defined in parametrize, since these will be covered in 3.3
                    if (
                        self.get_param(item, "target", None) is None
                        and self.target[0] in case.target_markers
                    ):
                        filtered_items.append(item)
        ...
        # 3.2. CollectMode.MULTI_SPECIFIC, like `pytest --target esp32,esp32`
            ...
            items[:] = [
                _item
                for _item in items
                if item_to_case_dict[_item].targets == self.target
            ]
        ...
        if self.config_name:
            ...
            for item in items:
                case = item_to_case_dict[item]
                if self.config_name not in set(
                    app.config or DEFAULT_SDKCONFIG for app in case.apps
                ):
                    self.additional_info[case.name]["skip_reason"] = (
                        f"Only run with sdkconfig {self.config_name}"
                    )
                else:
                    _items.append(item)
            items[:] = _items
        ...
        items[:] = sorted(
            _items,
            key=lambda x: (
                os.path.dirname(x.path),
                self.get_param(x, "config", DEFAULT_SDKCONFIG),
            ),
        )
        for item in items:
            case = item_to_case_dict[item]
            ...
            if (
                "esp32c2" in self.target
                and "esp32c2" in case.targets
                and "xtal_26mhz" not in case.all_markers
            ):
    ...
    def pytest_runtest_teardown(self, item: Function) -> None:
        ...
        for junit in junits:
            ...
            for case in testcases:
                ...
                new_case_name = format_case_id(
                    target, config, case.attrib["name"], is_qemu=is_qemu
                )
