...
def merge_junit_files(junit_files: t.List[str], target_path: str) -> None:
            ...
            for element_name in ["failure", "error"]:
                for element in testcase.findall(element_name):
                    ...
                    if existing_element is None:
                        ...
                        existing_element.attrib["message"] += ". " + element.get(
                            "message", ""
                        )  # type: ignore
    ...
    merged_testsuite.set(
        "failures", str(len(merged_testsuite.findall(".//testcase/failure")))
    )
    merged_testsuite.set(
        "errors", str(len(merged_testsuite.findall(".//testcase/error")))
    )
    merged_testsuite.set(
        "skipped", str(len(merged_testsuite.findall(".//testcase/skipped")))
    )
