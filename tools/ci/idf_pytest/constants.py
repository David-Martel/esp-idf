...
DEFAULT_IGNORE_WARNING_FILEPATH = os.path.join(
    IDF_PATH, "tools", "ci", "ignore_build_warnings.txt"
)
DEFAULT_BUILD_TEST_RULES_FILEPATH = os.path.join(
    IDF_PATH, ".gitlab", "ci", "default-build-test-rules.yml"
)
...
class PytestCase:
    ...
    def target_markers(self) -> t.Set[str]:
        def _get_temp_markers_disabled_targets(marker_name: str) -> t.Set[str]:
            ...
            if not temp_marker.kwargs.get("targets") or not temp_marker.kwargs.get(
                "reason"
            ):
                raise ValueError(
        ...
        return {
            marker for marker in self.all_markers if marker in TARGET_MARKERS
        } - skip_targets
    ...
    def all_built_in_app_lists(
        self, app_lists: t.Optional[t.List[str]] = None
    ) -> t.Optional[str]:
        """
