...
def get_mr_changed_files(source_branch: str) -> t.List[str]:
    ...
    git_output = subprocess.check_output(
        [
            ...
            "--name-only",
            "--diff-filter=d",
            f"origin/{mr.target_branch}...origin/{source_branch}",
        ]
    ).decode("utf8")
...
def get_modified_component(filepath: str) -> t.Optional[str]:
    ...
    for part in f_path.parts[1:]:
        if component_parent_dirs[-1] == "components" or component_parent_dirs[
            -1
        ].endswith("common_components"):
            if part not in _COMPONENT_NAME_DIR_RECORDS:
                logging.debug(
                    'Found component "%s" in path "%s"' % (part, component_parent_dirs)
                )
                ...
                logging.debug(
                    ...
                    % (
                        ...
                        _COMPONENT_NAME_DIR_RECORDS.get(part),
                    )
                )
...
def get_mr_components(
    source_branch: t.Optional[str] = None,
    ...
    if modified_files is None:
        if not source_branch:
            raise RuntimeError(
                "--src-branch is required if --modified-files is not provided"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Get the latest merge request info by pipeline"
    )
