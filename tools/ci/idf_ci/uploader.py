...
from idf_pytest.constants import DEFAULT_BUILD_LOG_FILENAME, DEFAULT_SIZE_JSON_FILENAME
...
class AppUploader(AppDownloader):
    ...
    def get_app_object_name(
        self, app_path: str, zip_name: str, artifact_type: ArtifactType
    ) -> str:
    ...
    def _upload_app(self, app_build_path: str, artifact_type: ArtifactType) -> bool:
        ...
        with ZipFile(
            ...
            for pattern in self.TYPE_PATTERNS_DICT[artifact_type]:
                for file in glob.glob(
                    os.path.join(app_build_path, pattern), recursive=True
                ):
                    zw.write(file)
                    has_file = True
        ...
        try:
            if has_file:
                obj_name = self.get_app_object_name(
                    app_path, zip_filename, artifact_type
                )
                self._client.fput_object(
                    getenv("IDF_S3_BUCKET"), obj_name, zip_filename
                )
                uploaded = True
        finally:
    ...
    def upload_app(
        ...
        print(
            f"Uploading {app_build_path} {[k.value for k in upload_types]} to minio server"
        )
    ...
    def _download_app(self, app_build_path: str, artifact_type: ArtifactType) -> None:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "Shouldn't happen, please report this bug in the CI channel"
                    + str(e)
                )
    ...
    def get_app_presigned_url(self, app: App, artifact_type: ArtifactType) -> str:
        obj_name = self.get_app_object_name(
            app.app_dir, f"{app.build_dir}.zip", artifact_type
        )
        try:
