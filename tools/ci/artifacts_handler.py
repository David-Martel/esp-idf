...
def _download_files(
    ...
    *,
    ...
    for obj in client.list_objects(
        getenv("IDF_S3_BUCKET"), prefix=prefix, recursive=True
    ):
...
if __name__ == "__main__":
    ...
    common_args.add_argument(
        ...
        type=str,
        nargs="+",
        choices=[a.value for a in ArtifactType],
        help="Types of files to download",
    )

    action = parser.add_subparsers(dest="action", help="Download or Upload")
    download = action.add_parser(
        "download", help="Download files from S3", parents=[common_args]
    )
    upload = action.add_parser(
        "upload", help="Upload files to S3", parents=[common_args]
    )
