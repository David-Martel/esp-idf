...
from .models import GitlabJob, Job, TestCase
...
def fetch_failed_jobs(commit_id: str) -> t.List[GitlabJob]:
    ...
    if response.status_code != 200:
        print(
            f"Failed to fetch jobs data: {response.status_code} with error: {response.text}"
        )
        return []
    ...
    if response.status_code != 200:
        print(
            f"Failed to fetch jobs failure rate data: {response.status_code} with error: {response.text}"
        )
        return []
...
def fetch_failed_testcases_failure_ratio(
    failed_testcases: t.List[TestCase], branches_filter: dict
) -> t.List[TestCase]:
    """
    ...
    """
    req_json = {
        "testcase_names": list(set([testcase.name for testcase in failed_testcases])),
        **branches_filter,
    }
    response = requests.post(
    ...
    if response.status_code != 200:
        print(
            f"Failed to fetch testcases failure rate data: {response.status_code} with error: {response.text}"
        )
        return []

    failure_rate_data = response.json()
    failure_rates = {
        item["name"]: item for item in failure_rate_data.get("testcases", [])
    }

    for testcase in failed_testcases:
        testcase.latest_total_count = failure_rates.get(testcase.name, {}).get(
            "total_count", 0
        )
        testcase.latest_failed_count = failure_rates.get(testcase.name, {}).get(
            "failed_count", 0
        )
