...
from idf_ci.app import enrich_apps_with_metrics_info, import_apps_from_txt
...
from dynamic_pipelines.utils import (
    fetch_app_metrics,
    fetch_failed_jobs,
...
def setup_argument_parser() -> argparse.ArgumentParser:
    report_type_parser: argparse.ArgumentParser = argparse.ArgumentParser(
        add_help=False
    )
    report_type_parser.add_argument(
        "--report-type",
        choices=["build", "target_test", "job"],
        required=True,
...
def common_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--project-id", type=int, default=os.getenv("CI_PROJECT_ID"), help="Project ID"
    )
    parser.add_argument(
        "--mr-iid",
        type=int,
    ...
    parser.add_argument(
        "--job-id", type=int, default=os.getenv("CI_JOB_ID"), help="Job ID"
    )
    parser.add_argument(
        "--commit-id", default=os.getenv("CI_COMMIT_SHA"), help="MR commit ID"
    )


def conditional_arguments(
    report_type_args: argparse.Namespace, parser: argparse.ArgumentParser
) -> None:
    if report_type_args.report_type == "build":
        parser.add_argument(
            "--app-list-filepattern",
            default="list_job_*.txt",
            help="Pattern to match app list files",
        )
    elif report_type_args.report_type == "target_test":
        parser.add_argument(
            "--junit-report-filepattern",
            default="XUNIT_RESULT*.xml",
            help="Pattern to match JUnit report files",
        )
...
def generate_build_report(args: argparse.Namespace) -> None:
    apps: t.List[t.Any] = [
        app
        for file_name in glob.glob(args.app_list_filepattern)
        for app in import_apps_from_txt(file_name)
    ]
...
def generate_target_test_report(args: argparse.Namespace) -> None:
    test_cases: t.List[t.Any] = parse_testcases_from_filepattern(
        args.junit_report_filepattern
    )
...
def generate_jobs_report(args: argparse.Namespace) -> None:
    ...
    report_generator.post_report(
        print_retry_jobs_message=any(job.is_failed for job in jobs)
    )
