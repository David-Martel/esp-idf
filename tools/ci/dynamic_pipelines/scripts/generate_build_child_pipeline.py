...
from dynamic_pipelines.constants import (
    DEFAULT_APPS_BUILD_PER_JOB,
    DEFAULT_BUILD_CHILD_PIPELINE_FILEPATH,
    DEFAULT_BUILD_CHILD_PIPELINE_NAME,
    DEFAULT_TEST_PATHS,
    NON_TEST_RELATED_APPS_FILENAME,
    NON_TEST_RELATED_BUILD_JOB_NAME,
...
def main(arguments: argparse.Namespace) -> None:
    ...
    if arguments.filter_expr:
        ...
        print(
            f"Generate test related apps file {TEST_RELATED_APPS_FILENAME} with {len(test_related_apps)} apps"
        )
    ...
    dump_jobs_to_yaml(
        build_jobs,
        arguments.yaml_output,
        DEFAULT_BUILD_CHILD_PIPELINE_NAME,
        extra_include_yml,
    )
    print(
        f"Generate child pipeline yaml file {arguments.yaml_output} with {sum(j.parallel for j in build_jobs)} jobs"
    )
...
if __name__ == "__main__":
    ...
    if (
        ...
        if (
            args.modified_components is not None
            and not args.ignore_app_dependencies_components
        ):
            # setting default values
            args.ignore_app_dependencies_components = DEFAULT_FULL_BUILD_TEST_COMPONENTS

        if (
            args.modified_files is not None
            and not args.ignore_app_dependencies_filepatterns
        ):
            # setting default values
