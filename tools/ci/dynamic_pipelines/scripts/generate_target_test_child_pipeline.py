...
from dynamic_pipelines.constants import (
    ...
    DEFAULT_TARGET_TEST_CHILD_PIPELINE_FILEPATH,
    DEFAULT_TARGET_TEST_CHILD_PIPELINE_NAME,
    DEFAULT_TARGET_TEST_JOB_TEMPLATE_NAME,
    DEFAULT_TEST_PATHS,
    KNOWN_GENERATE_TEST_CHILD_PIPELINE_WARNINGS_FILEPATH,
    TIMEOUT_4H_TEMPLATE_NAME,
)
...
def get_target_test_jobs(
    ...
    for (target_selector, env_markers), cases in res.items():
        ...
        target_test_job.set_variable(
            "ENV_MARKERS", "'" + " and ".join(env_markers) + "'"
        )
        target_test_job.set_variable(
            "PYTEST_NODES", " ".join([f"'{case.item.nodeid}'" for case in cases])
        )
...
def generate_target_test_child_pipeline(
    ...
    target_test_jobs, extra_include_yml, no_env_marker_test_cases = (
        get_target_test_jobs(
            paths=paths,
            apps=apps,
    ...
    known_no_env_marker_test_cases = set(
        known_warnings_dict.get("no_env_marker_test_cases", [])
    )
    ...
    print(
        f"Generate child pipeline yaml file {output_filepath} with {sum(j.parallel for j in target_test_jobs)} jobs"
    )
