...
sys.path.insert(
    0, os.path.join(f"{os.environ.get('IDF_PATH')}", "tools", "ci", "python_packages")
)
...
from idf_ci.app import (
    enrich_apps_with_metrics_info,  # noqa: E402
    import_apps_from_txt,  # noqa: E402
)

from dynamic_pipelines.models import GitlabJob  # noqa: E402
from dynamic_pipelines.report import (
    BuildReportGenerator,
    JobReportGenerator,
    TargetTestReportGenerator,
)  # noqa: E402
from dynamic_pipelines.utils import (  # noqa: E402
    load_file,
...
class TestReportGeneration(unittest.TestCase):
    ...
    def setup_patches(self) -> None:
        ...
        self.failure_rate_patcher = patch(
            "dynamic_pipelines.report.fetch_failed_testcases_failure_ratio"
        )
        ...
        self.MockUploader.return_value.get_app_presigned_url.return_value = (
    ...
    def load_test_and_job_reports(self) -> None:
        self.expected_target_test_report_html = load_file(
            os.path.join(
                self.reports_sample_data_path, "expected_target_test_report.html"
            )
        )
    ...
    def create_report_generators(self) -> None:
        jobs_response_raw = load_file(
            os.path.join(self.reports_sample_data_path, "jobs_api_response.json")
        )
        failure_rate_jobs_response = load_file(
            os.path.join(
                self.reports_sample_data_path, "failure_rate_jobs_response.json"
            )
        )
        built_apps_size_info_response = json.loads(
            load_file(
                os.path.join(
                    self.reports_sample_data_path, "apps_size_info_api_response.json"
                )
            )
        )
        failure_rates = {
            item["name"]: item
            for item in json.loads(failure_rate_jobs_response).get("jobs", [])
        }
        jobs = [
            GitlabJob.from_json_data(job_json, failure_rates.get(job_json["name"], {}))
            for job_json in json.loads(jobs_response_raw)["jobs"]
        ]
        test_cases = parse_testcases_from_filepattern(
            os.path.join(self.reports_sample_data_path, "XUNIT_*.xml")
        )
        apps = enrich_apps_with_metrics_info(
            built_apps_size_info_response,
            import_apps_from_txt(os.path.join(self.reports_sample_data_path, "apps")),
        )
    ...
    def test_known_failure_cases(self) -> None:
        known_failure_cases = (
            self.target_test_report_generator.get_known_failure_cases()
        )
    ...
    def test_failed_jobs_in_job_report(self) -> None:
        failed_jobs = self.job_report_generator._filter_items(
            self.job_report_generator.jobs, lambda job: job.is_failed
        )
    ...
    def test_generate_top_n_apps_by_size_table(self) -> None:
        ...
        self.assertIn(
            "| App Dir | Build Dir | Size Diff (bytes) | Size Diff (%) |",
            top_apps_table,
        )
    ...
    def test_get_built_apps_report_parts(self) -> None:
        apps = [
            MagicMock(
                ...
                preserve=True,
                metrics={
                    "binary_size": MagicMock(difference=1000, difference_percentage=1.0)
                },
            ),
            MagicMock(
                app_dir="test_app_2",
                ...
                preserve=False,
                metrics={
                    "binary_size": MagicMock(difference=2000, difference_percentage=2.0)
                },
            ),
        ]
    ...
    def test_get_failed_apps_report_parts(self) -> None:
        failed_apps = [
            MagicMock(
                ...
                metrics={
                    "binary_size": MagicMock(
                        difference=None, difference_percentage=None
                    )
                },
            ),
            MagicMock(
    ...
    def test_get_skipped_apps_report_parts(self) -> None:
        ...
        skipped_apps_report_parts = (
            build_report_generator.get_skipped_apps_report_parts()
        )
