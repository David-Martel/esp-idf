...
class YmlLinter:
    ...
    def _lint_1_yml_parser(self) -> None:
        for k, v in self.yml_config.config.items():
            if (
                ...
                raise SystemExit(
                    f"Parser incorrect. Key {k} not in global keys, anchors, templates,  or jobs"
                )
        ...
        for job_name, d in self.yml_config.jobs.items():
            for k, v in d.get("artifacts", {}).items():
                ...
                if v == defaults_artifacts[k]:
                    self._errors.append(
                        f"job {job_name} key {k} has same value as default value {v}"
                    )
    ...
    def _lint_submodule_patterns(self) -> None:
        ...
        submodule_paths_in_patterns = sorted(
            self.yml_config.config.get(".patterns-submodule", [])
        )
        ...
        if submodule_paths != submodule_paths_in_patterns:
            ...
            if unused_patterns:
                for item in unused_patterns:
                    self._errors.append(
                        f"non-exist pattern {item}. Please remove {item} from .patterns-submodule"
                    )
            ...
            if undefined_patterns:
                for item in undefined_patterns:
                    self._errors.append(
                        f"undefined pattern {item}. Please add {item} to .patterns-submodule"
                    )

    def _lint_gitlab_yml_templates(self) -> None:
        unused_templates = (
            self.yml_config.templates.keys() - self.yml_config.used_templates
        )
        ...
        undefined_templates = (
            self.yml_config.used_templates - self.yml_config.templates.keys()
        )
        ...
        for job_name, d in self.yml_config.jobs.items():
            if "artifacts" in d:
                if "expire_in" not in d["artifacts"]:
                    self._errors.append(
                        f"job {job_name} missing `artifacts: expire_in`. (suggest to set to `1 week`)"
                    )

                if "when" not in d["artifacts"]:
                    self._errors.append(
                        f"job {job_name} missing `artifacts: when`. (suggest to set to `always`)"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root-yml-filepath",
        help="root yml file path",
        default=os.path.join(IDF_PATH, ".gitlab-ci.yml"),
    )
