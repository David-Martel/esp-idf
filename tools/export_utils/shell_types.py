...
import click
from console_output import debug, status_message, warn

from utils import conf, run_cmd


class Shell:
    def __init__(
        self, shell: str, deactivate_cmd: str, new_esp_idf_env: Dict[str, str]
    ):
        self.shell = shell
        ...
        try:
            self.tmp_dir_path = Path(gettempdir()) / (
                "esp_idf_activate_" + getpass.getuser()
            )
        except Exception as e:
            self.tmp_dir_path = Path(gettempdir()) / "esp_idf_activate"
            warn(
                f"Failed to get username with error: {e}. Using default temporary directory {self.tmp_dir_path}."
            )
        ...
        if not conf.ARGS.debug and os.path.exists(self.tmp_dir_path):
            ...
            for item in self.tmp_dir_path.iterdir():
                try:
                    ...
                    warn(
                        f"Failed to clean temp activation directory with file {item}: {e}"
                    )
    ...
    def spawn(self) -> None:
        ...
        run([self.shell], check=False, env=new_env)
...
class UnixShell(Shell):
    def __init__(
        ...
        with NamedTemporaryFile(
            dir=self.tmp_dir_path, delete=False, prefix="activate_"
        ) as fd:
            self.script_file_path = Path(fd.name)
        debug(f"Temporary script file path: {self.script_file_path}")

        self.new_esp_idf_env["IDF_TOOLS_INSTALL_CMD"] = os.path.join(
            conf.IDF_PATH, "install.sh"
        )
        self.new_esp_idf_env["IDF_TOOLS_EXPORT_CMD"] = os.path.join(
            conf.IDF_PATH, "export.sh"
        )
...
class BashShell(UnixShell):
    ...
    def spawn(self) -> None:
        ...
        new_env = os.environ.copy()
        new_env.update(self.expanded_env())
        run([self.shell, "--init-file", str(self.script_file_path)], check=False, env=new_env)
...
class FishShell(UnixShell):
    def __init__(
        ...
        super().__init__(shell, deactivate_cmd, new_esp_idf_env)
        self.new_esp_idf_env["IDF_TOOLS_INSTALL_CMD"] = os.path.join(
            conf.IDF_PATH, "install.fish"
        )
        self.new_esp_idf_env["IDF_TOOLS_EXPORT_CMD"] = os.path.join(
            conf.IDF_PATH, "export.fish"
        )
    ...
    def autocompletion(self) -> str:
        ...
        env["_IDF.PY_COMPLETE"] = (
            "fish_source" if self.click_ver() >= 8 else "source_fish"
        )
    ...
    def spawn(self) -> None:
        ...
        new_env = os.environ.copy()
        new_env.update(self.expanded_env())
        run([self.shell, f"--init-command=source {self.script_file_path}"], check=False, env=new_env)
...
class PowerShell(Shell):
    def __init__(
        ...
        with NamedTemporaryFile(
            dir=self.tmp_dir_path, delete=False, prefix="activate_", suffix=".ps1"
        ) as fd:
            self.script_file_path = Path(fd.name)
        debug(f"Temporary script file path: {self.script_file_path}")

        self.new_esp_idf_env["IDF_TOOLS_INSTALL_CMD"] = os.path.join(
            conf.IDF_PATH, "install.ps1"
        )
        self.new_esp_idf_env["IDF_TOOLS_EXPORT_CMD"] = os.path.join(
            conf.IDF_PATH, "export.ps1"
        )
    ...
    def spawn(self) -> None:
        ...
        run(cmd, check=False, env=new_env)
...
class WinCmd(Shell):
    def __init__(
        ...
        with NamedTemporaryFile(
            dir=self.tmp_dir_path, delete=False, prefix="activate_", suffix=".bat"
        ) as fd:
            self.script_file_path = Path(fd.name)
        debug(f"Temporary script file path: {self.script_file_path}")

        self.new_esp_idf_env["IDF_TOOLS_INSTALL_CMD"] = os.path.join(
            conf.IDF_PATH, "install.bat"
        )
        self.new_esp_idf_env["IDF_TOOLS_EXPORT_CMD"] = os.path.join(
            conf.IDF_PATH, "export.bat"
        )
        self.new_esp_idf_env["IDF_TOOLS_JSON_PATH"] = os.path.join(
            conf.IDF_PATH, "tools", "tools.json"
        )
