...
from shell_types import SHELL_CLASSES, SUPPORTED_SHELLS
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help=("List supported shells.")
    )
    parser.add_argument(
        "-e",
        "--export",
...
def check_python_version() -> str:
    ...
    python_version_checker = os.path.join(
        conf.IDF_PATH, "tools", "python_version_checker.py"
    )
...
def get_idf_env() -> Dict[str, str]:
    ...
    extra_paths = os.pathsep.join(
        [os.path.join(conf.IDF_PATH, path) for path in extra_paths_list]
    )
    cmd = [
        sys.executable,
        conf.IDF_TOOLS_PY,
    ...
    try:
        ...
        debug(
            "\n".join(
                [
                    "Output from `./tools/idf_tools.py export --format key-value`:",
                    f"{stdout}",
                ]
            )
        )
...
def detect_shell(args: Any) -> str:
    ...
    while True:
        ...
        debug(
            f"Parent: pid: {parent_pid}, cmdline: {parent_cmdline}, exe: {parent_exe}, name: {parent_name}"
        )
...
def main() -> None:
    ...
    if detected_shell not in SHELL_CLASSES:
        die(
            f'"{detected_shell}" shell is not among the supported options: "{SUPPORTED_SHELLS}"'
        )

    shell = SHELL_CLASSES[detected_shell](
        detected_shell, deactivate_cmd, new_esp_idf_env
    )
