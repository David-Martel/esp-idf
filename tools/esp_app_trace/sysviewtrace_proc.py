...
from espytrace import apptrace, sysview
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ESP32 SEGGER SystemView Trace Parsing Tool"
    )

    parser.add_argument(
        "trace_sources",
        help="Trace data sources. Format: [file://]/path/to/file.",
        nargs="+",
        type=str,
    )
    parser.add_argument(
        "--elf-file", "-b", help="Path to program ELF file.", type=str, default=""
    )
    parser.add_argument(
        "--tmo",
        "-w",
        help="Data wait timeout in sec. -1: infinite, 0: no wait",
        type=int,
        default=0,
    )
    parser.add_argument(
        "--dump-events", "-d", help="Dump all events.", action="store_true"
    )
    parser.add_argument(
        "--print-events",
        "-p",
    ...
    parser.add_argument(
        "--to-json", "-j", help="Print JSON.", action="store_true", default=False
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    logging.basicConfig(
        level=verbosity_levels[args.verbose], format="[%(levelname)s] %(message)s"
    )
    ...
    try:
        proc = sysview.SysViewMultiStreamTraceDataProcessor(
            traces=parsers,
            print_events=args.dump_events,
            keep_all_events=True if args.to_json else False,
        )
        if include_events["heap"]:
            proc.add_stream_processor(
                ...
                sysview.SysViewHeapTraceDataProcessor(
                    ...
                    args.elf_file,
                    root_proc=proc,
                    print_heap_events=args.print_events,
                ),
            )
        if include_events["log"]:
            proc.add_stream_processor(
                sysview.SysViewTraceDataParser.STREAMID_LOG,
                sysview.SysViewLogTraceDataProcessor(
                    root_proc=proc, print_log_events=args.print_events
                ),
            )
    except Exception as e:
    ...
    try:
        ...
        if args.to_json:
            print(
                json.dumps(
                    proc,
                    cls=sysview.SysViewTraceDataJsonEncoder,
                    indent=4,
                    separators=(",", ": "),
                    sort_keys=True,
                )
