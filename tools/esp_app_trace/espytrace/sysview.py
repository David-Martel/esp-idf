...
class SysViewPredefinedEvent(SysViewEvent):
    ...
    _predef_events_fmt = {
        ...
        SYSVIEW_EVTID_OVERFLOW: (
            "svOverflow",
            [SysViewEventParamSimple("drop_cnt", _decode_u32)],
        ),
        SYSVIEW_EVTID_ISR_ENTER: (
            "svIsrEnter",
            [SysViewEventParamSimple("irq_num", _decode_u32)],
        ),
        ...
        SYSVIEW_EVTID_SYSTIME_CYCLES: (
            "svSysTimeCycles",
            [SysViewEventParamSimple("cycles", _decode_u32)],
        ),
        SYSVIEW_EVTID_SYSTIME_US: (
            "svSysTimeUs",
            [SysViewEventParamSimple("time", _decode_u64)],
        ),
        ...
        SYSVIEW_EVTID_MARK_START: (
            "svUserStart",
            [SysViewEventParamSimple("user_id", _decode_u32)],
        ),
        ...
        SYSVIEW_EVTID_TIMER_ENTER: (
            "svTimerEnter",
            [SysViewEventParamSimple("tim_id", _decode_u32)],
        ),
        ...
        SYSVIEW_EVTID_NAME_RESOURCE: (
            "svNameResource",
            [
                SysViewEventParamSimple("res_id", _decode_u32),
                SysViewEventParamSimple("name", _decode_str),
            ],
        ),
        ...
        SYSVIEW_EVTID_NUMMODULES: (
            "svNumModules",
            [SysViewEventParamSimple("mod_cnt", _decode_u32)],
        ),
    }
...
class SysViewTraceDataParser(apptrace.TraceDataProcessor):
    ...
    def __init__(self, print_events=False, core_id=0):
        ...
        """
        apptrace.TraceDataProcessor.__init__(
            self, print_events=print_events, keep_all_events=True
        )
        self.sys_info = None
        self._last_ts = 0
        self.irqs_info = {}
...
class SysViewTraceDataExtEventParser(SysViewTraceDataParser):
    def __init__(self, events_num, print_events=False, core_id=0):
        ...
        core_id : int
            id of the core this parser object relates to.
        """
        SysViewTraceDataParser.__init__(
            self, core_id=core_id, print_events=print_events
        )
        self.events_off = 0
...
class SysViewMultiTraceDataParser(SysViewTraceDataParser):
    ...
    def read_extension_event(self, evt_id, core_id, reader):
        ...
        for stream_id in self.stream_parsers:
            evt = self.stream_parsers[stream_id].read_extension_event(
                evt_id, core_id, reader
            )
            if evt:
                return evt
        return SysViewTraceDataParser.read_extension_event(
            self, evt_id, core_id, reader
        )
...
class SysViewTraceDataProcessor(apptrace.TraceDataProcessor):
    ...
    def __init__(
        self, traces, root_proc=None, print_events=False, keep_all_events=False
    ):
        """
        Constructor.
    ...
    def _process_event(self, event):
        ...
        if event.core_id not in self.traces:
            if 0 in self.traces and self.traces[0].esp_ext:
                ...
                raise SysViewTraceParseError(
                    "Event for unknown core %d" % event.core_id
                )
        else:
        ...
        if event.id == SYSVIEW_EVTID_ISR_ENTER:
            if event.params["irq_num"].value not in trace.irqs_info:
                raise SysViewTraceParseError(
                    "Enter unknown ISR %d" % event.params["irq_num"].value
                )
            if len(self.ctx_stack[event.core_id]):
                ...
                self.prev_ctx[event.core_id] = SysViewEventContext(
                    None, False, "IDLE%d" % event.core_id
                )
            # put new ISR context on top of the stack (the last in the list)
            self.ctx_stack[event.core_id].append(
                SysViewEventContext(
                    event.params["irq_num"].value,
                    True,
                    trace.irqs_info[event.params["irq_num"].value],
                )
            )
        elif (
            event.id == SYSVIEW_EVTID_ISR_EXIT
            or event.id == SYSVIEW_EVTID_ISR_TO_SCHEDULER
        ):
            if len(self.ctx_stack[event.core_id]):
            ...
            self.ctx_stack[event.core_id] = [
                SysViewEventContext(
                    event.params["tid"].value,
                    False,
                    trace.tasks_info[event.params["tid"].value],
                )
            ]
            ...
            if event.params["tid"].value not in trace.tasks_info:
                raise SysViewTraceParseError(
                    "Stop ready unknown task 0x%x" % event.params["tid"].value
                )
            if len(self.ctx_stack[event.core_id]):
                if (
                    not self.ctx_stack[event.core_id][-1].irq
                    and event.params["tid"].value
                    == self.ctx_stack[event.core_id][-1].handle
                ):
                    # return to the previous context (the last in the list)
                    self.prev_ctx[event.core_id] = self.ctx_stack[event.core_id].pop()
            else:
...
class SysViewMultiStreamTraceDataProcessor(SysViewTraceDataProcessor):
    ...
    def __init__(self, traces, print_events=False, keep_all_events=False):
        ...
        """
        SysViewTraceDataProcessor.__init__(
            self, traces, print_events=print_events, keep_all_events=keep_all_events
        )
        self.stream_procs = {}
    ...
    def default(self, obj):
        ...
        if isinstance(obj, SysViewMultiStreamTraceDataProcessor):
            ...
            for e in obj.events:
                ...
                if isinstance(e, SysViewPredefinedEvent) or isinstance(
                    e, SysViewOSEvent
                ):
                    json_events.append(e)
            return {
                "version": self.JSON_TRACE_VER,
                "streams": json_event_ids,
                "events": json_events,
            }
        ...
        if (
            isinstance(obj, SysViewPredefinedEvent)
            and obj.id == SYSVIEW_EVTID_PRINT_FORMATTED
        ):
            return {
                "ctx_name": obj.ctx_name,
                "in_irq": obj.in_irq,
...
class SysViewHeapTraceDataParser(SysViewTraceDataExtEventParser):
    ...
    def __init__(self, print_events=False, core_id=0):
        ...
        SysViewTraceDataExtEventParser.__init__(
            self,
            events_num=len(SysViewHeapEvent.events_fmt.keys()),
            core_id=core_id,
...
class SysViewHeapTraceDataProcessor(
    SysViewTraceDataProcessor, apptrace.BaseHeapTraceDataProcessorImpl
):
    """
    ...
    def __init__(
        ...
        """
        SysViewTraceDataProcessor.__init__(
            self, traces, root_proc=root_proc, print_events=print_events
        )
        ...
        # self.no_ctx_events = []
        self.name = "heap"
        stream = self.root_proc.get_trace_stream(
            0, SysViewTraceDataParser.STREAMID_HEAP
        )
        self.event_ids = {"alloc": stream.events_off, "free": stream.events_off + 1}

    def event_supported(self, event):
        heap_stream = self.root_proc.get_trace_stream(
            event.core_id, SysViewTraceDataParser.STREAMID_HEAP
        )
    ...
    def handle_event(self, event):
        ...
        if (event.id - heap_stream.events_off) == 0:
            heap_event = apptrace.HeapTraceEvent(
                event, True, toolchain=self.toolchain, elf_path=self.elf_path
            )
        else:
            heap_event = apptrace.HeapTraceEvent(
                event, False, toolchain=self.toolchain, elf_path=self.elf_path
            )
...
class SysViewLogTraceDataProcessor(
    SysViewTraceDataProcessor, apptrace.BaseLogTraceDataProcessorImpl
):
    """
    SystemView trace data processor supporting heap events.
    """
    ...
    def __init__(
        self, traces=[], root_proc=None, print_events=False, print_log_events=False
    ):
        """
        Constructor.
        see SysViewTraceDataProcessor.__init__()
