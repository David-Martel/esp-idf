...
from nvs_parser import NVS_Entry, NVS_Page, NVS_Partition, nvs_const
...
def check_partition_size(nvs_partition: NVS_Partition, nvs_log: NVS_Logger) -> bool:
    ...
    if len(nvs_partition.raw_data) % 0x1000 != 0:
        nvs_log.info(
            nvs_log.yellow("NVS Partition size must be a multiple of 0x1000 (4kiB)!")
        )
        return False
...
def identify_entry_duplicates(
    entry: NVS_Entry, entry_dict: Dict[str, List[NVS_Entry]]
) -> Dict[str, List[NVS_Entry]]:
...
def check_page_entries(
    nvs_page: NVS_Page, nvs_log: NVS_Logger
) -> Dict[str, List[NVS_Entry]]:
...
def filter_namespaces_fake_duplicates(
    duplicate_entries_dict: Dict[str, List[NVS_Entry]],
) -> Dict[str, List[NVS_Entry]]:
        ...
        for entry in duplicate_entries:
            if entry.metadata["type"] in nvs_const.item_type.values():
                ...
                if len(entry_same_namespace_collisions) != 0:
                    entry_same_namespace_collisions_list.update(
                        entry_same_namespace_collisions
                    )
...
def filter_blob_related_duplicates(
    duplicate_entries_dict: Dict[str, List[NVS_Entry]],
) -> Dict[str, List[NVS_Entry]]:
    """Takes a dictionary of entries (as written) and returns a new dictionary with "fake" duplicates,
    ...
    for key, duplicate_entries in duplicate_entries_dict.items():
        ...
        for entry in duplicate_entries:
            if entry.metadata["type"] == "blob_index":
                ...
                if len(blob_index_chunk_index_collisions) != 0:
                    blob_index_chunk_index_collisions_list.update(
                ...
                for other_entry in seen_blob_data:
                    if (
                        ...
                        and entry.metadata["chunk_index"]
                        == other_entry.metadata["chunk_index"]
                    ):
                ...
                if len(blob_data_chunk_index_collisions) != 0:
                    blob_data_chunk_index_collisions_list.update(
                        blob_data_chunk_index_collisions
                    )
        ...
        for (
            entry
        ) in seen_another_type_data:  # If there are any duplicates of other types
            new_duplicate_entries.append(entry)
...
def filter_entry_duplicates(
    entries: Dict[str, List[NVS_Entry]],
) -> Dict[str, List[NVS_Entry]]:
...
def print_entry_duplicates(
    duplicate_entries_list: Dict[str, List[NVS_Entry]], nvs_log: NVS_Logger
) -> None:
    if len(duplicate_entries_list) > 0:
        nvs_log.info(nvs_log.red("Found duplicate entries:"))
        nvs_log.info(
            nvs_log.red("Entry\tKey\t\t\tType\t\tNamespace idx\tPage\tPage status")
        )
