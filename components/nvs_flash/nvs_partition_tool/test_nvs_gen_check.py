...
from nvs_logger import NVS_Logger, nvs_log
...
def get_entry_type_bin(entry_type_str: str) -> Optional[int]:
    ...
    entry_type_bin: Optional[int] = next(
        key for key, value in nvs_const.item_type.items() if value == entry_type_str
    )
...
def create_entry_data_bytearray(
    ...
    key_encoded = (key_bytearray + bytearray({0x00}) * (16 - len(key_bytearray)))[
        :16
    ...
    entry_data[4:8] = crc32(raw_without_crc, 0xFFFFFFFF).to_bytes(
...
def generate_nvs() -> Callable:
    def _execute_nvs_setup(
        nvs_setup_func: Callable, size: int = 0x4000, output: Optional[Path] = None
    ) -> NVS_Partition:
...
def setup_ok_variable_len(nvs_obj: NVS) -> None:
    ...
    nvs_partition_gen.write_entry(
        nvs_obj, "short_string_key", "data", "string", "Hello world!"
    )
    nvs_partition_gen.write_entry(
        nvs_obj,
        "blob_key",
    ...
    nvs_partition_gen.write_entry(
        nvs_obj, "lorem_string_key", "data", "string", LOREM_STRING * 2
    )
    nvs_partition_gen.write_entry(
        nvs_obj, "uniq_string_key", "data", "string", "I am unique!"
    )
    nvs_partition_gen.write_entry(
        nvs_obj,
        "multi_blob_key",
...
def setup_ok_mixed(nvs_obj: NVS) -> None:
    ...
    for i in range(20):
        nvs_partition_gen.write_entry(
            nvs_obj, f"test_{i}", "data", prim_types[i % len(prim_types)], str(i)
        )
    nvs_partition_gen.write_entry(
        nvs_obj,
        "blob_key",
...
def setup_bad_mixed_same_key_different_page(nvs_obj: NVS) -> None:
    ...
    nvs_partition_gen.write_entry(
        nvs_obj, "storage2", "namespace", "", ""
    )  # New namespace, ignored
    nvs_partition_gen.write_entry(
        nvs_obj, "lorem_string_key", "data", "string", "abc"
...
def setup_bad_same_key_variable_len(nvs_obj: NVS) -> None:
    ...
    nvs_partition_gen.write_entry(
        nvs_obj, "same_string_key", "data", "string", "world!"
    )
    nvs_partition_gen.write_entry(
        nvs_obj, "unique_string_key", "data", "string", "I am unique!"
    )
...
def prepare_duplicate_list(nvs: NVS_Partition) -> Dict[str, List[NVS_Entry]]:
    ...
    for page in nvs.pages:
        ...
        for entry in page.entries:
            ...
            seen_written_entires_all = nvs_check.identify_entry_duplicates(
                entry, seen_written_entires_all
            )
    # Duplicate entry check (2) - same key, different index
    duplicates: Dict[str, List[NVS_Entry]] = nvs_check.filter_entry_duplicates(
        seen_written_entires_all
    )
    return duplicates
...
@pytest.mark.parametrize(
    "setup_func", [setup_ok_primitive, setup_ok_variable_len, setup_ok_mixed]
)
...
def test_check_empty_page_content__check_page_crc(
    generate_nvs: Callable, setup_func: Callable
) -> None:
    nvs = generate_nvs(setup_func)
    for page in nvs.pages:
...
def test_check_duplicates_bad_same_key_primitive_type(
    generate_nvs: Callable, setup_func: Callable
) -> None:
    nvs = generate_nvs(setup_func)
    duplicates = prepare_duplicate_list(nvs)
...
def test_check_duplicates_bad_same_key_variable_len_type(
    generate_nvs: Callable, setup_func: Callable
) -> None:
    nvs = generate_nvs(setup_func)
    duplicates = prepare_duplicate_list(nvs)
...
def test_check_duplicates_bad_same_key_different_pages(
    generate_nvs: Callable, setup_func: Callable
) -> None:
    nvs = generate_nvs(setup_func)
    duplicates = prepare_duplicate_list(nvs)
    assert (
        len(duplicates) == 9
    )  # 9 duplicate keys in total (8 pairs of 2 duplicates + 1 triplet)
    for i, value in enumerate(list(duplicates.values())):
        if i < 8:
            assert (
                len(value) == 2
            )  # i in range 0-7 -- pairs of 2 entries with the same key
        else:
...
def test_check_duplicates_bad_same_key_blob_index(
    generate_nvs: Callable, setup_func: Callable
) -> None:
    nvs = generate_nvs(setup_func)
    duplicates = prepare_duplicate_list(nvs)
    ...
    assert (
        len(list(duplicates.values())[0]) == 6
    )  # 6 entries with the blob_key (2x blob_index, 4x blob_data)
    nvs_check.integrity_check(nvs, logger)
