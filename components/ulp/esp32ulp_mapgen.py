...
def gen_ld_h_from_sym(
    ...
    if "::" in prefix:
        ...
        var_prefix = "_".join(
            tmp[1:]
        )  # Limit to a single namespace here to avoid complex mangling rules
    ...
    for line in f_sym:
        ...
        var_dimension = (
            f"[{size // 4}]" if size > 4 else ""
        )  # Use // to automatically perform an integer division
        ...
        name_in_ld = (
            name_mangling(namespace + "::" + var_name) if cpp_mode else var_name
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-addr",
        required=True,
        help="base address of the ULP memory, to be added to each symbol",
    )
    parser.add_argument(
        "-p",
        "--prefix",
        required=False,
        help="prefix for generated header file",
        default="ulp_",
    )
    ...
    with (
        open(args.outputfile + ".h", "w") as f_h,
        open(args.outputfile + ".ld", "w") as f_ld,
    ):
