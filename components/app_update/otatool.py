...
class OtatoolTarget:
    ...
    def switch_ota_partition(self, ota_id):
        ...
        for i in range(gen.NUM_PARTITION_SUBTYPE_APP_OTA):
            ota_partition = filter(
                lambda p: p.subtype == (gen.MIN_PARTITION_SUBTYPE_APP_OTA + i),
                partition_table,
            )
        ...
        try:
            if isinstance(ota_id, int):
                ota_partition_next = filter(
                    lambda p: p.subtype - gen.MIN_PARTITION_SUBTYPE_APP_OTA == ota_id,
                    ota_partitions,
                )
            else:
        ...
        if is_otadata_info_valid(otadata_info[0]) and is_otadata_info_valid(
            otadata_info[1]
        ):
...
def _read_otadata(target):
    ...
    print(
        "Firmware:  0x{:08x} \t0x{:08x} | \t0x{:08x} \t 0x{:08x}".format(
            otadata_info[0].seq,
            otadata_info[0].crc,
            otadata_info[1].seq,
            otadata_info[1].crc,
...
def main():
    ...
    parser.add_argument(
        "--esptool-write-args",
        help="additional subcommand arguments for esptool write_flash",
        nargs="+",
    )
    parser.add_argument(
        "--esptool-read-args",
        help="additional subcommand arguments for esptool read_flash",
        nargs="+",
    )
    parser.add_argument(
        "--esptool-erase-args",
        help="additional subcommand arguments for esptool erase_region",
        nargs="+",
    )
    ...
    parser.add_argument(
        "--port",
        "-p",
        help="port where the device to read the partition table from is attached",
    )
    ...
    subparsers.add_parser(
        "read_otadata", help="read otadata partition", parents=[spi_flash_sec_size]
    )
    ...
    subparsers.add_parser(
        ...
        help="switch otadata partition",
        parents=[slot_or_name_parser, spi_flash_sec_size],
    )

    read_ota_partition_subparser = subparsers.add_parser(
        "read_ota_partition",
        help="read contents of an ota partition",
        parents=[slot_or_name_parser],
    )
    read_ota_partition_subparser.add_argument(
        "--output",
        help="file to write the contents of the ota partition to",
        required=True,
    )

    write_ota_partition_subparser = subparsers.add_parser(
        "write_ota_partition",
        help="write contents to an ota partition",
        parents=[slot_or_name_parser],
    )
    ...
    subparsers.add_parser(
        "erase_ota_partition",
        help="erase contents of an ota partition",
        parents=[slot_or_name_parser],
    )
