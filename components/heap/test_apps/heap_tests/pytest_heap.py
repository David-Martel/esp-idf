...
def test_heap_poisoning_qemu(dut: Dut) -> None:
    for case in dut.test_menu:
        if (
            "qemu-ignore" not in case.groups
            and not case.is_ignored
            and case.type == "normal"
        ):
...
def test_heap_misc_options(dut: Dut) -> None:
    dut.run_all_single_board_cases(
        name=["IRAM_8BIT capability test", "test allocation and free function hooks"]
    )
...
@pytest.mark.temp_skip_ci(
    targets=["esp32c61"], reason="support TBD"
)  # TODO [ESP32C61] IDF-9858 IDF-10989
