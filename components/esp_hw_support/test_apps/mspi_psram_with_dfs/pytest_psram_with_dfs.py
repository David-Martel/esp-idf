...
MSPI_F8R8_configs = [
    ...
    for p in pathlib.Path(os.path.dirname(__file__)).glob("sdkconfig.ci.f8r8*")
]
...
MSPI_F4R8_configs = [
    p.name.replace("sdkconfig.ci.", "")
    for p in pathlib.Path(os.path.dirname(__file__)).glob("sdkconfig.ci.f4r8*")
]
...
MSPI_F4R4_configs = [
    p.name.replace("sdkconfig.ci.", "")
    for p in pathlib.Path(os.path.dirname(__file__)).glob("sdkconfig.ci.f4r4*")
]
