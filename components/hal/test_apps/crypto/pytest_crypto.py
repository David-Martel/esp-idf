...
def test_xts_aes_encryption(
    negotiated_key: bytes, plaintext_data: bytes, encrypted_data: bytes
) -> None:
    ...
    assert calculated_enc_data == encrypted_data, (
        "Calculated data does not match encrypted data obtained from firmware"
    )
...
def calculate_key_manager_ecdh0_negotiated_key(k2_G_hex: str, k1_ecdsa_key: str) -> Any:
    ...
    k1_bytes_big_endian = k1_int.to_bytes(
        (k1_int.bit_length() + 7) // 8, byteorder="big"
    )
...
def test_ecdsa_key(
    negotiated_key: bytes,
    digest: bytes,
    signature_r_le: bytes,
    signature_s_le: bytes,
    pubx: bytes,
    puby: bytes,
) -> None:
    ...
    assert calc_pubx == pubx_int, (
        "Public key calculated should match with public key obtained"
    )
    assert calc_puby == puby_int, (
        "Public key calculated should match with public key obtained"
    )
...
def test_crypto(dut: Dut) -> None:
    ...
    if dut.app.sdkconfig.get("SOC_KEY_MANAGER_SUPPORTED"):
        ...
        plaintext_data = dut.expect(
            r"Plaintext data: 0x([0-9a-fA-F]+)", timeout=timeout
        )[1]
        plaintext_data = binascii.unhexlify(plaintext_data)
        encrypted_data = dut.expect(
            r"Encrypted data: 0x([0-9a-fA-F]+)", timeout=timeout
        )[1].decode()
        encrypted_data = binascii.unhexlify(encrypted_data)
        negotiated_key = calculate_key_manager_ecdh0_negotiated_key(
            k2_G, "main/key_manager/k1_ecdsa.pem"
        )
        ...
        encrypted_data = binascii.unhexlify(encrypted_data)
        negotiated_key_0 = calculate_key_manager_ecdh0_negotiated_key(
            k2_G_0, "main/key_manager/k1_ecdsa.pem"
        )
        negotiated_key_1 = calculate_key_manager_ecdh0_negotiated_key(
            k2_G_1, "main/key_manager/k1_ecdsa.pem"
        )
        ...
        k2_G = dut.expect(r"K2_G: 0x([0-9a-fA-F]+)", timeout=timeout)[1].decode()
        digest = dut.expect(
            r"ECDSA message sha256 digest: 0x([0-9a-fA-F]+)", timeout=timeout
        )[1].decode()
        digest = binascii.unhexlify(digest)
        signature_r_le = dut.expect(
            r"ECDSA signature r_le: 0x([0-9a-fA-F]+)", timeout=timeout
        )[1].decode()
        signature_r_le = binascii.unhexlify(signature_r_le)
        signature_s_le = dut.expect(
            r"ECDSA signature s_le: 0x([0-9a-fA-F]+)", timeout=timeout
        )[1].decode()
        signature_s_le = binascii.unhexlify(signature_s_le)
        pub_x = dut.expect(r"ECDSA key pubx: 0x([0-9a-fA-F]+)", timeout=timeout)[
            1
        ].decode()
        pub_x = binascii.unhexlify(pub_x)
        pub_y = dut.expect(r"ECDSA key puby: 0x([0-9a-fA-F]+)", timeout=timeout)[
            1
        ].decode()
        ...
        test_ecdsa_key(
            negotiated_key, digest, signature_r_le, signature_s_le, pub_x, pub_y
        )

    test_numbers = dut.expect(
        r"(\d+) Tests (\d+) Failures (\d+) Ignored", timeout=timeout
    )
