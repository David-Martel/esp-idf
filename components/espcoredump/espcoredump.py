...
try:
    ...
    raise ModuleNotFoundError(
        'No module named "esp_coredump" please install esp_coredump by running '
...
def get_prefix_map_gdbinit_path(prog_path):  # type: (str) -> Any
    ...
    if not os.path.isfile(desc_path):
        logging.warning(
            '%s does not exist. Please build the app with "idf.py build"', desc_path
        )
        return ""
...
def main():  # type: () -> None
    ...
    try:
        if args.operation == "info_corefile":
            ...
            raise ValueError(
                "Please specify action, should be info_corefile or dbg_corefile"
            )
    finally:
        if temp_core_files:
