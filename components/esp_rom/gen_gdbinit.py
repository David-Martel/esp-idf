...
def generate_gdbinit_rom_add_symbols(target: str) -> str:
    ...
    for i, k in enumerate(roms[target], 1):
        ...
        r.append(
            indent(
                f'# if $_streq((char *) {hex(build_date_addr)}, "{k["build_date_str"]}")',
                indent_str,
            )
        )
        r.append(
            indent(
                get_rom_if_condition_str(build_date_addr, k["build_date_str"]),
                indent_str,
            )
        )
        r.append(
            indent(f"add-symbol-file {rom_elfs_dir}{rom_file}", indent_str + base_ident)
        )
