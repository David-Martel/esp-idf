...
def parameter_expand(
    existing_parameters: List[List[str]], value_list: List[str]
) -> List[List[str]]:
    ret = []
...
esp32_param_default = [
    pytest.param(*param)
    for param in parameter_expand(esp32_32_param, ["default|default"])
]
c6_param_default = [
    pytest.param(*param)
    for param in parameter_expand(esp32_c6_param, ["default|default"])
]

c6_param_retention = [
    pytest.param(*param)
    for param in parameter_expand(esp32_c6_param, ["default|sleep_retention"])
]
...
def test_sdio_speed_frhost_flow(
    dut: Tuple[IdfDut, IdfDut], expected_4b_speed: int, expected_1b_speed: int
) -> None:
    dut[1].expect("Press ENTER to see the list of tests")
...
# To host speed tests
def test_sdio_speed_tohost_flow(
    dut: Tuple[IdfDut, IdfDut], expected_4b_speed: int, expected_1b_speed: int
) -> None:
    dut[1].expect("Press ENTER to see the list of tests")
