...
class FatFSGen(unittest.TestCase):
    ...
    def test_empty_file_sn_fat12(self) -> None:
        ...
        self.assertEqual(
            file_system[0x3000:0x300C], b"TESTFILE   \x20"
        )  # check entry name and type
        self.assertEqual(
            file_system[0x1000:0x1006], b"\xf8\xff\xff\xff\x0f\x00"
        )  # check fat
    ...
    def test_empty_file_sn_fat12_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0x2000:0x200C], b"TESTFILE   \x20"
        )  # check entry name and type
        self.assertEqual(
            file_system[0x1000:0x1006], b"\xf8\xff\xff\xff\x0f\x00"
        )  # check fat
    ...
    def test_directory_sn_fat12(self) -> None:
        ...
        self.assertEqual(
            file_system[0x3000:0x300C], b"TESTFOLD   \x10"
        )  # check entry name and type
        self.assertEqual(
            file_system[0x1000:0x1006], b"\xf8\xff\xff\xff\x0f\x00"
        )  # check fat
        self.assertEqual(
            file_system[0x7000:0x700C], b".          \x10"
        )  # reference to itself
        self.assertEqual(
            file_system[0x7020:0x702C], b"..         \x10"
        )  # reference to parent
    ...
    def test_directory_sn_fat12_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0x2000:0x200C], b"TESTFOLD   \x10"
        )  # check entry name and type
        self.assertEqual(
            file_system[0x1000:0x1006], b"\xf8\xff\xff\xff\x0f\x00"
        )  # check fat
        self.assertEqual(
            file_system[0x6000:0x600C], b".          \x10"
        )  # reference to itself
        self.assertEqual(
            file_system[0x6020:0x602C], b"..         \x10"
        )  # reference to parent
    ...
    def test_empty_file_with_extension_sn_fat12(self) -> None:
        ...
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
            file_system[0x3000:0x300C], b"TESTF   TXT\x20"
        )  # check entry name and type
        self.assertEqual(
            file_system[0x1000:0x1006], b"\xf8\xff\xff\xff\x0f\x00"
        )  # check fat
    ...
    def test_empty_file_with_extension_sn_fat12_one_fat(self) -> None:
        ...
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
            file_system[0x2000:0x200C], b"TESTF   TXT\x20"
        )  # check entry name and type
        self.assertEqual(
            file_system[0x1000:0x1006], b"\xf8\xff\xff\xff\x0f\x00"
        )  # check fat
    ...
    def test_write_to_file_with_extension_sn_fat12(self) -> None:
        ...
        self.assertEqual(
            file_system[0x3000:0x300C], b"WRITEF  TXT\x20"
        )  # check entry name and type
        self.assertEqual(
            file_system[0x301A:0x3020], b"\x02\x00\x0b\x00\x00\x00"
        )  # check size and cluster ref
        self.assertEqual(
            file_system[0x1000:0x1006], b"\xf8\xff\xff\xff\x0f\x00"
        )  # check fat
        self.assertEqual(
            file_system[0x7000:0x700F], b"testcontent\x00\x00\x00\x00"
        )  # check file content
    ...
    def test_write_to_file_with_extension_sn_fat12_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0x2000:0x200C], b"WRITEF  TXT\x20"
        )  # check entry name and type
        self.assertEqual(
            file_system[0x201A:0x2020], b"\x02\x00\x0b\x00\x00\x00"
        )  # check size and cluster ref
        self.assertEqual(
            file_system[0x1000:0x1006], b"\xf8\xff\xff\xff\x0f\x00"
        )  # check fat
        self.assertEqual(
            file_system[0x6000:0x600F], b"testcontent\x00\x00\x00\x00"
        )  # check file content
    ...
    def test_write_to_file_in_folder_sn_fat12(self) -> None:
        ...
        fatfs.write_content(
            path_from_root=["TESTFOLD", "WRITEF.TXT"], content=b"testcontent"
        )
        fatfs.write_filesystem(CFG["output_file"])
        file_system = read_filesystem(CFG["output_file"])
    ...
    def test_full_sector_file(self) -> None:
        ...
        fatfs.create_file("WRITEF", extension="TXT")
        fatfs.write_content(
            path_from_root=["WRITEF.TXT"], content=CFG["sector_size"] * b"a"
        )
        fatfs.write_filesystem(CFG["output_file"])
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
    ...
    def test_file_chaining(self) -> None:
        ...
        fatfs.create_file("WRITEF", extension="TXT")
        fatfs.write_content(
            path_from_root=["WRITEF.TXT"], content=CFG["sector_size"] * b"a" + b"a"
        )
        fatfs.write_filesystem(CFG["output_file"])
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
        ...
        self.assertEqual(
            file_system[0x8000:0x9000], b"a" + (CFG["sector_size"] - 1) * b"\x00"
        )
    ...
    def test_full_two_sectors_folder(self) -> None:
        ...
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
            file_system[0x106000:0x106010],
            b"later\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        )
        self.assertEqual(
            file_system[0x109000:0x109010],
    ...
    def test_e2e_deep_folder_into_image(self) -> None:
        ...
        self.assertEqual(
            file_system[0x7070:0x7080],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x05\x00\x0b\x00\x00\x00",
        )
    ...
    def test_e2e_deep_folder_into_image_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0x9000:0x9010], b"thisistest\n\x00\x00\x00\x00\x00"
        )
        self.assertEqual(
            file_system[0xA000:0xA010],
            b"ahoj\n\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
    ...
    def test_e2e_deep_folder_into_image_ext(self) -> None:
        ...
        self.assertEqual(
            file_system[0xC000:0xC009], b"\xff\xff\xff\xff\xff\xff\xff\xff\xff"
        )
    ...
    def test_e2e_deep_folder_into_image_ext_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0xB000:0xB009], b"\xff\xff\xff\xff\xff\xff\xff\xff\xff"
        )
    ...
    def test_chaining_fat16(self) -> None:
        ...
        self.assertEqual(
            file_system[0xC000:0xD000], b"a" + (CFG["sector_size"] - 1) * b"\x00"
        )
    ...
    def test_chaining_fat16_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0x9000:0xA000], b"a" + (CFG["sector_size"] - 1) * b"\x00"
        )
    ...
    def test_empty_lfn_short_name(self) -> None:
        ...
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
            file_system[0x3000:0x3019],
            b"HELLO   TXT \x18\x00\x00\x00!\x00!\x00\x00\x00\x00\x00!",
        )
    ...
    def test_lfn_short_name(self) -> None:
        ...
        self.assertEqual(
            file_system[0x3010:0x3020],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x02\x00\x0e\x00\x00\x00",
        )
    ...
    def test_lfn_empty_name(self) -> None:
        ...
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
            file_system[0x3000:0x3010], b"Bl\x00o\x00.\x00t\x00x\x00\x0f\x00\xadt\x00"
        )
        self.assertEqual(
            file_system[0x3012:0x3020],
            b"\xff\xff\xff\xff\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff",
        )
        self.assertEqual(
            file_system[0x3020:0x3030],
            b"\x01h\x00e\x00l\x00l\x00o\x00\x0f\x00\xadh\x00",
        )
        self.assertEqual(
            file_system[0x3030:0x3040], b"e\x00l\x00l\x00o\x00h\x00\x00\x00e\x00l\x00"
        )
        self.assertEqual(file_system[0x3040:0x3050], b"HELLOH~\x01TXT \x00\x00\x00\x00")
        self.assertEqual(
            file_system[0x3050:0x3060],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x02\x00\x00\x00\x00\x00",
        )
    ...
    def test_lfn_empty_name_one_fat(self) -> None:
        ...
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
            file_system[0x2000:0x2010], b"Bl\x00o\x00.\x00t\x00x\x00\x0f\x00\xadt\x00"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            file_system[0x2020:0x2030],
            b"\x01h\x00e\x00l\x00l\x00o\x00\x0f\x00\xadh\x00",
        )
        self.assertEqual(
            file_system[0x2030:0x2040], b"e\x00l\x00l\x00o\x00h\x00\x00\x00e\x00l\x00"
        )
    ...
    def test_lfn_plain_name(self) -> None:
        ...
        fatfs.create_file("HELLOHELLOHELLO", extension="TXT")
        fatfs.write_content(
            path_from_root=["HELLOHELLOHELLO.TXT"], content=b"this is a test"
        )
        fatfs.write_filesystem(CFG["output_file"])
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
    ...
    def test_lfn_plain_name_no_ext(self) -> None:
        ...
        fatfs.write_content(
            path_from_root=["HELLOHELLOHELLO"], content=b"this is a test"
        )
        fatfs.write_filesystem(CFG["output_file"])
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
            file_system[0x3000:0x3010],
            b"Bl\x00o\x00\x00\x00\xff\xff\xff\xff\x0f\x00P\xff\xff",
        )
        self.assertEqual(
            file_system[0x3012:0x3020],
            b"\xff\xff\xff\xff\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff",
        ...
        self.assertEqual(
            file_system[0x3020:0x3030], b"\x01h\x00e\x00l\x00l\x00o\x00\x0f\x00Ph\x00"
        )
        self.assertEqual(
            file_system[0x3030:0x3040], b"e\x00l\x00l\x00o\x00h\x00\x00\x00e\x00l\x00"
        )
    ...
    def test_lfn_plain_name_no_ext_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0x2020:0x2030], b"\x01h\x00e\x00l\x00l\x00o\x00\x0f\x00Ph\x00"
        )
        self.assertEqual(
            file_system[0x2030:0x2040], b"e\x00l\x00l\x00o\x00h\x00\x00\x00e\x00l\x00"
        )
    ...
    def test_lfn_short_entry_exception(self) -> None:
        fatfs = fatfsgen.FATFS(fat_tables_cnt=2, long_names_enabled=True)
        self.assertRaises(
            LowerCaseException, fatfs.create_file, "hello", extension="txt"
        )
    ...
    def test_lfn_nested_empty(self) -> None:
        ...
        self.assertEqual(
            file_system[0x3000:0x3010],
            b"Bo\x00l\x00d\x00\x00\x00\xff\xff\x0f\x00\xa0\xff\xff",
        )
        self.assertEqual(
            file_system[0x3012:0x3020],
            b"\xff\xff\xff\xff\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff",
        ...
        self.assertEqual(
            file_system[0x3020:0x3030],
            b"\x01v\x00e\x00r\x00y\x00l\x00\x0f\x00\xa0o\x00",
        )
        self.assertEqual(
            file_system[0x3030:0x3040], b"n\x00g\x00t\x00e\x00s\x00\x00\x00t\x00f\x00"
        )
        self.assertEqual(
            file_system[0x3040:0x3050], b"VERYLO~\x01   \x10\x00\x00\x00\x00"
        )
        self.assertEqual(
            file_system[0x3050:0x3060],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x02\x00\x00\x00\x00\x00",
        ...
        self.assertEqual(file_system[0x7000:0x7010], b".          \x10\x00\x00\x00\x00")
        self.assertEqual(
            file_system[0x7012:0x7020],
            b"!\x00\x00\x00\x00\x00!\x00\x02\x00\x00\x00\x00\x00",
        )
        self.assertEqual(file_system[0x7020:0x7030], b"..         \x10\x00\x00\x00\x00")
        self.assertEqual(
            file_system[0x7030:0x7040],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x01\x00\x00\x00\x00\x00",
        )
        self.assertEqual(file_system[0x7040:0x7050], b"HELLO   TXT \x18\x00\x00\x00")
        self.assertEqual(
            file_system[0x7050:0x7060],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x03\x00\x00\x00\x00\x00",
        )
    ...
    def test_lfn_nested_empty_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0x2030:0x2040], b"n\x00g\x00t\x00e\x00s\x00\x00\x00t\x00f\x00"
        )
        self.assertEqual(
            file_system[0x2040:0x2050], b"VERYLO~\x01   \x10\x00\x00\x00\x00"
        )
        self.assertEqual(
            file_system[0x2050:0x2060],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x02\x00\x00\x00\x00\x00",
    ...
    def test_lfn_nested_long_empty(self) -> None:
        fatfs: fatfsgen.fatfs = fatfsgen.FATFS(
            fat_tables_cnt=2, long_names_enabled=True
        )
        fatfs.create_directory("verylongtestfold")
        fatfs.create_file(
            "hellohellohello", extension="txt", path_from_root=["verylongtestfold"]
        )
        fatfs.write_filesystem(CFG["output_file"])
        ...
        self.assertEqual(
            file_system[0x3000:0x3010],
            b"Bo\x00l\x00d\x00\x00\x00\xff\xff\x0f\x00\n\xff\xff",
        )
        self.assertEqual(
            file_system[0x3012:0x3020],
            b"\xff\xff\xff\xff\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff",
        ...
        self.assertEqual(
            file_system[0x3020:0x3030], b"\x01v\x00e\x00r\x00y\x00l\x00\x0f\x00\no\x00"
        )
        self.assertEqual(
            file_system[0x3030:0x3040], b"n\x00g\x00t\x00e\x00s\x00\x00\x00t\x00f\x00"
        )
        ...
        self.assertEqual(
            file_system[0x7040:0x7050], b"Bl\x00o\x00.\x00t\x00x\x00\x0f\x00\xcft\x00"
        )
        self.assertEqual(
            file_system[0x7050:0x7060],
    ...
    def test_lfn_nested_long_empty_one_fat(self) -> None:
        fatfs: fatfsgen.FATFS = fatfsgen.FATFS(
            fat_tables_cnt=1, long_names_enabled=True
        )
        fatfs.create_directory("verylongtestfold")
        fatfs.create_file(
            "hellohellohello", extension="txt", path_from_root=["verylongtestfold"]
        ...
        self.assertEqual(
            file_system[0x2020:0x2030], b"\x01v\x00e\x00r\x00y\x00l\x00\x0f\x00\no\x00"
        )
        self.assertEqual(
            file_system[0x2030:0x2040], b"n\x00g\x00t\x00e\x00s\x00\x00\x00t\x00f\x00"
        )
        ...
        self.assertEqual(
            file_system[0x6040:0x6050], b"Bl\x00o\x00.\x00t\x00x\x00\x0f\x00\xcft\x00"
        )
        self.assertEqual(
            file_system[0x6050:0x6060],
            b"\x00\x00\xff\xff\xff\xff\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff",
    ...
    def test_lfn_nested_text(self) -> None:
        ...
        fatfs.create_file("HELLO", extension="TXT", path_from_root=["VERYLONGTESTFOLD"])
        fatfs.write_content(
            path_from_root=["VERYLONGTESTFOLD", "HELLO.TXT"], content=b"this is a test"
        )
        fatfs.write_filesystem(CFG["output_file"])
        file_system = read_filesystem(CFG["output_file"])
        self.assertEqual(
        ...
        self.assertEqual(
            file_system[0x7050:0x7060],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x03\x00\x0e\x00\x00\x00",
        )
    ...
    def test_boundary_clusters12(self) -> None:
        output: bytes = check_output(
            ["python", "../fatfsgen.py", "--partition_size", "16732160", "test_dir"],
            stderr=STDOUT,
        )
        self.assertEqual(
            output,
    ...
    def test_boundary_clusters16(self) -> None:
        output: bytes = check_output(
            ["python", "../fatfsgen.py", "--partition_size", "268390400", "test_dir"],
            stderr=STDOUT,
        )
        self.assertEqual(
            output,
    ...
    def test_lfn_increasing(self) -> None:
        ...
        fatfs.create_directory("VERYLONGTESTFOLD")
        fatfs.create_file(
            "HELLOHELLOHELLOOOOOOO",
            extension="TXT",
            path_from_root=["VERYLONGTESTFOLD"],
        )
        fatfs.create_file(
            "HELLOHELLOHELLOOOOOOB",
            extension="TXT",
            path_from_root=["VERYLONGTESTFOLD"],
        )
        fatfs.write_content(
            path_from_root=["VERYLONGTESTFOLD", "HELLOHELLOHELLOOOOOOO.TXT"],
            content=b"this is a test A",
        )
        fatfs.write_content(
            path_from_root=["VERYLONGTESTFOLD", "HELLOHELLOHELLOOOOOOB.TXT"],
        ...
        self.assertEqual(
            file_system[0x3011:0x3020],
            b"\xff\xff\xff\xff\xff\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff",
        )
        self.assertEqual(
            file_system[0x3020:0x3030],
            b"\x01v\x00e\x00r\x00y\x00l\x00\x0f\x00\xa0o\x00",
        ...
        self.assertEqual(
            file_system[0x7040:0x7050], b"Bl\x00o\x00o\x00o\x00o\x00\x0f\x00\xado\x00"
        )
        self.assertEqual(
            file_system[0x7050:0x7060],
            b"o\x00o\x00.\x00t\x00x\x00\x00\x00t\x00\x00\x00",
        )
        ...
        self.assertEqual(
            file_system[0x7060:0x7070],
            b"\x01h\x00e\x00l\x00l\x00o\x00\x0f\x00\xadh\x00",
        )
        self.assertEqual(
            file_system[0x7070:0x7080], b"e\x00l\x00l\x00o\x00h\x00\x00\x00e\x00l\x00"
        )
        self.assertEqual(file_system[0x7080:0x7090], b"HELLOH~\x01TXT \x00\x00\x00\x00")
        self.assertEqual(
            file_system[0x7090:0x70A0],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x03\x00\x10\x00\x00\x00",
        )
        self.assertEqual(
            file_system[0x70A0:0x70B0], b"Bl\x00o\x00o\x00o\x00o\x00\x0f\x00\x8do\x00"
        )

        self.assertEqual(
            file_system[0x70B0:0x70C0],
            b"o\x00b\x00.\x00t\x00x\x00\x00\x00t\x00\x00\x00",
        )
        self.assertEqual(
            file_system[0x70C0:0x70D0],
            b"\x01h\x00e\x00l\x00l\x00o\x00\x0f\x00\x8dh\x00",
        )
        self.assertEqual(
            file_system[0x70D0:0x70E0], b"e\x00l\x00l\x00o\x00h\x00\x00\x00e\x00l\x00"
        )
    ...
    def test_lfn_increasing_one_fat(self) -> None:
        ...
        self.assertEqual(
            file_system[0x6040:0x6050], b"Bl\x00o\x00o\x00o\x00o\x00\x0f\x00\xado\x00"
        )
        self.assertEqual(
            file_system[0x6050:0x6060],
            b"o\x00o\x00.\x00t\x00x\x00\x00\x00t\x00\x00\x00",
        ...
        self.assertEqual(
            file_system[0x6060:0x6070],
            b"\x01h\x00e\x00l\x00l\x00o\x00\x0f\x00\xadh\x00",
        )
        self.assertEqual(
            file_system[0x6070:0x6080], b"e\x00l\x00l\x00o\x00h\x00\x00\x00e\x00l\x00"
        )
        self.assertEqual(file_system[0x6080:0x6090], b"HELLOH~\x01TXT \x00\x00\x00\x00")
        self.assertEqual(
            file_system[0x6090:0x60A0],
            b"!\x00!\x00\x00\x00\x00\x00!\x00\x03\x00\x10\x00\x00\x00",
        )
        self.assertEqual(
            file_system[0x60A0:0x60B0], b"Bl\x00o\x00o\x00o\x00o\x00\x0f\x00\x8do\x00"
        )

        self.assertEqual(
            file_system[0x60B0:0x60C0],
            b"o\x00b\x00.\x00t\x00x\x00\x00\x00t\x00\x00\x00",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            file_system[0x60D0:0x60E0], b"e\x00l\x00l\x00o\x00h\x00\x00\x00e\x00l\x00"
        )
    ...
    def test_bs_not_initialized(self) -> None:
        ...
        self.assertRaises(
            NotInitialized, lambda: BootSector().binary_image
        )  # encapsulate property to callable
        ...
        self.assertEqual(
            Entry.get_cluster_id(
                Entry.ENTRY_FORMAT_SHORT_NAME.parse(
                    bytearray(
                        b"AHOJ        \x18\x00\xb0[&U&U\x00\x00\xb0[&U\x02\x00\x08\x00\x00\x00"
                    )
                )
            ),
            2,
    ...
    def test_retrieve_file_chaining(self) -> None:
        ...
        fatfs.write_filesystem(CFG["output_file"])
        self.assertEqual(
            fatfs.fat.get_chained_content(1)[:15], b"WRITEF  TXT \x00\x00\x00"
        )
        self.assertEqual(fatfs.fat.get_chained_content(2)[:15], b"aaaaaaaaaaaaaaa")

    def test_lstrip(self) -> None:
        self.assertEqual(
            right_strip_string("\x20\x20\x20thisistest\x20\x20\x20"), "   thisistest"
        )
