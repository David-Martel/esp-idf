...
from test_utils import (
    compare_folders,
    fill_sector,
    generate_local_folder_structure,
    generate_test_dir_2,
)
...
class FatFSGen(unittest.TestCase):
    ...
    def test_gen_parse() -> None:
        run(
            ...
            check=False, stderr=STDOUT,
        )

        run([sys.executable, "../fatfsgen.py", "output_data/tst_str"], check=False, stderr=STDOUT)
        run([sys.executable, "../fatfsparse.py", "fatfs_image.img"], check=False, stderr=STDOUT)
    ...
    def test_full_two_sectors_folder() -> None:
        ...
        assert set(os.listdir("Espressif/TESTFOLD")) == {
            f"A{str(i).upper()}" for i in range(256)
        }
    ...
    def test_full_sector_folder_fat16() -> None:
        ...
        assert set(os.listdir("Espressif")) == {"TESTFOLD"}
        assert set(os.listdir("Espressif/TESTFOLD")) == {
            f"A{str(i).upper()}" for i in range(128)
        }
    ...
    def file_(x: str, content_: str = "hey this is a test") -> dict:
        return {"type": "file", "name": x, "content": content_}
    ...
    def test_e2e_deeper(self) -> None:
        ...
        struct_: dict = {
            "type": "folder",
            "name": "testf",
            "content": [self.file_("MY_NEW"), folder_],
        }
    ...
    def test_e2e_deeper_large(self) -> None:
        ...
        struct_: dict = {
            "type": "folder",
            "name": "testf",
            "content": [self.file_("MY_NEW"), folder_, folder2_, folder3_],
        }
        generate_local_folder_structure(struct_, path_=".")
        run(
            [
    ...
    def test_parse_long_name(self) -> None:
        self.assertEqual(
            Entry.parse_entry_long(
                b"\x01t\x00h\x00i\x00s\x00_\x00\x0f\x00\xfbi\x00s\x00_\x00l\x00o\x00n\x00\x00\x00g\x00_\x00",
                251,
            ),
            {
                "order": 1,
        ...
        self.assertEqual(
            Entry.parse_entry_long(
                b"\x01t\x00h\x00i\x00s\x00_\x00\x0f\x01\xfbi\x00s\x00_\x00l\x00o\x00n\x00\x00\x00g\x00_\x00",
                251,
            ),
            {},
        )
