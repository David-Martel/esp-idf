...
def get_obj_name(
    obj_: dict, directory_bytes_: bytes, entry_position_: int, lfn_checksum_: int
) -> str:
    ...
    for pos in range(
        entry_position_ - 1, -1, -1
    ):  # loop from the current entry back to the start
        obj_address_: int = FATDefaults.ENTRY_SIZE * pos
        entry_bytes_: bytes = directory_bytes_[
            obj_address_ : obj_address_ + FATDefaults.ENTRY_SIZE
        ]
        struct_ = Entry.parse_entry_long(entry_bytes_, lfn_checksum_)
        if len(struct_.items()) > 0:
            full_name[struct_["order"]] = build_file_name(
                struct_["name1"], struct_["name2"], struct_["name3"]
            )
            if struct_["is_last"]:
                break
    ...
    for i in range(entries_count_):
        ...
        if obj_["DIR_Attr"] == Entry.ATTR_ARCHIVE:
            ...
            if obj_["DIR_FileSize"] > 0:
                content_ = fat_.get_chained_content(
                    cluster_id_=Entry.get_cluster_id(obj_), size=obj_["DIR_FileSize"]
                )
            ...
            if obj_name_ in (".", ".."):
                continue
            child_directory_bytes_ = fat_.get_chained_content(
...
if __name__ == "__main__":
    ...
    argument_parser.add_argument(
        "input_image", help="Path to the image that will be parsed and extracted."
    )
    argument_parser.add_argument(
        "--long-name-support", action="store_true", help=argparse.SUPPRESS
    )

    # ensures backward compatibility
    argument_parser.add_argument(
        "--wear-leveling", action="store_true", help=argparse.SUPPRESS
    )
    argument_parser.add_argument(
        "--wl-layer",
    ...
    argument_parser.add_argument(
        "--verbose", action="store_true", help="Prints details about FAT image."
    )
    ...
    if args.wear_leveling and args.wl_layer:
        raise NotImplementedError(
            "Argument --wear-leveling cannot be combined with --wl-layer!"
        )
    ...
    full_ = fs[
        boot_dir_start_ : boot_dir_start_
        + boot_dir_sectors * boot_sector_.boot_sector_state.sector_size
    ]
    traverse_folder_tree(
        full_,
