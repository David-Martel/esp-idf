...
from fatfs_utils.utils import (
    FULL_BYTE,
    UINT32_MAX,
    FATDefaults,
    crc32,
    generate_4bytes_random,
...
def remove_wl(binary_image: bytes) -> bytes:
    ...
    wl_state_size: int = (
        WLFATFS.WL_STATE_HEADER_SIZE + WLFATFS.WL_STATE_RECORD_SIZE * total_sectors
    )
    wl_state_sectors_cnt: int = (
        wl_state_size + FATDefaults.WL_SECTOR_SIZE - 1
    ) // FATDefaults.WL_SECTOR_SIZE
    wl_state_total_size: int = wl_state_sectors_cnt * FATDefaults.WL_SECTOR_SIZE
    wl_sectors_size: int = (
        wl_state_sectors_cnt * FATDefaults.WL_SECTOR_SIZE * WLFATFS.WL_STATE_COPY_COUNT
        + FATDefaults.WL_SECTOR_SIZE
    )

    correct_wl_configuration = binary_image[-wl_sectors_size:]

    data_ = WLFATFS.WL_STATE_T_DATA.parse(
        correct_wl_configuration[: WLFATFS.WL_STATE_HEADER_SIZE]
    )

    total_records = 0
    # iterating over records field of the first copy of the state sector
    for i in range(
        WLFATFS.WL_STATE_HEADER_SIZE, wl_state_total_size, WLFATFS.WL_STATE_RECORD_SIZE
    ):
        if (
            correct_wl_configuration[i : i + WLFATFS.WL_STATE_RECORD_SIZE]
            != WLFATFS.WL_STATE_RECORD_SIZE * b"\xff"
        ):
            total_records += 1
        else:
            break
    before_dummy = binary_image[: total_records * FATDefaults.WL_SECTOR_SIZE]
    after_dummy = binary_image[
        total_records * FATDefaults.WL_SECTOR_SIZE + FATDefaults.WL_SECTOR_SIZE :
    ]
    new_image: bytes = before_dummy + after_dummy

    # remove wl sectors
    new_image = new_image[
        : len(new_image) - (FATDefaults.WL_SECTOR_SIZE + 2 * wl_state_total_size)
    ]
...
class WLFATFS:
    ...
    WL_CONFIG_T_DATA = Struct(
        ...
        "page_size" / Int32ul,
        "sector_size"
        / Int32ul,  # always 4096 for the types of NOR flash supported by ESP-IDF!
        "updaterate" / Int32ul,
        "wr_size" / Int32ul,
        "version" / Int32ul,
    ...
    def __init__(
        ...
        self.wl_state_sectors = (
            self.wl_state_size + FATDefaults.WL_SECTOR_SIZE - 1
        ) // FATDefaults.WL_SECTOR_SIZE

        self.boot_sector_start = (
            FATDefaults.WL_SECTOR_SIZE
        )  # shift by one "dummy" sector
        self.fat_table_start = (
            self.boot_sector_start + reserved_sectors_cnt * FATDefaults.WL_SECTOR_SIZE
        )
    ...
    def _add_dummy_sector(self) -> None:
        self.fatfs_binary_image = (
            FATDefaults.WL_SECTOR_SIZE * FULL_BYTE + self.fatfs_binary_image
        )
    ...
    def _add_config_sector(self) -> None:
        ...
        self.fatfs_binary_image += (
            wl_config
            + (FATDefaults.WL_SECTOR_SIZE - WLFATFS.WL_CONFIG_HEADER_SIZE) * FULL_BYTE
        )
    ...
    def _add_state_sectors(self) -> None:
        ...
        wl_state_sector_padding: bytes = (
            FATDefaults.WL_SECTOR_SIZE - WLFATFS.WL_STATE_HEADER_SIZE
        ) * FULL_BYTE
        wl_state_sector: bytes = (
            wl_state
            + wl_state_sector_padding
            + (self.wl_state_sectors - 1) * FATDefaults.WL_SECTOR_SIZE * FULL_BYTE
        )
