...
from fatfs_utils.utils import (
    ...
    FATFS_INCEPTION,
    FATFS_MIN_ALLOC_UNIT,
    RESERVED_CLUSTERS_COUNT,
    FATDefaults,
    get_args_for_partition_generator,
    get_fat_sectors_count,
    get_non_data_sectors_cnt,
    read_filesystem,
...
class FATFS:
    ...
    def __init__(
        ...
        if int(root_entry_count) > 128:
            assert (
                (int(root_entry_count) * BYTES_PER_DIRECTORY_ENTRY) // sector_size
            ) % 2 == 0

        root_dir_sectors_cnt: int = (
            int(root_entry_count) * BYTES_PER_DIRECTORY_ENTRY
        ) // sector_size
        ...
        binary_image: bytes = bytearray(
            read_filesystem(binary_image_path)
            if binary_image_path
            else self.create_empty_fatfs()
        )
        ...
        root_dir_size = (
            self.state.boot_sector_state.root_dir_sectors_cnt
            * self.state.boot_sector_state.sector_size
        )
    ...
    def duplicate_fat(self) -> None:
        ...
        if boot_sec_st.fat_tables_cnt == 2:
            ...
            fat_end = (
                fat_start + boot_sec_st.sectors_per_fat_cnt * boot_sec_st.sector_size
            )
            second_fat_shift = boot_sec_st.sectors_per_fat_cnt * boot_sec_st.sector_size
            self.state.binary_image[
                fat_start + second_fat_shift : fat_end + second_fat_shift
    ...
    def _generate_partition_from_folder(
        ...
        if os.path.isfile(real_path):
            ...
            dir_content = sorted(os.listdir(real_path))
            for path in dir_content:
                self._generate_partition_from_folder(
                    os.path.join(lower_path, path), folder_path=folder_path
                )
    ...
    def generate(self, input_directory: str) -> None:
        ...
        self._generate_partition_from_folder(
            folder_name, folder_path=path_to_folder, is_dir=True
        )
...
def calculate_min_space(
    ...
    for file in sorted(os.listdir(os.path.join(*path, fs_entity))):
        if long_file_names and True:
            ...
            dir_size += (
                get_required_lfn_entries_count(fs_entity) + 1
            ) * FATDefaults.ENTRY_SIZE
        else:
            dir_size += FATDefaults.ENTRY_SIZE
        buff += calculate_min_space(
            path + [fs_entity], file, sector_size, long_file_names, is_root=False
        )
...
def main() -> None:
    args = get_args_for_partition_generator(
        "Create a FAT filesystem and populate it with directory content", wl=False
    )

    if args.partition_size == -1:
        clusters = calculate_min_space(
            [],
        ...
        root_dir_sectors = (
            FATDefaults.ROOT_ENTRIES_COUNT * FATDefaults.ENTRY_SIZE
        ) // args.sector_size
        args.partition_size = max(
            ...
            (
                ...
                + fats
                + get_non_data_sectors_cnt(
                    RESERVED_CLUSTERS_COUNT, fats, args.fat_count, root_dir_sectors
                )
            )
            * args.sector_size,
        )
