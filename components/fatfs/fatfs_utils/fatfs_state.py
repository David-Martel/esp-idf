...
class FATFSState:
    ...
    def __init__(
        ...
        if (size // sector_size) * sectors_per_cluster in (
            FAT12_MAX_CLUSTERS,
            FAT16_MAX_CLUSTERS,
        ):
            print(
                "WARNING: It is not recommended to create FATFS with bounding "
...
class BootSectorState:
    ...
    def __init__(
        ...
        self.sectors_per_fat_cnt = get_fat_sectors_count(
    ...
    def clusters(self) -> int:
        ...
        """
        clusters_cnt_: int = (
            number_of_clusters(self.data_sectors, self.sectors_per_cluster)
            + RESERVED_CLUSTERS_COUNT
        )
    ...
    def entries_root_count(self) -> int:
        entries_root_count_: int = (
            self.root_dir_sectors_cnt * self.sector_size
