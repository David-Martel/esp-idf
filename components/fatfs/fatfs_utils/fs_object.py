...
from .long_filename_utils import (
    build_lfn_full_name,
    build_lfn_unique_entry_name_order,
    get_required_lfn_entries_count,
    split_name_to_lfn_entries,
    split_name_to_lfn_entry_blocks,
)
from .utils import (
    INVALID_SFN_CHARS_PATTERN,
    MAX_EXT_SIZE,
    MAX_NAME_SIZE,
    FATDefaults,
    build_lfn_short_entry_name,
    build_name,
    lfn_checksum,
    required_clusters_count,
    split_content_into_sectors,
    split_to_name_and_extension,
)
...
class File:
    ...
    def name_equals(self, name: str, extension: str) -> bool:
        equals_: bool = build_name(name, extension) == build_name(
    ...
    def write(self, content: bytes) -> None:
        ...
        for content_part in split_content_into_sectors(
            content, self.fatfs_state.boot_sector_state.sector_size
        ):
            ...
            self.fatfs_state.binary_image[address : address + len(content_part)] = (
                content_as_list
            )
...
class Directory:
    ...
    def init_directory(self) -> None:
        ...
        for dir_id, name_ in (
            ...
            (self.parent, self.PARENT_DIRECTORY),
        ):
    ...
    def lookup_entity(self, object_name: str, extension: str):  # type: ignore
        for entity in self.entities:
            if build_name(entity.name, entity.extension) == build_name(
    ...
    def allocate_long_name_object(
        ...
        lfn_unique_entry_order: int = build_lfn_unique_entry_name_order(
            target_dir.entities, name
        )
        lfn_short_entry_name: str = build_lfn_short_entry_name(
            name, extension, lfn_unique_entry_order
        )
        ...
        split_names_reversed = list(
            reversed(
                list(enumerate(split_name_to_lfn_entries(lfn_full_name, entries_count)))
            )
        )
    ...
    def _is_valid_sfn(name: str, extension: str) -> bool:
        if INVALID_SFN_CHARS_PATTERN.search(name) or INVALID_SFN_CHARS_PATTERN.search(
            name
        ):
            return False
    ...
    def allocate_object(
        ...
        target_dir: Directory = (
            self if not path_from_root else self.recursive_search(path_from_root, self)
        )
        free_entry: Entry = target_dir.find_free_entry() or target_dir.chain_directory()

        fatfs_date_ = (
            object_timestamp_.year,
            object_timestamp_.month,
            object_timestamp_.day,
        )
        fatfs_time_ = (
            object_timestamp_.hour,
            object_timestamp_.minute,
            object_timestamp_.second,
        )

        if not self.fatfs_state.long_names_enabled or self._is_valid_sfn(
            name, extension
        ):
            free_entry.allocate_entry(
                first_cluster_id=free_cluster_id,
    ...
    def write_to_file(self, path: List[str], content: bytes) -> None:
        ...
        if isinstance(entity_to_write, File):
            clusters_cnt: int = required_clusters_count(
                cluster_size=self.fatfs_state.boot_sector_state.sector_size,
                content=content,
            )
