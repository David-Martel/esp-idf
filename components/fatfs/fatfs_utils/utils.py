...
def get_non_data_sectors_cnt(
    ...
    return (
        ...
        + sectors_per_fat_cnt * fat_tables_cnt
...
def get_fat_sectors_count(clusters_count: int, sector_size: int) -> int:
    ...
    fat_size_bytes: int = (
        (clusters_count * 2 + cluster_s)
        if fatfs_type_ == FAT16
        else (clusters_count * 3 + 1) // 2 + cluster_s
    )
...
def build_lfn_short_entry_name(name: str, extension: str, order: int) -> str:
    return "{}{}".format(
        pad_string(
            content=name[: MAX_NAME_SIZE - 2] + "~" + chr(order), size=MAX_NAME_SIZE
        ),
...
def lfn_checksum(short_entry_name: str) -> int:
    ...
    for i in range(MAX_NAME_SIZE + MAX_EXT_SIZE):
        ...
        checksum_result = (
            (0x80 if checksum_result & 1 else 0x00)
            + (checksum_result >> 1)
            + ord(short_entry_name[i])
        )
        checksum_result &= 0xFF
    return checksum_result


def convert_to_utf16_and_pad(
    content: str, expected_size: int, pad: bytes = FULL_BYTE
) -> bytes:
...
def split_content_into_sectors(content: bytes, sector_size: int) -> List[bytes]:
    ...
    clusters_cnt: int = required_clusters_count(
        cluster_size=sector_size, content=content
    )
...
def get_args_for_partition_generator(desc: str, wl: bool) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        + (
            ""
            if wl
            else " Use `--partition_size detect` for detecting the minimal partition size."
        ),
    )
    parser.add_argument(
        "--sector_size",
    ...
    if not os.path.isdir(args.input_directory):
        raise NotADirectoryError(
            f"The target directory `{args.input_directory}` does not exist!"
        )
...
DATE_ENTRY = BitStruct(
    "year" / BitsInteger(7), "month" / BitsInteger(4), "day" / BitsInteger(5)
)
...
def build_date_entry(year: int, mon: int, mday: int) -> int:
    ...
    return int.from_bytes(
        DATE_ENTRY.build(dict(year=year - FATFS_INCEPTION_YEAR, month=mon, day=mday)),
        "big",
    )
...
class FATDefaults:
    ...
    ROOT_ENTRIES_COUNT: int = (
        512  # number of entries in the root directory, recommended 512
    )
    MEDIA_TYPE: int = 0xF8
