...
class BootSector:
    ...
    BOOT_SECTOR_HEADER = Struct(
        ...
        "BPB_TotSec16"
        / Int16ul,  # zero if the FAT type is 32, otherwise number of sectors
        "BPB_Media" / Int8ul,
        "BPB_FATSz16"
        / Int16ul,  # for FAT32 always zero, for FAT12/FAT16 number of sectors per FAT
        "BPB_SecPerTrk" / Int16ul,
        "BPB_NumHeads" / Int16ul,
        "BPB_HiddSec" / Int32ul,
        "BPB_TotSec32"
        / Int32ul,  # zero if the FAT type is 12/16, otherwise number of sectors
    ...
    def generate_boot_sector(self) -> None:
        ...
        pad_header: bytes = (
            boot_sector_state.sector_size - BootSector.BOOT_HEADER_SIZE
        ) * EMPTY_BYTE
        ...
        root_dir_content: bytes = (
            boot_sector_state.root_dir_sectors_cnt
            * boot_sector_state.sector_size
            * EMPTY_BYTE
        )
        data_content: bytes = (
            boot_sector_state.data_sectors * boot_sector_state.sector_size * FULL_BYTE
        )
        ...
        self.boot_sector_state.binary_image = (
            BootSector.BOOT_SECTOR_HEADER.build(
                dict(
                    ...
                    BS_OEMName=pad_string(
                        boot_sector_state.oem_name, size=BootSector.MAX_OEM_NAME_SIZE
                    ),
                    ...
                    BPB_TotSec16=0x00
                    if boot_sector_state.fatfs_type == FAT32
                    else boot_sector_state.sectors_count,
                    ...
                    BPB_TotSec32=boot_sector_state.sectors_count
                    if boot_sector_state.fatfs_type == FAT32
                    else 0x00,
                    BS_VolID=volume_uuid,
                    BS_VolLab=pad_string(
                        boot_sector_state.volume_label, size=BootSector.MAX_VOL_LAB_SIZE
                    ),
                    BS_FilSysType=pad_string(
                        boot_sector_state.file_sys_type,
    ...
    def parse_boot_sector(self, binary_data: bytes) -> None:
        ...
        total_root_bytes: int = (
            self._parsed_header["BPB_RootEntCnt"] * FATDefaults.ENTRY_SIZE
        )
        root_dir_sectors_cnt_: int = (
            total_root_bytes // self._parsed_header["BPB_BytsPerSec"]
        )
        ...
        assert self.boot_sector_state.file_sys_type in (
            f"FAT{self.boot_sector_state.fatfs_type}   ",
            "FAT     ",
        )
    ...
    def __str__(self) -> str:
        ...
        for member in getmembers(self.boot_sector_state, lambda a: not (isroutine(a))):
            ...
            if (
                isinstance(prop_, int)
                or isinstance(prop_, str)
    ...
    def binary_image(self) -> bytes:
        ...
        if (
            self.boot_sector_state is None
            or len(self.boot_sector_state.binary_image) == 0
        ):
