...
def generate_tests_cases(target):  # type: (str) -> None
    ...
    print(
        "Generating tests cases for {} (max key size = {})".format(target, max_key_size)
    )
    ...
    with open("digital_signature_test_cases.h", "w", encoding="utf-8") as f:
        ...
        f.write(
            "static const uint32_t test_messages[NUM_MESSAGES][%d/32] = {\n"
            % max_key_size
        )
        for m in messages:
            f.write("        // Message %d\n" % messages.index(m))
        ...
        for case in range(NUM_CASES):
            ...
            aes_key = hmac.HMAC(
                hmac_keys[hmac_key_idx], b"\xff" * 32, hashlib.sha256
            ).digest()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Generates Digital Signature Test Cases"""
    )
