...
class ParttoolTarget:
        ...
        if partition_table_file:
            ...
            try:
                self._call_esptool(
                    [
                        "read_flash",
                        str(partition_table_offset),
                        str(gen.MAX_PARTITION_LENGTH),
                        temp_file.name,
                    ]
                )
    ...
    def get_partition_info(self, partition_id):
        ...
        if partition_id.name:
            ...
            partition = list(
                self.partition_table.find_by_type(
                    partition_id.type, partition_id.subtype
                )
            )
            ...
            for subtype in search:
                partition = next(
                    self.partition_table.find_by_type("app", subtype), None
                )
                if partition:
                    break
    ...
    def erase_partition(self, partition_id):
        ...
        self._call_esptool(
            ["erase_region", str(partition.offset), str(partition.size)]
            + self.esptool_erase_args
        )

    def read_partition(self, partition_id, output):
        partition = self.get_partition_info(partition_id)
        self._call_esptool(
            ["read_flash", str(partition.offset), str(partition.size), output]
            + self.esptool_read_args
        )
    ...
    def write_partition(self, partition_id, input, ignore_readonly=False):
        ...
        if partition.readonly and not ignore_readonly:
            raise SystemExit(
                f'"{partition.name}" partition is read-only, (use the --ignore-readonly flag to skip it)'
            )
        ...
        self._call_esptool(
            ["write_flash", str(partition.offset), input] + self.esptool_write_args
        )
...
def _write_partition(target, partition_id, input, ignore_readonly=False):
    ...
    status(
        "Written contents of file '{}' at offset 0x{:x}".format(input, partition.offset)
    )
...
def _erase_partition(target, partition_id):
    ...
    partition = target.get_partition_info(partition_id)
    status(
        "Erased partition '{}' at offset 0x{:x}".format(
...
def main():
    ...
    parser.add_argument(
        "--port",
        "-p",
        help="port where the target device of the command is connected to; the partition table is sourced from this device \
    ...
    parser.add_argument(
        ...
        "-o",
        help="offset to read the partition table from",
        type=str,
    )
    parser.add_argument(
        "--primary-bootloader-offset", help="offset for primary bootloader", type=str
    )
    parser.add_argument(
        "--recovery-bootloader-offset", help="offset for recovery bootloader", type=str
    )
    parser.add_argument(
    ...
    partition_selection_args.add_argument(
        "--partition-name", "-n", help="name of the partition"
    )
    partition_selection_args.add_argument(
        "--partition-type", "-t", help="type of the partition"
    )
    partition_selection_args.add_argument(
    ...
    partition_selection_parser.add_argument(
        "--partition-subtype", "-s", help="subtype of the partition"
    )
    partition_selection_parser.add_argument(
    ...
    subparsers = parser.add_subparsers(
        dest="operation", help="run parttool -h for additional help"
    )
    ...
    read_part_subparser.add_argument(
        "--output", help="file to dump the read partition contents to"
    )
    ...
    write_part_subparser.add_argument(
        "--input", help="file whose contents are to be written to the partition offset"
    )
    write_part_subparser.add_argument(
        "--ignore-readonly", help="Ignore read-only attribute", action="store_true"
    )

    subparsers.add_parser(
        "erase_partition",
        help="erase the contents of a partition on the device",
        parents=[partition_selection_parser],
    )

    print_partition_info_subparser = subparsers.add_parser(
        "get_partition_info",
        help="get partition information",
        parents=[partition_selection_parser],
    )
    ...
    print_partition_info_subparser.add_argument(
        "--part_list",
        help="Get a list of partitions suitable for a given type",
        action="store_true",
    )
    ...
    if args.partition_name:
        ...
        if not args.partition_subtype:
            raise RuntimeError(
                "--partition-subtype should be defined when --partition-type is defined"
            )
        partition_id = PartitionType(
            args.partition_type,
            args.partition_subtype,
            getattr(args, "part_list", None),
        )
    ...
    if args.primary_bootloader_offset:
        target_args["primary_bootloader_offset"] = int(
            args.primary_bootloader_offset, 0
        )

    if args.recovery_bootloader_offset:
        target_args["recovery_bootloader_offset"] = int(
            args.recovery_bootloader_offset, 0
        )
