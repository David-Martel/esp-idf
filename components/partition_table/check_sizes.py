...
def check_bootloader(partition_table_offset, bootloader_offset, binary_file):  # type: (int, int, IO) -> None
    ...
    print(
        "Bootloader binary size {:#x} bytes. {:#x} bytes ({}%) free.".format(
...
def check_partition(ptype, subtype, partition_table_file, bin_file):  # type: (str, str, io.IOBase, IO) -> None
    ...
    if len(partitions) == 0:
        print(
            "WARNING: Partition table does not contain any partitions matching {}".format(
                ptype_str
            )
        )
        return
    ...
    if smallest_size >= bin_size:
        ...
        print(
            "{} binary size {:#x} bytes. Smallest {} partition is {:#x} bytes. {:#x} bytes ({:.0%}) free.".format(
                bin_name,
                bin_size,
                ptype_str,
                smallest_size,
        ...
        if free_size_relative < free_size_relative_critical:
            print(
                "Warning: The smallest {} partition is nearly full ({:.0%} free space left)!".format(
                    ptype_str, free_size_relative
                )
            )
        return
    ...
    if len(partitions) == 1:
        ...
        msg = "{}/{} {} partitions are".format(
            len(too_small_partitions), len(partitions), ptype_str
        )
...
def main():  # type: () -> None
    ...
    parser = argparse.ArgumentParser(
        description="Check binary sizes against partition table entries"
    )
    ...
    parser.add_argument(
        "--offset", "-o", help="Set partition table offset", default="0x8000"
    )
    ...
    sp_bootloader.add_argument(
        "bootloader_offset", help="Hex offset of bootloader in flash"
    )
    sp_bootloader.add_argument(
        "bootloader_binary",
        type=argparse.FileType("rb"),
        help="Bootloader binary (.bin) file from build output",
    )

    sp_part = subparsers.add_parser("partition")
    sp_part.add_argument(
        "--type",
        type=str,
        help="Check the file size against all partitions of this type.",
        required=True,
    )
    sp_part.add_argument(
        "--subtype",
        type=str,
        help="Optional, only check the file size against all partitions of this subtype.",
    )
    sp_part.add_argument(
        "partition_table", type=argparse.FileType("rb"), help="Partition table file"
    )
    sp_part.add_argument(
        "binary",
        type=argparse.FileType("rb"),
        help="Binary file which will have the size checked",
    )
    ...
    if (
        args.check_target is None
    ):  # add_subparsers only has a 'required' argument since Python 3
        parser.print_help()
        sys.exit(1)
    if args.check_target == "bootloader":
        check_bootloader(
            args.offset, int(args.bootloader_offset, 0), args.bootloader_binary
        )
    else:
