...
MAX_PARTITION_LENGTH = (
    0xC00  # 3K for partition data (96 entries) leaves 1K in a 4K sector for signature
)
MD5_PARTITION_BEGIN = (
    b"\xeb\xeb" + b"\xff" * 14
)  # The first 2 bytes are like magic numbers for MD5 sum
    ...
    for line_no in csv:
        try:
            ...
            for subtype, subtype_values in SUBTYPES.items():
                if int(
                    fields[2], 16
                ) in subtype_values.values() and subtype == get_partition_type(
                    fields[0]
                ):
...
class PartitionTable(list):
    ...
    def from_csv(cls, csv_contents):
        ...
        for line_no in range(len(lines)):
            ...
            try:
                ...
                critical(
                    "Unexpected error parsing CSV line %d: %s" % (line_no + 1, line)
                )
                raise

        # fix up missing offsets & negative sizes
        last_end = (
            offset_part_table + PARTITION_TABLE_SIZE
        )  # first offset after partition table
        for e in res:
            is_primary_bootloader = (
                e.type == BOOTLOADER_TYPE and e.subtype == SUBTYPES[e.type]["primary"]
            )
            is_primary_partition_table = (
                e.type == PARTITION_TABLE_TYPE
                and e.subtype == SUBTYPES[e.type]["primary"]
            )
    ...
    def verify(self):
        ...
        for p in sorted(self, key=lambda x: x.offset):
            if p.offset < offset_part_table + PARTITION_TABLE_SIZE:
                is_primary_bootloader = (
                    p.type == BOOTLOADER_TYPE
                    and p.subtype == SUBTYPES[p.type]["primary"]
                )
                is_primary_partition_table = (
                    p.type == PARTITION_TABLE_TYPE
                    and p.subtype == SUBTYPES[p.type]["primary"]
                )
                if not (is_primary_bootloader or is_primary_partition_table):
                    raise InputError(
                        "Partition offset 0x%x is below 0x%x"
                        % (p.offset, offset_part_table + PARTITION_TABLE_SIZE)
                    )
            if last is not None and p.offset < last.offset + last.size:
                raise InputError(
                    "Partition at 0x%x overlaps 0x%x-0x%x"
                    % (p.offset, last.offset, last.offset + last.size - 1)
                )
            last = p
        ...
        otadata_duplicates = [
            p
            for p in self
            if p.type == TYPES["data"] and p.subtype == SUBTYPES[DATA_TYPE]["ota"]
        ]
        ...
        otadata_duplicates = [
            p
            for p in self
            if p.type == TYPES["data"] and p.subtype == SUBTYPES[DATA_TYPE]["tee_ota"]
        ]
        if len(otadata_duplicates) > 1:
            for p in otadata_duplicates:
                critical("%s" % (p.to_csv()))
            raise InputError(
                'Found multiple TEE otadata partitions. Only one partition can be defined with type="data"(1) and subtype="tee_ota"(0x90).'
            )
    ...
    def from_binary(cls, b):
        ...
        for o in range(0, len(b), 32):
            ...
            if (
                md5sum and data[:2] == MD5_PARTITION_BEGIN[:2]
            ):  # check only the magic number part
    ...
    def to_binary(self):
        ...
        if len(result) >= MAX_PARTITION_LENGTH:
            raise InputError(
                "Binary partition table length (%d) longer than max" % len(result)
            )
        result += b"\xff" * (
            MAX_PARTITION_LENGTH - len(result)
        )  # pad the sector, for signing
        return result
...
class PartitionDefinition(object):
    ...
    for ota_slot in range(NUM_PARTITION_SUBTYPE_APP_OTA):
        SUBTYPES[TYPES["app"]]["ota_%d" % ota_slot] = (
            MIN_PARTITION_SUBTYPE_APP_OTA + ota_slot
        )

    # add subtypes for the 2 TEE OTA slot values ("tee_XX, etc.")
    for tee_slot in range(NUM_PARTITION_SUBTYPE_APP_TEE):
        SUBTYPES[TYPES["app"]]["tee_%d" % tee_slot] = (
            MIN_PARTITION_SUBTYPE_APP_TEE + tee_slot
        )
    ...
    def parse_size(self, strval, ptype):
        if ptype == BOOTLOADER_TYPE:
            if primary_bootloader_offset is None:
                raise InputError(
                    "Primary bootloader offset is not defined. Please use --primary-bootloader-offset"
                )
    ...
    def verify(self):
        ...
        if self.offset % offset_align:
            raise ValidationError(
                self, "Offset 0x%x is not aligned to 0x%x" % (self.offset, offset_align)
            )
        if self.type == APP_TYPE:
            size_align = get_alignment_size_for_type(self.type)
            if self.size % size_align:
                raise ValidationError(
                    self, "Size 0x%x is not aligned to 0x%x" % (self.size, size_align)
                )
        ...
        if (
            self.name in all_subtype_names
            and SUBTYPES.get(self.type, {}).get(self.name, "") != self.subtype
        ):
            critical(
        ...
        always_rw_data_subtypes = [
            SUBTYPES[DATA_TYPE]["ota"],
            SUBTYPES[DATA_TYPE]["coredump"],
        ]
        if (
            self.type == TYPES["data"]
            and self.subtype in always_rw_data_subtypes
            and self.readonly is True
        ):
            raise ValidationError(
                self,
    ...
    def from_binary(cls, b):
        if len(b) != 32:
            raise InputError(
                "Partition definition length must be exactly 32 bytes. Got %d bytes."
                % len(b)
            )
        res = cls()
        ...
        if magic != cls.MAGIC_BYTES:
            raise InputError(
                "Invalid magic bytes (%r) for partition definition" % magic
            )
        ...
        if flags != 0:
            critical(
                "WARNING: Partition definition had unknown flag(s) 0x%08x. Newer binary format?"
                % flags
            )
        return res
...
def parse_int(v, keywords={}):
    ...
    try:
        ...
        try:
            ...
            raise InputError(
                "Value '%s' is not valid. Known keywords: %s" % (v, ", ".join(keywords))
            )
...
def main():
    ...
    parser.add_argument(
        "--disable-md5sum",
        help="Disable md5 checksum for the partition table",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--no-verify", help="Don't verify partition table fields", action="store_true"
    )
    parser.add_argument(
        "--verify",
        "-v",
    ...
    parser.add_argument(
        "--offset", "-o", help="Set offset partition table", default="0x8000"
    )
    parser.add_argument(
        "--primary-bootloader-offset",
        help="Set primary bootloader offset",
        default=None,
    )
    parser.add_argument(
        "--recovery-bootloader-offset",
        help="Set recovery bootloader offset",
        default=None,
    )
    parser.add_argument(
        "--secure",
    ...
    parser.add_argument(
        "input",
        help="Path to CSV or binary file to parse.",
    ...
    if input_is_binary:
        ...
        with (
            sys.stdout
            if args.output == "-"
            else open(args.output, "w", encoding="utf-8") as f
        ):
            f.write(output)
    else:
