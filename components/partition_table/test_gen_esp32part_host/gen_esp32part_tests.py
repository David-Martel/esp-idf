...
LONGER_BINARY_TABLE += (
    b"\xf9\xbd\x06\x1b\x45\x68\x6f\x86\x57\x1a\x2c\xd5\x2a\x1d\xa6\x5b"
)
# empty partition
...
"""
        with self.assertRaisesRegex(
            gen_esp32part.InputError, "Partition names must be unique"
        ):
            t = gen_esp32part.PartitionTable.from_csv(csv)
            t.verify()
...
"""
        ...
        bootloader_size = (
            gen_esp32part.offset_part_table - gen_esp32part.primary_bootloader_offset
        )
        t = gen_esp32part.PartitionTable.from_csv(csv)
        t.verify()
        # bootloader
...
"""
        ...
        self.assertEqual(
            b"\x43\x03\x3f\x33\x40\x87\x57\x51\x69\x83\x9b\x40\x61\xb1\x27\x26",
            tb[48:64],
        )
    ...
    def test_basic_cmdline(self):
        try:
            ...
            output = subprocess.check_output(
                [sys.executable, "../gen_esp32part.py", binpath, csvpath],
            ...
            self.assertEqual(
                _strip_trailing_ffs(from_csv.to_binary()), LONGER_BINARY_TABLE
            )

            # run gen_esp32part.py to convert the CSV to binary again
            output = subprocess.check_output(
                [sys.executable, "../gen_esp32part.py", csvpath, binpath],
                stderr=subprocess.STDOUT,
            )
            self.assertNotIn(b"WARNING", output)
...
class VerificationTests(Py23TestCase):
    def _run_genesp32(self, csvcontents, args):
        ...
        try:
            output = subprocess.check_output(
                [sys.executable, "../gen_esp32part.py", csvpath] + args,
                stderr=subprocess.STDOUT,
            )
            return output.strip()
...
"""
        with self.assertRaisesRegex(
            gen_esp32part.ValidationError, r"Offset.+not aligned"
        ):
            t = gen_esp32part.PartitionTable.from_csv(csv)
            t.verify()
...
"""
        with self.assertRaisesRegex(
            gen_esp32part.InputError,
            r"CSV Error at line 3: Partitions overlap. Partition sets offset 0x0",
        ):
            gen_esp32part.PartitionTable.from_csv(csv)
...
"""
        with self.assertRaisesRegex(
            gen_esp32part.InputError, r"Found multiple otadata partitions"
        ):
            t = gen_esp32part.PartitionTable.from_csv(csv_txt)
            t.verify()
...
"""
        with self.assertRaisesRegex(
            gen_esp32part.InputError, r"otadata partition must have size = 0x2000"
        ):
            t = gen_esp32part.PartitionTable.from_csv(csv_txt)
            t.verify()
...
"""
        with self.assertRaisesRegex(
            gen_esp32part.InputError, r"App partition cannot have an empty subtype"
        ):
            t = gen_esp32part.PartitionTable.from_csv(csv_txt)
            t.verify()
...
ota_1,  app,    ota_1,          ,  1M
        ...
        self.assertEqual(
            rpt(["--partition-boot-default", "--info", "offset"]), b"0x30000"
        )  # ota_0
