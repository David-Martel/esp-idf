...
class StartupEntry:
    def __init__(
        self, filename: str, func: str, stage: str, affinity: str, priority: int
    ) -> None:
...
def main() -> None:
    ...
    for extension in ("c", "cpp"):
        glob_iter = glob.glob(
            os.path.join(idf_path, "components", "**", f"*.{extension}"), recursive=True
        )
    ...
    startup_entries = sorted(startup_entries, key=sort_key)
    ...
    with open(
        os.path.join(idf_path, STARTUP_ENTRIES_FILE), "r", encoding="utf-8"
    ) as startup_entries_expected_file:
    ...
    diff_lines = list(
        difflib.unified_diff(
            startup_entries_expected_lines, startup_entries_lines, lineterm=""
        )
    )
    if len(diff_lines) > 0:
        print(
