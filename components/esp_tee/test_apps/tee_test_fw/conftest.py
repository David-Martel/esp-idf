...
class TEESerial(IdfSerial):
    ...
    def bootloader_force_flash_if_req(self) -> None:
        ...
        if any(
            ...
            encrypt = (
                "--encrypt"
                if self.app.sdkconfig.get("SECURE_FLASH_ENC_ENABLED")
                else ""
            )
    ...
    def copy_test_tee_img(self, partition: str, is_rollback: bool = False) -> None:
        ...
        encrypt = (
            "--encrypt" if self.app.sdkconfig.get("SECURE_FLASH_ENC_ENABLED") else ""
        )
        flash_size = self._get_flash_size()
        ...
        if is_rollback:
            ...
            with open(datafile, "wb") as data_file:
                bin_data = esp_tee_empty_bin.get(
                    self.app.sdkconfig.get("IDF_TARGET"), None
                )
