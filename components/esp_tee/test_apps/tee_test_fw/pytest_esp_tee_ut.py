...
def test_esp_tee_apm_violation(dut: IdfDut) -> None:
    for check in TEE_APM_VIOLATION_EXC_CHK:
        ...
        exc = (
            dut.expect(r"Core ([01]) panic\'ed \(([^)]+)\)", timeout=30)
            .group(2)
            .decode()
        )
    ...
    for stage in range(1, stages + 1):
        ...
        if 1 < stage <= stages:
            ...
            if rst_rsn != "Cache error":
                raise RuntimeError(
                    "Incorrect reset reason observed after TEE image failure!"
                )
...
def test_esp_tee_flash_prot_esp_partition_mmap(dut: IdfDut) -> None:
    ...
    for test_case in extra_data:
        if (
            test_case.name
            == "Test REE-TEE isolation: Flash - SPI1 (esp_partition_mmap)"
        ):
...
def tee_ota_stage_checks(dut: IdfDut, stage: TeeOtaStage, offset: str) -> None:
    if stage == TeeOtaStage.PRE:
        ...
        dut.expect_exact(
            f"Next partition - Subtype: 0x31 (Offset: {offset})", timeout=30
        )
...
def test_esp_tee_ota_rollback(dut: IdfDut) -> None:
    ...
    rst_rsn = (
        dut.expect(r"rst:(0x[0-9A-Fa-f]+) \(([^)]+)\)", timeout=30).group(2).decode()
    )
