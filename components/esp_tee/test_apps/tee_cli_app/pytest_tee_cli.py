...
def test_tee_cli_secure_storage(dut: Dut) -> None:
    ...
    test_msg_hash = dut.expect(
        r"Message digest \(SHA256\) -\s*([0-9a-fA-F]{64})", timeout=30
    )[1].decode()
    time.sleep(1)
    ...
    for i in range(iterations):
        ...
        test_msg_sign = dut.expect(
            r"Generated signature -\s*([0-9a-fA-F]{128})", timeout=30
        )[1].decode()
        test_msg_pubkey = dut.expect(
            r"Public key \(Uncompressed\) -\s*([0-9a-fA-F]{130})", timeout=30
        )[1].decode()
        dut.expect("Signature verified successfully", timeout=30)

        vk = VerifyingKey.from_string(
            bytes.fromhex(test_msg_pubkey), curve=NIST256p, hashfunc=hashlib.sha256
        )
        assert vk.verify_digest(
            bytes.fromhex(test_msg_sign), bytes.fromhex(test_msg_hash)
        )
        time.sleep(1)
    ...
    for i in range(iterations):
        ...
        test_msg_cipher = dut.expect(r"Ciphertext -\s*([0-9a-fA-F]{64})", timeout=30)[
            1
        ].decode()
        test_msg_tag = dut.expect(r"Tag -\s*([0-9a-fA-F]{32})", timeout=30)[1].decode()

        dut.write(
            f"tee_sec_stg_decrypt {sec_stg_slots.get(i)} {test_msg_cipher} {test_msg_tag}"
        )
        test_msg_decipher = dut.expect(
            r"Decrypted plaintext -\s*([0-9a-fA-F]{64})", timeout=30
        )[1].decode()
...
def verify_att_token_signature(att_tk: str) -> Any:
    ...
    tk_pubkey_val = json.dumps(tk_info["public_key"], separators=(",", ":")).encode(
        "latin-1"
    )
    ...
    signature = utils.encode_dss_signature(
        int.from_bytes(tk_sign_r, "big"), int.from_bytes(tk_sign_s, "big")
    )
...
def start_https_server(ota_image_dir: str, server_ip: str, server_port: int) -> None:
    ...
    httpd.socket = ssl.wrap_socket(
        httpd.socket, keyfile=key_file, certfile=server_file, server_side=True
    )
    httpd.serve_forever()
