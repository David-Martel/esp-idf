...
class FuseTable(list):
    ...
    def from_csv(cls, csv_contents):
        ...
        for i in res:
            if i.field_name == "" and last_field is None:
                raise InputError(
                    "Error at line %d: %s missing field name" % (line_no + 1, i)
                )
        ...
        if len(duplicates) != 0:
            ...
            for p in self:
                ...
                if field_name != "" and len(duplicates.intersection([field_name])) != 0:
                    fl_error = True
                    print(
                        "Field at %s, %s, %s, %s have duplicate field_name"
    ...
    def show_range_used_bits(self):
        ...
        for p in sorted(self, key=lambda x: (x.efuse_block, x.bit_start)):
            rows += (
                "{0} \t{1:<30} \t{2} \t{3:^8} \t{4:^8}".format(
                    num, p.field_name, p.efuse_block, p.bit_start, p.bit_count
                )
                + "\n"
            )
            num += 1
    ...
    def get_str_position_last_free_bit_in_blk(self, blk):
        ...
        for p in self:
            if p.efuse_block == blk:
                ...
                elif last_used_bit < p.bit_start + p.bit_count:
    ...
    def to_header(self, file_name):
        ...
        rows += [
            ...
            + file_name
            + ".csv. DO NOT CHANGE THIS FILE MANUALLY.",
            "// If you want to change some fields, you need to change "
            + file_name
            + ".csv file",
    ...
    def to_c_file(self, file_name, debug):
        ...
        if idf_target == "esp32":
            ...
            last_free_bit_blk1 = self.get_str_position_last_free_bit_in_blk(
                "EFUSE_BLK1"
            )
            last_free_bit_blk2 = self.get_str_position_last_free_bit_in_blk(
                "EFUSE_BLK2"
            )
            last_free_bit_blk3 = self.get_str_position_last_free_bit_in_blk(
                "EFUSE_BLK3"
            )
            ...
            if last_free_bit_blk1 is not None:
                rows += [
                    '_Static_assert(LAST_FREE_BIT_BLK1 <= MAX_BLK_LEN, "The eFuse table does not match the coding scheme. '
                    'Edit the table and restart the efuse_common_table or efuse_custom_table command to regenerate the new files.");'
                ]
            if last_free_bit_blk2 is not None:
                rows += [
                    '_Static_assert(LAST_FREE_BIT_BLK2 <= MAX_BLK_LEN, "The eFuse table does not match the coding scheme. '
                    'Edit the table and restart the efuse_common_table or efuse_custom_table command to regenerate the new files.");'
                ]
            if last_free_bit_blk3 is not None:
                rows += [
                    '_Static_assert(LAST_FREE_BIT_BLK3 <= MAX_BLK_LEN, "The eFuse table does not match the coding scheme. '
                    'Edit the table and restart the efuse_common_table or efuse_custom_table command to regenerate the new files.");'
                ]
        ...
        for p in self:
            if p.field_name != last_name:
                if last_name != "":
                    rows += ["};\n"]
                rows += [
                    "static const esp_efuse_desc_t "
                    + p.field_name.replace(".", "_")
                    + "[] = {"
                ]
    ...
    def parse_block(self, strval):
        ...
        if idf_target == "esp32":
            if strval not in ["EFUSE_BLK0", "EFUSE_BLK1", "EFUSE_BLK2", "EFUSE_BLK3"]:
                raise InputError(
                    "Field 'efuse_block' should be one of EFUSE_BLK0..EFUSE_BLK3"
                )
        elif strval not in [
            "EFUSE_BLK0",
            "EFUSE_BLK1",
            ...
            raise InputError(
                "Field 'efuse_block' should be one of EFUSE_BLK0..EFUSE_BLK10"
            )
    ...
    def verify(self, type_table):
        ...
        if self.bit_start + self.bit_count > max_bits:
            raise ValidationError(
                self,
                "The field is outside the boundaries(max_bits = %d) of the %s block"
                % (max_bits, self.efuse_block),
            )
...
def main():
    ...
    parser.add_argument(
        "--debug",
        help="Create header file with debug info",
        default=False,
        action="store_false",
    )
    parser.add_argument(
        "--info",
        help="Print info about range of used bits",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--max_blk_len", help="Max number of bits in BLOCKs", type=int, default=256
    )
    parser.add_argument(
        "common_input",
        help="Path to common CSV file to parse.",
        type=argparse.FileType("r"),
    )
    parser.add_argument(
        "custom_input",
        help="Path to custom CSV file to parse.",
        type=argparse.FileType("r"),
        nargs="?",
        default=None,
    )
