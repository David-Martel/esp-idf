    ...
    def send_eth_packet(self, mac: str) -> None:
        with self.configure_eth_if() as so:
            ...
            for i, _ in enumerate(payload):
                payload[i] = i & 0xFF
            eth_frame = Ether(
                dst=mac, src=so.getsockname()[4], type=self.eth_type
            ) / raw(payload)
            try:
        ...
        with self.configure_eth_if(eth_type_ctrl) as so:
            ...
            for _ in range(10):
                ...
                if mac == eth_frame.src and eth_frame.load[0] == 0xFA:
                    ...
                    logging.warning(
                        "Expected Ctrl command: 0xfa, actual: 0x%x", eth_frame.load[0]
                    )
...
@pytest.mark.parametrize(
    "config", ["default_ip101", "release_ip101", "single_core_ip101"], indirect=True
)
