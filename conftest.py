...
if (
    os.path.join(os.path.dirname(__file__), "tools", "ci", "python_packages")
    not in sys.path
):
    sys.path.append(
        os.path.join(os.path.dirname(__file__), "tools", "ci", "python_packages")
...
def pipeline_id(request: FixtureRequest) -> t.Optional[str]:
    return request.config.getoption("pipeline_id", None) or os.getenv(
...
class BuildReportDownloader(AppDownloader):
    def __init__(self, presigned_url_yaml: str) -> None:
        self.app_presigned_urls_dict: t.Dict[str, t.Dict[str, str]] = yaml.safe_load(
            presigned_url_yaml
        )
    ...
    def download_app(
        self, app_build_path: str, artifact_type: t.Optional[ArtifactType] = None
    ) -> None:
...
def app_downloader(pipeline_id: t.Optional[str]) -> t.Optional[AppDownloader]:
    ...
    for child_pipeline in gl.project.pipelines.get(pipeline_id, lazy=True).bridges.list(
        iterator=True
    ):
        if child_pipeline.name == "build_child_pipeline":
            for job in gl.project.pipelines.get(
                child_pipeline.downstream_pipeline["id"], lazy=True
            ).jobs.list(iterator=True):
    ...
    if app_downloader:
        ...
        if case.requires_elf_or_map:
            ...
            app_downloader.download_app(
                app_build_path, ArtifactType.BUILD_DIR_WITHOUT_MAP_AND_ELF_FILES
            )
...
def junit_properties(
    test_case_name: str, record_xml_attribute: t.Callable[[str, object], None]
) -> None:
    """
...
@pytest.fixture(autouse=True)
def set_dut_log_url(
    record_xml_attribute: t.Callable[[str, object], None], _pexpect_logfile: str
) -> t.Generator:
    ...
    def real_func(item: str, value: float, target: str) -> None:
        ...
        def _find_perf_item(operator: str, path: str) -> float:
            with open(path, encoding="utf-8") as f:
                data = f.read()
            match = re.search(
                rf"#define\s+IDF_PERFORMANCE_{operator}_{item.upper()}\s+([\d.]+)", data
            )
...
def pytest_configure(config: Config) -> None:
    ...
    target = [
        _t.strip().lower()
        for _t in (config.getoption("target", "") or "").split(",")
        if _t.strip()
    ]
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):  # type: ignore
    if dut_artifacts_url:
        ...
        terminalreporter.section(
            "Failed Test Artifacts URL", sep="-", red=True, bold=True
        )
