...
linkcheck_ignore = [
    "https://webhome.phy.duke.edu/~rgb/General/dieharder.php",  # Certificate error
...
idf_build_system = {
    ...
    "component_info_ignore_file": Path(os.environ["IDF_PATH"])
    / "docs"
...
def conf_setup(app, config):
    config.add_warnings_content = "This document is not updated for {} yet, so some of the content may not be correct.".format(
        config.idf_target.upper()
    )

    add_warnings_file = "{}/../docs_not_updated/{}.txt".format(
        app.confdir, config.idf_target
    )
