...
class SwitchSsh:
    ...
    def __init__(self, host_ip: str, usr: str, passwd: str, device_type: int):
        ...
        if self.type == self.EDGE_SWITCH_5XP:
            ...
            self.ssh_client.connect(
                hostname=self.host_ip, username=usr, password=passwd
            )
        else:
            edgeSwitch = {
    ...
    def exec_cmd(self, cmd: Union[str, List[str]]) -> str:
        if self.type == self.EDGE_SWITCH_5XP:
            ...
            out = self.ssh_client.send_config_set(
                cmd, cmd_verify=False, exit_config_mode=False
            )
        return out  # type: ignore
...
def get_host_mac_by_interface(
    interface_name: str, addr_type: int = netifaces.AF_LINK
) -> str:
...
def get_host_brcast_ip_by_interface(
    interface_name: str, ip_type: int = netifaces.AF_INET
) -> str:
...
def run_iperf(
    ...
    if ipaddress.ip_address(server_ip).is_multicast:
        ...
        client_res = endnode.exec_cmd(
            "iperf -u -c %s -t %i -i 1 -b %iM --ttl 5 -B %s"
            % (server_ip, interval, bandwidth_lim, endnode_ip)
        )
        ...
        # Configure Client
        client_res = endnode.exec_cmd(
            "iperf %s -c %s -t %i -i 1 -b %iM"
            % (proto, server_ip, interval, bandwidth_lim)
        )
        if server_proc.wait(10) is None:  # Process did not finish.
            server_proc.terminate()
...
def send_brcast_msg_host_to_endnode(
    endnode: EndnodeSsh, host_brcast_ip: str, test_msg: str
) -> str:
...
def send_brcast_msg_endnode_to_host(
    endnode: EndnodeSsh, host_brcast_ip: str, test_msg: str
) -> str:
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.settimeout(5)
...
def test_esp_eth_bridge(dut: Dut, dev_user: str, dev_password: str) -> None:
    ...
    switch1 = SwitchSsh(
        f"10.10.{sw_num}.100", dev_user, dev_password, SwitchSsh.EDGE_SWITCH_10XP
    )
    ...
    switch1.switch_port_up(
        port_num
    )  # link up port #1 as last to ensure we Got IP address after link port #2 is up
    dut.expect_exact(f"Ethernet ({p1_id}) Link Up")
    dut.expect_exact("Ethernet Got IP Address")
    ...
    # multicast UDP
    bandwidth_mcast_udp = run_iperf(
        "udp", endnode, "224.0.1.4", IPERF_BW_LIM, 5, host_if, endnode_if
    )
    if bandwidth_mcast_udp < MIN_UDP_THROUGHPUT:
        ...
        bandwidth_mcast_udp = run_iperf(
            "udp", endnode, "224.0.1.4", IPERF_BW_LIM, 60, host_if, endnode_if
        )
    logging.info("Multicast UDP average bandwidth: %s Mbits/s", bandwidth_mcast_udp)

    if bandwidth_udp < MIN_UDP_THROUGHPUT:
        raise RuntimeError(
            "Unicast UDP throughput expected %.2f, actual %.2f"
            % (MIN_UDP_THROUGHPUT, bandwidth_udp)
            + " Mbits/s"
        )
    if bandwidth_tcp < MIN_TCP_THROUGHPUT:
        raise RuntimeError(
            "Unicast TCP throughput expected %.2f, actual %.2f"
            % (MIN_TCP_THROUGHPUT, bandwidth_tcp)
            + " Mbits/s"
        )
    if bandwidth_mcast_udp < MIN_UDP_THROUGHPUT:
        raise RuntimeError(
            "Multicast UDP throughput expected %.2f, actual %.2f"
            % (MIN_UDP_THROUGHPUT, bandwidth_mcast_udp)
            + " Mbits/s"
        )
