...
def ping_test(config: dict) -> None:
    ...
    print(
        f"Captured: {len(vlanServer_pkt_list)} packets on interface {config['vlanServer']['name']}"
    )
...
def udp_client(serverip: str, port: int) -> None:
    ...
    print(
        run_cmd_sec(
            f"timeout 10s ip netns exec ns_vlanClient iperf3 -c {serverip} -u -p {port} --bidir -k 20"
        )
    )
...
def udp_server_client_comm(config: dict, port: int) -> None:
    ...
    client_thread = threading.Thread(
        target=udp_client, args=(config["vlanServer"]["ip"], port)
    )
...
def udp_lfilter(packet: layers.l2.Ether) -> layers.l2.Ether:
    return UDP in packet and (
        packet[UDP].dport == udp_port or packet[UDP].sport == udp_port
    )
...
def udp_test(config: dict) -> None:
    ...
    capture = AsyncSniffer(
        iface=config["vlanServer"]["name"], filter="udp", lfilter=udp_lfilter, count=10
    )
...
def tcp_client(serverip: str, port: int) -> None:
    ...
    print(
        run_cmd_sec(
            f"timeout 10s ip netns exec ns_vlanClient iperf3 -c {serverip} -p {port} --bidir -k 20"
        )
    )
...
def tcp_server_client_comm(config: dict, port: int) -> None:
    ...
    client_thread = threading.Thread(
        target=tcp_client, args=(config["vlanServer"]["ip"], port)
    )
...
def tcp_lfilter(packet: layers.l2.Ether) -> layers.l2.Ether:
    return TCP in packet and (
        packet[TCP].dport == tcp_port or packet[TCP].sport == tcp_port
    )
...
def tcp_test(config: dict) -> None:
    ...
    capture = AsyncSniffer(
        iface=config["vlanServer"]["name"], filter="tcp", lfilter=tcp_lfilter, count=10
    )
