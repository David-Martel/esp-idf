...
def test_wifi_prov_mgr(dut: Dut, sec_ver: int) -> None:
    ...
    # Parse BLE devname
    devname = (
        dut.expect(b"Provisioning started with service name : PROV_(.{6})")[0]
        .decode("utf-8")
        .split(" : ")[1]
    )
    ...
    if sec_ver == 1:
        ...
        security = esp_prov.get_security(
            sec_ver, sec2_username, sec2_password, pop, verbose
        )
    elif sec_ver == 2:
        pop = None
...
@pytest.mark.xfail(
    reason="Runner unable to connect to target over Bluetooth", run=False
)
