...
def test_config_zigbee_network(dut: Tuple[Dut, Dut]) -> None:
    ...
    switch_node_expanid = switch.expect(
        r"Extended PAN ID: (([a-z0-9]{2}:?){8})", timeout=3
    )[1].decode()
    ...
    if (light_node_expanid != switch_node_expanid) or (
        light_node_panid != switch_node_panid
    ):
        assert False
