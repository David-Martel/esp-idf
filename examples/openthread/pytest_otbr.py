...
def test_Bidirectional_IPv6_connectivity(
    Init_interface: bool, dut: Tuple[IdfDut, IdfDut, IdfDut]
) -> None:
    br = dut[2]
    cli = dut[1]
    ...
    try:
        ...
        host_global_unicast_addr = re.findall(
            r"\W+(%s(?:\w+:){3}\w+)\W+" % onlinkprefix, str(out_str)
        )
        rx_nums = 0
...
def test_multicast_forwarding_A(
    Init_interface: bool, dut: Tuple[IdfDut, IdfDut, IdfDut]
) -> None:
    br = dut[2]
    cli = dut[1]
    ...
    try:
        ...
        target_udp = ocf.udp_parameter(
            "INET6", "ff04::125", 12350, "", False, 15.0, b"hello"
        )
...
def test_multicast_forwarding_B(
    Init_interface: bool, dut: Tuple[IdfDut, IdfDut, IdfDut]
) -> None:
    br = dut[2]
    cli = dut[1]
...
def test_UDP_NAT64(Init_interface: bool, dut: Tuple[IdfDut, IdfDut, IdfDut]) -> None:
    ...
    try:
        ...
        host_ipv4_address = ocf.get_host_ipv4_address()
        print("host_ipv4_address: ", host_ipv4_address)
        myudp = ocf.udp_parameter(
            "INET4", host_ipv4_address, 5090, "", False, 15.0, b""
        )
        udp_mission = threading.Thread(target=ocf.create_host_udp_server, args=(myudp,))
        udp_mission.start()
        start_time = time.time()
...
def test_TCP_NAT64(Init_interface: bool, dut: Tuple[IdfDut, IdfDut, IdfDut]) -> None:
    ...
    try:
        ...
        mytcp = ocf.tcp_parameter(
            "INET4", host_ipv4_address, 12345, False, False, 15.0, b""
        )
...
def test_ot_sleepy_device(dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    try:
        ...
        time.sleep(3)
        leader_para = ocf.thread_parameter(
            "leader", "", "12", "7766554433221100", False
        )
        ...
        info = sleepy_device.expect(r"(.+)detached -> child", timeout=20)[1].decode(
            errors="replace"
        )
        assert not bool(fail_info.search(str(info)))
        info = sleepy_device.expect(r"(.+)PMU_SLEEP_PD_TOP: True", timeout=10)[
            1
        ].decode(errors="replace")
        assert not bool(fail_info.search(str(info)))
        info = sleepy_device.expect(r"(.+)PMU_SLEEP_PD_MODEM: True", timeout=20)[
            1
        ].decode(errors="replace")
        assert not bool(fail_info.search(str(info)))
...
def test_br_meshcop(
    Init_interface: bool, Init_avahi: bool, dut: Tuple[IdfDut, IdfDut]
) -> None:
    br = dut[1]
    assert Init_interface
    assert Init_avahi
...
def test_https_NAT64_DNS(
    ...
    try:
        ...
        assert "<html>" in str(message)
        assert "This domain is for use in illustrative examples in documents" in str(
            message
        )
    finally:
        ocf.execute_command(br, "factoryreset")
