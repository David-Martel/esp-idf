...
def joinWiFiNetwork(dut: IdfDut, wifi: wifi_parameter) -> Tuple[str, int]:
    ...
    raise Exception(
        f"{dut} connect wifi {str(wifi.ssid)} with password {str(wifi.psk)} fail"
    )
...
def get_global_unicast_addr(dut: IdfDut, br: IdfDut) -> str:
    ...
    dut_adress = dut.expect(r"(%s(?:\w+:){3}\w+)\r" % str(omrprefix), timeout=5)[
        1
    ].decode()
    return dut_adress
...
def get_host_interface_name() -> str:
    ...
    raise Exception(
        "Warning: No valid network interface detected. Please check your configuration."
    )
    ...
    try:
        ...
        if myudp.udp_type == "INET6" and myudp.group != "":
            sock.setsockopt(
                ...
                struct.pack(
                    "16si", socket.inet_pton(socket.AF_INET6, myudp.group), if_index
                ),
            )
...
def host_udp_send_message(udp_target: udp_parameter) -> None:
    ...
    try:
        ...
        sock.bind(("::", 12350))
        sock.setsockopt(
            socket.SOL_SOCKET, socket.SO_BINDTODEVICE, interface_name.encode()
        )
...
def host_close_service() -> None:
    ...
    out_str = out_bytes.decode("utf-8")
    service_info = [
        line for line in out_str.splitlines() if "testxxx _testxxx._udp" in line
    ]
...
def flush_ipv6_addr_by_interface() -> None:
    ...
    subprocess.run(["ip", "link", "set", interface_name, "down"], check=False)
    subprocess.run(["ip", "-6", "addr", "flush", "dev", interface_name], check=False)
    subprocess.run(["ip", "-6", "route", "flush", "dev", interface_name], check=False)
    subprocess.run(["ip", "link", "set", interface_name, "up"], check=False)
    time.sleep(5)
