...
class IperfTestUtilityEth(IperfUtility.IperfTestUtility):
    ...
    def setup(self) -> Tuple[str, int]:
        ...
        dut_ip = self.dut.expect(
            r"esp_netif_handlers: .+ ip: (\d+\.\d+\.\d+\.\d+),"
        ).group(1)
        rssi = 0
        return dut_ip, rssi
...
def test_esp_eth_iperf(
    ...
    test_utility = IperfTestUtilityEth(
        dut, "ethernet", pc_nic_ip, pc_iperf_log_file, test_result
    )
...
def test_esp_eth_iperf_dm9051(
    ...
    test_esp_eth_iperf(
        dut, log_performance, check_performance, spi_eth=True, udp_rx_bw_lim=10
    )
