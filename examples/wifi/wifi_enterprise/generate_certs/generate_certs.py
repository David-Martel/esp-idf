...
class GenCerts:
    ...
    def generate_ca_certificate(self) -> None:
        if self.cert_type == "p384":
            ...
            private_key = rsa.generate_private_key(
                public_exponent=65537, key_size=int(self.cert_type)
            )
    ...
    def generate_user_certificate(self, role: str) -> None:
        ...
        subject = x509.Name([x509.NameAttribute(x509.NameOID.COMMON_NAME, role)])
        ...
        user_crt_builder = (
            ...
            .add_extension(
                x509.SubjectKeyIdentifier.from_public_key(user_csr.public_key()),
                critical=False,
            )
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "cert_type", choices=["2048", "3072", "4096", "p384"], help="select cert type"
    )
    parser.add_argument(
        "message_digest",
        choices=["sha256", "sha384"],
        help="select message digest, suiteB should be sha384",
    )
    file_path = os.path.dirname(__file__)
    parser.add_argument(
        "--output", "-o", default=file_path, help="path to store certificates"
    )
