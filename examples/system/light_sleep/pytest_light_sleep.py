...
def test_light_sleep(dut: Dut) -> None:
    ...
    EXIT_SLEEP_REGEX = (
        r"Returned from light sleep, reason: (\w+), t=(\d+) ms, slept for (\d+) ms"
    )
    EXIT_SLEEP_PIN_REGEX = (
        r"Returned from light sleep, reason: (pin), t=(\d+) ms, slept for (\d+) ms"
    )
    EXIT_SLEEP_UART_REGEX = (
        r"Returned from light sleep, reason: (uart), t=(\d+) ms, slept for (\d+) ms"
    )
    ...
    logging.info(
        "Got third sleep period, wakeup from {}, slept for {}".format(
            match.group(1), match.group(3)
        )
    )
