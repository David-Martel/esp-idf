...
TIMER_EXPIRY_HANDLING = (
    "TIMER_EVENTS:TIMER_EVENT_EXPIRY: timer_expiry_handler, executed {} out of "
    + str(TIMER_EVENT_LIMIT)
    + " times"
)
...
TASK_ITERATION_POST = (
    "TASK_EVENTS:TASK_ITERATION_EVENT: posting to default loop, {} out of "
    + str(TASK_ITERATION_LIMIT)
)
TASK_ITERATION_HANDLING = (
    "TASK_EVENTS:TASK_ITERATION_EVENT: task_iteration_handler, executed {} times"
)
...
def test_default_event_loop_example_iteration_events(dut: Dut) -> None:
    ...
    for iteration in range(1, TASK_ITERATION_LIMIT + 1):
        ...
        logging.info(
            "Posted iteration {} out of {}".format(iteration, TASK_ITERATION_LIMIT)
        )
        ...
        if iteration < TASK_UNREGISTRATION_LIMIT:
            ...
            logging.info(
                "Unregistered handler at iteration {} out of {}".format(
                    iteration, TASK_ITERATION_LIMIT
                )
            )
        else:
            dut.expect_exact("TASK_EVENTS:TASK_ITERATION_EVENT: all_event_handler")
        ...
        logging.info(
            "Handled iteration {} out of {}".format(iteration, TASK_ITERATION_LIMIT)
        )
...
def test_default_event_loop_example_timer_events(dut: Dut) -> None:
    ...
    for expiries in range(1, TIMER_EVENT_LIMIT + 1):
        ...
        logging.info(
            "Posted timer expiry event {} out of {}".format(expiries, TIMER_EVENT_LIMIT)
        )

        if expiries >= TIMER_EVENT_LIMIT:
            dut.expect_exact(
                "TIMER_EVENTS:TIMER_EVENT_STOPPED: posting to default loop"
            )
        ...
        logging.info(
            "Handled timer expiry event {} out of {}".format(
                expiries, TIMER_EVENT_LIMIT
            )
        )
