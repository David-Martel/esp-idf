...
server_file = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test_certs/server_cert.pem"
)
key_file = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test_certs/server_key.pem"
)


def restart_device_with_random_delay(
    dut: Dut, min_delay: int = 10, max_delay: int = 30
) -> None:
    """
...
def start_redirect_server(
    ota_image_dir: str, server_ip: str, server_port: int, redirection_port: int
) -> None:
    os.chdir(ota_image_dir)
...
def test_examples_protocol_advanced_https_ota_example(dut: Dut) -> None:
    ...
    thread1 = multiprocessing.Process(
        target=start_https_server, args=(dut.app.binary_path, "0.0.0.0", server_port)
    )
    thread1.daemon = True
    thread1.start()
    try:
        ...
        for _ in range(iterations):
            ...
            try:
                ip_address = dut.expect(
                    r"IPv4 address: (\d+\.\d+\.\d+\.\d+)[^\d]", timeout=30
                )[1].decode()
            ...
            print(
                "writing to device: {}".format(
                    "https://" + host_ip + ":" + str(server_port) + "/" + bin_name
                )
            )
...
def test_examples_protocol_advanced_https_ota_example_truncated_bin(dut: Dut) -> None:
    ...
    with open(binary_file, "rb+") as f:
        with open(
            os.path.join(dut.app.binary_path, truncated_bin_name), "wb+"
        ) as output_file:
    ...
    try:
        ...
        print(
            "writing to device: {}".format(
                "https://" + host_ip + ":" + str(server_port) + "/" + truncated_bin_name
            )
        )
        dut.write(
            "https://" + host_ip + ":" + str(server_port) + "/" + truncated_bin_name
...
def test_examples_protocol_advanced_https_ota_example_truncated_header(
    ...
    try:
        ...
        dut.expect(
            "advanced_https_ota_example: esp_https_ota_get_img_desc failed", timeout=30
        )
        try:
            os.remove(binary_file)
        except OSError:
...
def test_examples_protocol_advanced_https_ota_example_random(dut: Dut) -> None:
    ...
    try:
        ...
        print(
            "writing to device: {}".format(
                "https://" + host_ip + ":" + str(server_port) + "/" + random_bin_name
            )
        )
...
def test_examples_protocol_advanced_https_ota_example_chunked(dut: Dut) -> None:
    ...
    try:
        ...
        dut.expect("Starting Advanced OTA example", timeout=30)
        print(
            "writing to device: {}".format("https://" + host_ip + ":8070/" + bin_name)
        )
...
def test_examples_protocol_advanced_https_ota_example_redirect_url(dut: Dut) -> None:
    ...
    dut.expect("Loaded app from partition at offset", timeout=30)
    try:
        ip_address = dut.expect(r"IPv4 address: (\d+\.\d+\.\d+\.\d+)[^\d]", timeout=30)[
            1
        ].decode()
        print("Connected to AP/Ethernet with IP: {}".format(ip_address))
    except pexpect.exceptions.TIMEOUT:
    ...
    host_ip = get_host_ip4_by_dest_ip(ip_address)
    thread1 = multiprocessing.Process(
        target=start_https_server, args=(dut.app.binary_path, host_ip, server_port)
    )
    thread1.daemon = True
    ...
    thread3 = multiprocessing.Process(
        target=start_redirect_server,
        args=(dut.app.binary_path, host_ip, redirection_server_port1, server_port),
    )
    thread3.daemon = True
    thread1.start()
    thread2.start()
    ...
    try:
        ...
        dut.write(
            "https://" + host_ip + ":" + str(redirection_server_port) + "/" + bin_name
        )
        dut.expect("upgrade successful. Rebooting ...", timeout=150)
        # after reboot
        dut.expect("Loaded app from partition at offset", timeout=30)
...
def test_examples_protocol_advanced_https_ota_example_anti_rollback(dut: Dut) -> None:
    ...
    with open(binary_file, "rb+") as f:
        with open(
            os.path.join(dut.app.binary_path, anti_rollback_bin_name), "wb+"
        ) as output_file:
    ...
    try:
        ...
        dut.write(
            "https://" + host_ip + ":" + str(server_port) + "/" + anti_rollback_bin_name
        )
        dut.expect(
...
def test_examples_protocol_advanced_https_ota_example_ota_resumption_partial_download_request(
    dut: Dut,
) -> None:
    """
    This is a positive test case, to test OTA workflow with Range HTTP header.
...
def test_examples_protocol_advanced_https_ota_example_openssl_aligned_bin(
    ...
    with open(binary_file, "rb+") as f:
        with open(
            os.path.join(dut.app.binary_path, aligned_bin_name), "wb+"
        ) as output_file:
    ...
    try:
        ...
        print(
            "writing to device: {}".format(
                "https://" + host_ip + ":8070/" + aligned_bin_name
            )
        )
