...
@pytest.mark.timeout(2400)
@pytest.mark.parametrize(
    "config", ["flash_enc_wifi", "flash_enc_wifi_2"], indirect=True
)
@pytest.mark.parametrize("skip_autoflash", ["y"], indirect=True)
...
def update_partitions(dut: Dut, env_name: Optional[str]) -> None:
    ...
    thread1 = multiprocessing.Process(
        target=start_https_server, args=(dut.app.binary_path, "0.0.0.0", port)
    )
    thread1.daemon = True
    thread1.start()
    try:
...
if __name__ == "__main__":
    if sys.argv[2:]:  # if two or more arguments provided:
        ...
        start_https_server(
            bin_dir, "0.0.0.0", server_port, server_file=server_file, key_file=key_file
        )
