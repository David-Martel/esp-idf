...
def start_https_server(
    ...
    httpd = http.server.HTTPServer(
        (server_ip, server_port), http.server.SimpleHTTPRequestHandler
    )
...
def setting_connection(dut: Dut, env_name: Optional[str] = None) -> Any:
    if (
        ...
        and dut.app.sdkconfig.get("EXAMPLE_WIFI_SSID_PWD_FROM_STDIN") is True
    ):
...
def test_examples_protocol_simple_ota_example(dut: Dut) -> None:
    ...
    thread1 = multiprocessing.Process(
        target=start_https_server, args=(dut.app.binary_path, "0.0.0.0", 8000)
    )
    thread1.daemon = True
    thread1.start()
    try:
        ...
        check_sha256(
            sha256_app,
            str(dut.expect(r"SHA-256 for current firmware:\s+([a-f0-9]){64}")[0]),
        )
...
def test_examples_protocol_simple_ota_example_with_flash_encryption_wifi(
    dut: Dut,
) -> None:
    """
    steps: |
...
def test_examples_protocol_simple_ota_example_with_verify_app_signature_on_update_no_secure_boot_ecdsa(
    ...
    try:
        ...
        dut.write(f"https://{host_ip}:8000/simple_ota.bin")
        dut.expect(
            f"Writing to <ota_1> partition at offset {OTA_1_ADDRESS}", timeout=20
        )
...
if __name__ == "__main__":
    if sys.argv[2:]:  # if two or more arguments provided:
        ...
        port = int(sys.argv[2])
        cert_dir = (
            bin_dir if not sys.argv[3:] else os.path.join(this_dir, sys.argv[3])
        )  # optional argument
