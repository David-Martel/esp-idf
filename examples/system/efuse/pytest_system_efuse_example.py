...
def basic_efuse_example(dut: Dut) -> None:
    ...
    dut.expect(
        [
            ...
            r"example: 1. read MAC address: {}".format(
                r":".join((r"[0-9a-f]{2}",) * 6)
            ),
...
def test_examples_efuse_with_virt_flash_enc(dut: Dut) -> None:
    ...
    logging.info(
        "{}_bootloader_virt_flash_enc_bin_size: {}KB".format(
            dut.app.target, bin_size // 1024
        )
    )
    ...
    if dut.app.target == "esp32":
        dut.expect_exact(
            "flash encryption is enabled (3 plaintext flashes left)", timeout=3
        )
    else:
...
def test_examples_efuse_with_virt_flash_enc_aes_256(dut: Dut) -> None:
    ...
    bin_size = os.path.getsize(binary_file)
    logging.info(
        "{}_bootloader_virt_flash_enc_aes_256_bin_size: {}KB".format(
            dut.app.target, bin_size // 1024
        )
    )
...
def test_examples_efuse_with_virt_flash_enc_release(dut: Dut) -> None:
    ...
    bin_size = os.path.getsize(binary_file)
    logging.info(
        "{}_bootloader_virt_flash_enc_release_bin_size: {}KB".format(
            dut.app.target, bin_size // 1024
        )
    )
    ...
    dut.expect("Checking flash encryption...")
    dut.expect_exact(
        "flash encryption is enabled (0 plaintext flashes left)", timeout=5
    )
...
def test_examples_efuse_with_virt_secure_boot_v1(dut: Dut) -> None:
    ...
    bin_size = os.path.getsize(binary_file)
    logging.info(
        "{}_bootloader_virt_secure_boot_v1_bin_size: {}KB".format(
            dut.app.target, bin_size // 1024
        )
    )
    ...
    dut.expect("Verifying image signature...")
    dut.expect(
        "secure_boot_v1: bootloader secure boot is already enabled. No need to generate digest. continuing.."
    )
    dut.expect("boot: Checking secure boot...")
    dut.expect(
        "secure_boot_v1: bootloader secure boot is already enabled, continuing.."
    )
    dut.expect("Start eFuse example")
    dut.expect("example: Done")
...
def test_examples_efuse_with_virt_secure_boot_v2(dut: Dut) -> None:
    ...
    bin_size = os.path.getsize(binary_file)
    logging.info(
        "{}_bootloader_secure_boot_v2_bin_size: {}KB".format(
            dut.app.target, bin_size // 1024
        )
    )
    ...
    dut.expect("Verifying image signature...")
    dut.expect(
        "secure_boot_v2: Secure boot V2 is not enabled yet and eFuse digest keys are not set"
    )
    ...
    dut.expect_exact(
        "secure_boot_v2: 1 signature block(s) found appended to the bootloader"
    )
...
def test_examples_efuse_with_virt_secure_boot_v2_esp32xx(dut: Dut) -> None:
    ...
    bin_size = os.path.getsize(binary_file)
    logging.info(
        "{}_bootloader_virt_secure_boot_v2_bin_size: {}KB".format(
            dut.app.target, bin_size // 1024
        )
    )
...
def test_example_efuse_with_virt_secure_boot_v2_esp32xx_pre_loaded(dut: Dut) -> None:
    ...
    print(
        " - Flash emul_efuse with pre-loaded efuses (SECURE_BOOT_EN 1 -> 0, SECURE_BOOT_KEY_REVOKE[0..2] -> 0)"
    )
...
def test_examples_efuse_with_virt_sb_v1_and_fe(dut: Dut) -> None:
    ...
    bin_size = os.path.getsize(binary_file)
    logging.info(
        "{}_bootloader_virt_sb_v1_and_fe_bin_size: {}KB".format(
            dut.app.target, bin_size // 1024
        )
    )
    ...
    dut.expect("Checking flash encryption...")
    dut.expect_exact(
        "flash_encrypt: flash encryption is enabled (3 plaintext flashes left)"
    )
    dut.expect("Checking secure boot...")
    dut.expect(
        "secure_boot_v1: bootloader secure boot is already enabled, continuing.."
...
def test_examples_efuse_with_virt_sb_v2_and_fe(dut: Dut) -> None:
    ...
    bin_size = os.path.getsize(binary_file)
    logging.info(
        "{}_bootloader_virt_sb_v2_and_fe_bin_size: {}KB".format(
            dut.app.target, bin_size // 1024
        )
    )
...
def test_examples_efuse_with_virt_sb_v2_and_fe_qemu(dut: QemuDut) -> None:
    try:
        ...
        dut.expect("secure_boot_v2: Secure boot digests absent, generating..")
        dut.expect(
            "secure_boot_v2: Digests successfully calculated, 1 valid signatures"
        )
        dut.expect_exact(
            "secure_boot_v2: 1 signature block(s) found appended to the bootloader"
        )
        ...
        dut.expect_exact(
            "secure_boot_v2: 1 signature block(s) found appended to the app"
        )
        dut.expect_exact(
            "secure_boot_v2: Application key(0) matches with bootloader key(0)"
        )
        ...
        dut.expect_exact(
            "flash_encrypt: Flash encryption mode is DEVELOPMENT (not secure)"
        )
        dut.expect("main_task: Calling app_main()")
        dut.expect("Start eFuse example")
        dut.expect("example: Flash Encryption is NOT in RELEASE mode")
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "test", "esp32eco3_efuses.bin"),
            "wb",
        ) as efuse_file:
