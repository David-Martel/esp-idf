...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            None,
            "-f board/esp32s2-kaluga-1.cfg",
            marks=[pytest.mark.esp32s2, pytest.mark.jtag],
        ),
        pytest.param(
            None,
            "-f board/esp32c2-ftdi.cfg",
            marks=[pytest.mark.esp32c2, pytest.mark.jtag],
        ),
        pytest.param(
            "/dev/serial_ports/ttyACM-esp32",
            "-f board/esp32s3-builtin.cfg",
            marks=[pytest.mark.esp32s3, pytest.mark.usb_serial_jtag],
        ),
        pytest.param(
            "/dev/serial_ports/ttyACM-esp32",
            "-f board/esp32c3-builtin.cfg",
            marks=[pytest.mark.esp32c3, pytest.mark.usb_serial_jtag],
        ),
    ],
    indirect=True,
)
def test_examples_app_trace_basic(dut: IdfDut, openocd: OpenOcd) -> None:
    ...
    assert "Targets connected." in dut.openocd.write(
    ...
    with open("apptrace.log", encoding="utf-8") as apptrace_log:
        for sample_num in range(1, 51):
            log_str = "Apptrace test data[{}]:{}".format(
