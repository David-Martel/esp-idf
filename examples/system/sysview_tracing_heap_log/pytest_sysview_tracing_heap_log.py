...
def test_examples_sysview_tracing_heap_log(idf_path: str, dut: IdfDut) -> None:
    ...
    if dut.target in ["esp32", "esp32s3", "esp32p4"]:
        trace_log.append(
            os.path.join(os.path.dirname(dut.gdb._logfile), "heap_log1.svdat")
    ...
    command = [
        os.path.join(idf_path, "tools", "esp_app_trace", "sysviewtrace_proc.py"),
        "-p",
    ] + trace_log
    ...
    with open(dut.gdb._logfile, encoding="utf-8") as fr:  # pylint: disable=protected-access
        ...
        gdb_pexpect_proc.expect_exact(
            'Thread 2 "main" hit Temporary breakpoint 1, heap_trace_start (mode_param',
            timeout=10,
        )  # should be (mode_param=HEAP_TRACE_ALL) # TODO GCC-329
        gdb_pexpect_proc.expect_exact(
            'Thread 2 "main" hit Temporary breakpoint 2, heap_trace_stop ()', timeout=10
        )
