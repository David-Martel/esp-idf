...
app.layout = html.Div(
    html.Div(
        [
            ...
            dcc.Graph(
                id="live-update-graph", style={"height": 800}
            ),  # Height of the plotting area set to 800px
            dcc.Interval(
...
def update_graph_live(_n: Any) -> Any:  # pylint: disable=undefined-argument
    ...
    fig = make_subplots(
        ...
        subplot_titles=[each_plot["title"] for each_plot in plots],
    )

    for i, each_plot in enumerate(plots, start=1):
        for each_subplot in each_plot["plots"]:
            plot_dict = {
                k: each_subplot[k] for k in each_subplot.keys() - excluded_keys_for_plot
            }
...
def parse_data_and_print(packet: bytes, offset_time: Any) -> None:
    ...
    x_axis_raw_data = int.from_bytes(
        packet[ID_LENGTH : ID_LENGTH + x_axis_data_length], "little"
    )
    ...
    y_axis_data = int.from_bytes(
        packet[
            x_axis_data_length + ID_LENGTH : x_axis_data_length
            + ID_LENGTH
            + y_axis_data_length
        ],
        "little",
            ...
            for key, value in mandatory_keys.items():
                try:
                    ...
                    if not isinstance(val, value):
                        print(
                            "[{}][data_streams][{}][{}] expected {} found {}".format(
                                each_plot,
                                each_subplot,
                                key,
                                value,
                    ...
                    print(
                        "[{}][data_streams][{}][{}] key not found. Aborting".format(
                            each_plot, each_subplot, key
                        )
                    )
                    raise SystemExit(1)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-file", help="Path to program output file in txt format", type=str
    )
