...
def test_esp_timer(dut: Dut) -> None:
    ...
    assert (
        match.group(1).decode("utf8") == "periodic"
        and int(match.group(2)) == INITIAL_TIMER_PERIOD
    )
    match = dut.expect(TIMER_DUMP_LINE_REGEX, timeout=2)
    ...
    logging.info(
        "One-shot timer, time: {} us, diff: {}".format(one_shot_timer_time, diff)
    )
    assert abs(diff) < 400
    ...
    if dut.app.sdkconfig.get("SOC_LIGHT_SLEEP_SUPPORTED"):
        ...
        logging.info(
            "Enter sleep: {}, exit sleep: {}, slept: {}".format(
