...
def test_gcov(dut: IdfDut, openocd: OpenOcd) -> None:
    ...
    os.makedirs(
        os.path.join(
            dut.app.binary_path, "esp-idf", "main", "CMakeFiles", "__idf_main.dir"
        ),
        exist_ok=True,
    )
    os.makedirs(
        os.path.join(
            dut.app.binary_path, "esp-idf", "sample", "CMakeFiles", "__idf_sample.dir"
        ),
        exist_ok=True,
    )
    ...
    def expect_counter_output(loop: int, timeout: int = 10) -> None:
        dut.expect_exact(
            [
                f"blink_dummy_func: Counter = {loop}",
                f"some_dummy_func: Counter = {loop * 2}",
            ],
            expect_all=True,
            timeout=timeout,
        )
