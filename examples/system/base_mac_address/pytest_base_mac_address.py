...
def test_base_mac_address(dut: Dut) -> None:
    ...
    mac_m = dut.expect(
        r'BASE_MAC: Using "' + hex_r + r'" as base MAC address', timeout=5
    ).groups()
    ...
    def get_expected_mac_string(increment: int, target: str) -> str:
        ...
        if target == "esp32s2" and increment == 1:
            ...
            if mac_bytes >= int("7cdfa1003000", 16) and mac_bytes <= int(
                "7cdfa1005fff", 16
            ):
                ...
                return ", ".join(
                    "0x{}".format(hex_string[i : i + 2].lstrip("0"))
                    for i in range(0, len(hex_string), 2)
                )

        return ", ".join(
            ["0x{}".format(m.decode("utf8")) for m in mac_m[:-1]]
            + [hex((int(mac_m[-1], 16) + increment) & 0xFF)]
        )
    ...
    if sdkconfig.get("ESP_WIFI_ENABLED"):
        dut.expect_exact(
            "WIFI_STA MAC: " + get_expected_mac_string(0, dut.target), timeout=2
        )
    ...
    if dut.target != "esp32s2" and dut.target != "esp32h2":
        ...
        if sdkconfig.get("SOC_WIFI_SUPPORTED") or sdkconfig.get(
            "ESP_MAC_ADDR_UNIVERSE_BT"
        ):
            dut.expect_exact("Ethernet MAC: " + get_expected_mac_string(3, dut.target))
        else:
            dut.expect_exact(
                "Ethernet MAC: " + get_expected_mac_string(0, dut.target)
            )  # for esp32p4
