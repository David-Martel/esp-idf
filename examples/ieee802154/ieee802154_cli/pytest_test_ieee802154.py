...
def dlt_pendingtable(ser: IdfDut, addr: str) -> None:
    if addr != "":
        ...
        if len(addr_list) == 2 or len(addr_list) == 8:
            ...
            if len(addr_list) == 2:
                ser.expect(
                    "delete pending shortaddr: {}".format(addr_field), timeout=10
                )
            else:
...
def dlt_short_extern_addr_in_pendingtable(
    ser: IdfDut, table: List, short: int = 5, extern: int = 5
) -> None:
    cnt = 0
    while cnt < short:
...
def test_based_autoack(dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    transmit.write(
        "tx 0x20 0x88 0x00 0x0A 0x28 0xDB 0x6F 0xBC 0x94 0x5A 0x43 0x68 0x02 0xaa 0x15 0x30 0x01 0x02"
    )
    transmit.expect("tx sfd done, Radio state: 4", timeout=10)
...
def test_based_autopending(dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    receive.expect("RX Start", timeout=10)
    transmit.write(
        "tx 0x23 0x08 0x0c 0x66 0x3E 0x70 0x39 0x01 0xef 0x2e 0x9f 0xf3 0xfe 0x75 0x99 0x35 0x66 0xd3"
    )
    transmit.expect("Rx ack 5 bytes", timeout=10)
    ...
    receive.expect("RX Start", timeout=10)
    transmit.write(
        "tx 0x23 0x88 0x00 0x9B 0x2D 0x21 0x3B 0xC3 0xD9 0xEE 0x93 0x04 0xc8 0x05 0xf8 0x1d 0x6e 0xe3"
    )
    transmit.expect("Rx ack 5 bytes", timeout=10)
    transmit.expect("12 00 00", timeout=10)
    ...
    receive.expect("RX Start", timeout=10)
    transmit.write(
        "tx 0x21 0x88 0x10 0x16 0xA1 0x9C 0x93 0x0B 0x9B 0xE4 0x04 0xb8 0xc5 0x63 0x43 0xa7 0x81 0xaf"
    )
    transmit.expect("Rx ack 5 bytes", timeout=10)
    ...
    receive.expect("RX Start", timeout=10)
    transmit.write(
        "tx 0x21 0x88 0x10 0xC9 0xE0 0x25 0x33 0x4F 0x84 0x4C 0x7C 0xa8 0xff 0x51 0x72 0xef 0xc1 0xbb"
    )
    transmit.expect("Rx ack 5 bytes", timeout=10)
