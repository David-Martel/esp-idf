...
def test_examples_sd_card_sdspi(dut: Dut) -> None:
    ...
    sd_card_format = re.compile(
        str.encode("Formatting card, allocation unit size=\\S+")
    )
    message_list2 = (
    ...
    dut.expect(
        sd_card_format, timeout=180
    )  # Provide enough time for SD card FATFS format operation
