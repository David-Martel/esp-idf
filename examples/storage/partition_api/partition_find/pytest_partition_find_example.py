...
def test_partition_find_example(dut: Dut) -> None:
    def expect_partition(name: str, offset: int, size: int) -> None:
        dut.expect(
            re.compile(
                str.encode(
                    "found partition '{}' at offset {:#x} with size {:#x}".format(
                        name, offset, size
                    )
                )
            ),
    ...
    def expect_find_partition(
        _type: str, subtype: str, label: str, name: str, offset: int, size: int
    ) -> None:
        dut.expect(
            re.compile(
                str.encode(
                    "Find partition with type {}, subtype {}, label {}".format(
                        _type, subtype, label
                    )
                )
            ),
