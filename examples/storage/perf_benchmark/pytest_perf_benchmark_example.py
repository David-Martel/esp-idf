...
def test_examples_perf_benchmark_spiflash(dut: Dut) -> None:
    ...
    dut.expect(
        "example: WL layer unmounted", timeout=240
    )  # SPI flash has slow write speed

    dut.expect(
        "example: FATFS mounted to", timeout=90
    )  # Increased timeout due to formatting
    dut.expect(
        "example: FATFS partition unmounted", timeout=240
    )  # SPI flash has slow write speed

    dut.expect(
        "example: SPIFFS mounted to", timeout=90
    )  # Increased timeout due to formatting
    dut.expect(
        "example: SPIFFS partition unmounted", timeout=240
    )  # SPI flash has slow write speed

    dut.expect(
        "example: LittleFS mounted to", timeout=90
    )  # Increased timeout due to formatting
    dut.expect(
        "example: LittleFS partition unmounted", timeout=240
    )  # SPI flash has slow write speed
...
def test_examples_perf_benchmark_sdcard_sdmmc(dut: Dut) -> None:
    ...
    dut.expect(
        "example: SD card mounted - FATFS", timeout=120
    )  # Increased timeout due to formatting
    dut.expect("example: SD card unmounted - FATFS", timeout=180)

    dut.expect(
        "example: SD card mounted - LittleFS", timeout=120
    )  # Increased timeout due to formatting
...
@pytest.mark.esp32c5
@pytest.mark.temp_skip_ci(
    targets=["esp32c61"], reason="C5 C61 GPSPI same, so testing on C5 is enough"
)
