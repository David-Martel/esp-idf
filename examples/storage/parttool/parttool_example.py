...
def main():
    ...
    assert_file_same(
        args.binary, "app.bin", "Device app binary does not match built binary"
    )
    ...
    print(
        "Found data partition at offset 0x{:x} with size 0x{:x}".format(
    ...
    assert_file_same(
        "blank.bin", "read.bin", "Contents of storage partition not fully erased"
    )
