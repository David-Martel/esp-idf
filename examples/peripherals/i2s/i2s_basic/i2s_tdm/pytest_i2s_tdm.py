...
def test_i2s_tdm_example(dut: Dut) -> None:
    ...
    dut.expect(
        r"i2s_tdm: The tx channel on I2S0 has been initialized to TDM mode successfully",
        timeout=5,
    )
    dut.expect(
        r"i2s_common: DMA malloc info: dma_desc_num = ([0-9]+), "
    ...
    dut.expect(
        r"i2s_tdm: The rx channel on I2S0 has been initialized to TDM mode successfully",
        timeout=5,
    )
    chan_enable_pattern = [
