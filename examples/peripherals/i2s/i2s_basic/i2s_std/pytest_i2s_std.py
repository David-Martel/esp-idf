...
def test_i2s_basic_example(dut: Dut) -> None:
    ...
    dut.expect(
        r"i2s_std: The tx channel on I2S0 has been initialized to STD mode successfully",
        timeout=5,
    )
    dut.expect(
        r"i2s_common: DMA malloc info: dma_desc_num = ([0-9]+), "
    ...
    dut.expect(
        r"i2s_std: The rx channel on I2S0 has been initialized to STD mode successfully",
        timeout=5,
    )
    chan_enable_pattern = [
