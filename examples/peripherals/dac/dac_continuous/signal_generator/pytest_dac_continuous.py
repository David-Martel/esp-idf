...
def test_dac_continuous_output(
    dut: Dut, mode: str, chan0_io: str, chan1_io: str
) -> None:
    dut.expect(
        "dac continuous: --------------------------------------------------", timeout=10
    )
    dut.expect("dac continuous: DAC continuous output by {}".format(mode), timeout=10)
    dut.expect(
        "dac continuous: DAC channel 0 io: GPIO_NUM_{}".format(chan0_io), timeout=10
    )
    dut.expect(
        "dac continuous: DAC channel 1 io: GPIO_NUM_{}".format(chan1_io), timeout=10
    )
    dut.expect(
        "dac continuous: Waveform: SINE -> TRIANGLE -> SAWTOOTH -> SQUARE", timeout=10
    )
    dut.expect(
        "dac continuous: DAC conversion frequency \\(Hz\\): ([0-9]+)", timeout=10
    )
    ...
    dut.expect(
        r"DAC channel 0 value:( +)(\d+)(.*)DAC channel 1 value:( +)(\d+)", timeout=10
    )
