...
def dump_binary_raw_data_to_vcd(
    chan_num: int, raw_in_io: FileIO, vcd_out_io: TextIOWrapper
) -> None:
    vcd = VCDDumper()
...
def dump_vcd_main() -> None:
    ...
    parser.add_argument(
        "-n", "--chan-num", type=int, help="The channel number that probed"
    )
    parser.add_argument(
        "-i",
        "--input-file",
        type=argparse.FileType("rb"),
        help="The the input raw data file",
    )
    parser.add_argument(
        "-o",
        "--output-file",
        type=argparse.FileType("w"),
        help="The output vcd file name (optional)",
        default=None,
    )
