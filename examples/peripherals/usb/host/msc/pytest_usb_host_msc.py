...
@pytest.mark.temp_skip_ci(
    targets=["esp32s2"], reason="lack of runners with usb_host_flash_disk tag"
)
...
def test_usb_host_msc_example(dut: Dut) -> None:
    ...
    write_throughput = float(
        dut.expect(r"example: Write speed ([0-9]*[.]?[0-9]+) MiB")[1].decode()
    )
    read_throughput = float(
        dut.expect(r"example: Read speed ([0-9]*[.]?[0-9]+) MiB")[1].decode()
    )
