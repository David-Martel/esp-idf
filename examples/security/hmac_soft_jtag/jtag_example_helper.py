...
def main() -> None:
    ...
    hmac_generator_parser.add_argument(
        "hmac_key_file", help="File to store generated HMAC key"
    )

    token_generator_parser = subparsers.add_parser("generate_token")
    token_generator_parser.add_argument(
        "hmac_key_file", help="File containing the HMAC key"
    )
    token_generator_parser.add_argument(
        "output_file", nargs="?", help="File to store the generated token (optional)"
    )
