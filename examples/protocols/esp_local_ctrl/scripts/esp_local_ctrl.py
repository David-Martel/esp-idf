...
async def set_property_values(
    tp, security_ctx, props, indices, values, check_readonly=False
):
    try:
        if check_readonly:
...
async def main():
    ...
    obj_security = get_security(
        args.secver, args.sec2_usr, args.sec2_pwd, args.pop, args.verbose
    )
    ...
    while True:
        ...
        if not await set_property_values(
            obj_transport, obj_security, properties, set_indices, set_values
        ):
