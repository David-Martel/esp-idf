...
class CustomProcess(object):
    def __init__(self, cmd: str, logfile: str, verbose: bool = True) -> None:
        ...
        self.pexpect_proc = pexpect.spawn(
            cmd, timeout=60, logfile=self.f, encoding="utf-8", codec_errors="ignore"
        )
    ...
    with CustomProcess(cmd, esp_local_ctrl_log) as ctrl_py:
        ...
        def expect_properties(prop1: int, prop3: str) -> None:
            ...
            ctrl_py.pexpect_proc.expect(
                re.compile(r"\[ 1\] timestamp \(us\)\s+TIME\(us\)\s+Read-Only\s+\d+")
            )
            ctrl_py.pexpect_proc.expect(
                re.compile(r"\[ 2\] property1\s+INT32\s+{}".format(prop1))
            )
            ctrl_py.pexpect_proc.expect(
                re.compile(r"\[ 3\] property2\s+BOOLEAN\s+Read-Only\s+(True)|(False)")
            )
            ctrl_py.pexpect_proc.expect(
