...
def _run_test(dut: Dut) -> None:
    ...
    ip = dut.expect(
        r"64 bytes from (\d+\.\d+\.\d+\.\d+) icmp_seq=\d ttl=\d+ time=\d+ ms"
    )[1].decode()
...
def test_protocols_icmp_echo_ipv6_only(dut: Dut) -> None:
    ...
    interface_nr = dut.expect(
        r"Connected on interface: [a-z]{2}\d \((\d+)\)", timeout=30
    )[1].decode()
    ...
    # expect at least two packets (there could be lost packets)
    ip = dut.expect(r"64 bytes from ([0-9a-fA-F:]+) icmp_seq=\d ttl=\d+ time=\d+ ms")[
        1
    ].decode()
    dut.expect(rf"64 bytes from {ip} icmp_seq=[2-5] ttl=\d+ time=")
