...
def start_https_server(
    server_file: str, key_file: str, server_ip: str, server_port: int
) -> None:
...
def test_examples_protocol_https_request_cli_session_tickets(dut: Dut) -> None:
    ...
    server_file = os.path.join(
        os.path.dirname(__file__), "main", "local_server_cert.pem"
    )
    key_file = os.path.join(os.path.dirname(__file__), "main", "local_server_key.pem")
    thread1 = multiprocessing.Process(
        target=start_https_server, args=(server_file, key_file, "0.0.0.0", server_port)
    )
    thread1.daemon = True
    thread1.start()
...
def test_examples_protocol_https_request_dynamic_buffers(dut: Dut) -> None:
    ...
    dut.expect("Loaded app from partition at offset", timeout=30)
    try:
        ip_address = dut.expect(r"IPv4 address: (\d+\.\d+\.\d+\.\d+)[^\d]", timeout=60)[
            1
        ].decode()
        print("Connected to AP/Ethernet with IP: {}".format(ip_address))
    except pexpect.exceptions.TIMEOUT:
...
def test_examples_protocol_https_request(dut: Dut) -> None:
    ...
    try:
        ...
        logging.info(
            'Failed the test for "https_request using server supported ciphersuites"'
        )
        raise
    logging.info(
        'Passed the test for "https_request using server supported ciphersuites"'
    )
    ...
    try:
        ...
        logging.info(
            'Failed the test for "https_request using server unsupported ciphersuites"'
        )
        raise
    logging.info(
        'Passed the test for "https_request using server unsupported ciphersuites"'
    )
