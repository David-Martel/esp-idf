...
def send_recv_eth_frame(
    payload_str: str, eth_type: int, dest_mac: str, eth_if: str = ""
) -> str:
    with configure_eth_if(eth_type, eth_if) as so:
        so.settimeout(10)
        eth_frame = Ether(dst=dest_mac, src=so.getsockname()[4], type=eth_type) / raw(
            payload_str.encode()
        )
        try:
            ...
            if eth_frame_repl.type == eth_type:
                logging.info(
                    "Received %d bytes echoed from %s",
                    len(eth_frame_repl),
                    eth_frame_repl.src,
                )
