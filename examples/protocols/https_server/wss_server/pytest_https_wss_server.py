                ...
                try:
                    ...
                    if opcode == OPCODE_TEXT:
                        if data == CORRECT_ASYNC_DATA:
                            ...
                            logging.info(
                                "Thread {} obtained correct async message".format(
                                    self.name
                                )
                            )
...
def test_examples_protocol_https_wss_server(dut: Dut) -> None:
    ...
    got_port = int(
        dut.expect(r"Server listening on port (\d+)", timeout=30)[1].decode()
    )
    ...
    with WsClient(got_ip, int(got_port), ca_file) as ws:
        ...
        client_fd = int(
            dut.expect(r"New client connected (\d+)", timeout=30)[1].decode()
        )
        ...
        while True:
            try:
                ...
                if opcode == OPCODE_PING:
                    ...
                    logging.info(
                        "Received PING, replying PONG (to update the keepalive)"
                    )
                # Keep sending pong to update the keepalive in the server
    ...
    logging.info(
        "Testing for multiple simultaneous client connections (approx time = 30s)"
    )
