...
def mqqt_server_sketch(my_ip, port):  # type: (str, str) -> None
    ...
    try:
        ...
        print(
            ...
            "to accept connections on this address".format(
                my_ip, port, sys.exc_info()[0]
            )
        )
        raise
    data = q.recv(1024)
...
def test_examples_protocol_mqtt_qos1(dut: Dut) -> None:
    ...
    msgid_enqueued = (
        dut.expect(b"outbox: ENQUEUE msgid=([0-9]+)", timeout=30).group(1).decode()
    )
    msgid_deleted = (
        dut.expect(b"outbox: DELETED msgid=([0-9]+)", timeout=30).group(1).decode()
    )
