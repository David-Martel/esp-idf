...
def on_message(client, userdata, msg):  # type: (mqtt.Client, tuple, mqtt.client.MQTTMessage) -> None
    ...
    if msg.topic == "/topic/binary":
        ...
        print(
            "Receiving binary from esp and comparing with {}, size {}...".format(
                binary, bin_size
            )
        )
        with open(binary, "rb") as f:
            ...
            raise ValueError(
                "Received binary (saved as: {}) does not match the original file: {}".format(
                    recv_binary, binary
                )
            )
...
def test_examples_protocol_mqtt_ssl(dut):  # type: (Dut) -> None
    ...
    try:
        value = re.search(
            r"\:\/\/([^:]+)\:([0-9]+)", dut.app.sdkconfig.get("BROKER_URI")
        )
    ...
    try:
        ...
        if not event_client_connected.wait(timeout=30):
            raise ValueError(
                "ENV_TEST_FAILURE: Test script cannot connect to broker: {}".format(
                    broker_url
                )
            )
        try:
