...
try:
    ...
    print(
        "Please install 'websocket' by running 'python -m pip install websocket-client'"
    )
    raise
    ...
    with WsClient(got_ip, int(got_port)) as ws:
        ...
        for expected_opcode in [OPCODE_TEXT, OPCODE_BIN, OPCODE_PING]:
            ...
            if (
                ...
                or data != DATA
                or opcode != int(dut_opcode)
                or (data not in str(dut_data))
            ):
                raise RuntimeError(
                    "Failed to receive correct opcode:{} or data:{}".format(
                        opcode, data
        ...
        opcode, data = ws.read()
        logging.info(
            "Testing async send: Received opcode:{}, data:{}".format(opcode, data)
        )
        data = data.decode()
        if opcode != OPCODE_TEXT or data != "Async data":
            raise RuntimeError(
                "Failed to receive correct opcode:{} or data:{}".format(opcode, data)
            )
