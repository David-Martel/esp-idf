...
try:
    ...
    wifi_tools_path = (
        str(os.getenv("IDF_PATH")) + "/examples/provisioning/softap_prov/utils"
    )
...
def test_captive_page(ip: str, port: str, uri: str) -> bool:
    ...
    if search_str not in resp_data:
        raise RuntimeError(
            "Failed to match {} with data from captive portal: {}".format(
                search_str, resp_data
            )
        )
...
def test_example_captive_portal(dut: Dut) -> None:
    ...
    wifi_info = dut.expect(
        r"wifi_init_softap finished. SSID:'(\S+)' password:'(\S+)'", timeout=30
    )
    ...
    try:
        ...
        try:
            ...
            if ip != got_ip:
                raise RuntimeError(
                    "SoftAP connected to another host! {} != {}".format(ip, got_ip)
                )
