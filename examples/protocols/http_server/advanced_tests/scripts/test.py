...
class adder_thread(threading.Thread):
    ...
    def adder_result(self):
        ...
        for i in range(len(self.response)):
            if not test_val(
                "Thread" + str(self.id) + " response[" + str(i) + "]",
                str(self.id * (i + 1)),
                str(self.response[i]),
            ):
                return False
...
def get_test_headers(dut, port):
    ...
    for val in header2_values:
        ...
        if hdr_val_start_idx == -1:
            ...
            "header: Header2", val[hdr_val_start_idx:], resp.read().decode()
        ):
            conn.close()
            return False
        resp.read()
...
def get_hello_type(dut, port):
    ...
    Utility.console_log(
        "[test] GET /hello/type_html has Content-Type of text/html =>", end=" "
    )
    conn = http.client.HTTPConnection(dut, int(port), timeout=15)
...
def parallel_sessions_adder(dut, port, max_sessions):
    ...
    Utility.console_log(
        "[test] POST {pipelined} on /adder in " + str(max_sessions) + " sessions =>",
        end=" ",
    )
    t = []
    # Create all sessions
...
def leftover_data_test(dut, port):
    ...
    Utility.console_log(
        "[test] Leftover data in POST is purged (valid and invalid URIs) =>", end=" "
    )
...
def spillover_session(dut, port, max_sess):
    ...
    Utility.console_log(
        "[test] Session max_sess_sessions (" + str(max_sess) + ") + 1 is rejected =>",
        end=" ",
    )
    s = []
    _verbose_ = True
    for i in range(max_sess + 1):
        ...
        try:
            ...
            resp = a.getresponse()
            if not test_val(
                "Connection " + str(i), "Hello World!", resp.read().decode()
            ):
                a.close()
                break
            s.append(a)
...
def send_postx_hdr_len(dut, port, length):
    ...
    custom_hdr_field = "\r\nCustom: "
    custom_hdr_val = "x" * (
        length - len(host) - len(custom_hdr_field) - len("\r\n\r\n") + len("0")
    )
    request = (
        "POST "
        + path
