...
def test_examples_udp_server_ipv4(dut: Dut) -> None:
    ...
    ipv4 = dut.expect(r"IPv4 address: (\d+\.\d+\.\d+\.\d+)[^\d]", timeout=30)[
        1
    ].decode()
    ...
    for _ in range(MAX_RETRIES):
        ...
        raise ValueError(
            "IPv4: Did not receive UDP message after {} retries".format(MAX_RETRIES)
        )
    dut.expect(MESSAGE)
...
def test_examples_udp_server_ipv6(dut: Dut) -> None:
    ...
    for _ in range(MAX_RETRIES):
        ...
        raise ValueError(
            "IPv6: Did not receive UDP message after {} retries".format(MAX_RETRIES)
        )
    dut.expect(MESSAGE)
