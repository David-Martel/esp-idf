...
def tcp_client(address: str, port: int, payload: str) -> str:
    for res in socket.getaddrinfo(
        address, port, socket.AF_UNSPEC, socket.SOCK_STREAM, 0, socket.AI_PASSIVE
    ):
...
def main() -> None:
    ...
    parser.add_argument(
        "remote_ip", help="TCP server ip address, eg: 192.168.1.1, fe80::2%%eth0"
    )
    parser.add_argument("--port", default=DEF_PORT, type=int, help="TCP server port")
    parser.add_argument(
        "--message", default=DEF_MESSAGE, help="Message to send to the server."
    )
