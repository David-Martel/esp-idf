...
def test_get_time_from_sntp_server(dut: Dut) -> None:
    ...
    TIME_DIFF = datetime.timedelta(
        seconds=10 + 2
    )  # cpu spends 10 seconds in deep sleep
    NY_time = None
    SH_time = None

    def check_time(prev_NY_time: Any, prev_SH_time: Any) -> Tuple[Any, Any]:
        NY_str = dut.expect(
            r"The current date/time in New York is: ({})".format(TIME_FORMAT_REGEX)
        )[1].decode()
        SH_str = dut.expect(
            r"The current date/time in Shanghai is: ({})".format(TIME_FORMAT_REGEX)
        )[1].decode()
