...
def test_bt_spp_only(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    if "ESP_BT_GAP_KEY_REQ_EVT Please enter passkey!" in str(initiator_output):
        passkey = (
            acceptor.expect(r"ESP_BT_GAP_KEY_NOTIF_EVT passkey:(\d+)")
            .group(1)
            .decode("utf8")
        )
...
def test_bt_a2dp(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    source_dut = dut[1]
    source_dut_mac = (
        source_dut.expect(r"Bluetooth MAC: (([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2})")
        .group(1)
        .decode("utf8")
    )
    ...
    sink_dut.expect_exact(
        f"A2DP connection state: Connected, [{source_dut_mac}]", timeout=30
    )
...
def test_bt_hid(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    hid_host_mac = (
        hid_host.expect(r"Bluetooth MAC: (([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2})")
        .group(1)
        .decode("utf8")
    )
    hid_device_mac = (
        hid_device.expect(r"Bluetooth MAC: (([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2})")
        .group(1)
        .decode("utf8")
    )
    ...
    hid_device.expect_exact(
        "making self non-discoverable and non-connectable", timeout=30
    )
