...
def test_gatt_func(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    gatt_client_addr = (
        gatt_client.expect(
            r"Bluetooth MAC: (([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2})", timeout=30
        )
        .group(1)
        .decode("utf8")
    )
    ...
    gatt_client.expect_exact(
        f"Connected, conn_id 0, remote {gatt_server_addr}", timeout=30
    )
    gatt_server.expect_exact(
        f"Connected, conn_id 0, remote {gatt_client_addr}", timeout=30
    )
    ...
    assert "rst:" not in str(gatt_client_output) and "boot:" not in str(
        gatt_client_output
    )
    assert "rst:" not in str(gatt_server_output) and "boot:" not in str(
        gatt_server_output
    )
...
def test_c2_26mhz_xtal_gatt_func(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    gatt_server = dut[0]
    gatt_client_addr = (
        gatt_client.expect(r"Bluetooth MAC: (([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2})")
        .group(1)
        .decode("utf8")
    )
    gatt_server_addr = (
        gatt_server.expect(r"Bluetooth MAC: (([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2})")
        .group(1)
        .decode("utf8")
    )
    gatt_client.expect_exact("GATT client register, status 0", timeout=30)
...
def test_gatt_security_func(
    app_path: str, dut: Tuple[IdfDut, IdfDut], target: Tuple
) -> None:
    gatt_security_client = dut[1]
    gatt_security_server = dut[0]
    gatt_security_client_addr = (
        gatt_security_client.expect(
            r"Bluetooth MAC: (([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2})", timeout=30
        )
        .group(1)
        .decode("utf8")
    ...
    if target == ("esp32", "esp32"):
        ...
        gatt_security_server.expect_exact(
            f"Connected, conn_id 0, remote {gatt_security_client_addr}", timeout=30
        )
    gatt_security_client.expect_exact(
        "Key exchanged, key_type ESP_LE_KEY_PID", timeout=30
    )
    gatt_security_client.expect_exact(
        "Key exchanged, key_type ESP_LE_KEY_LENC", timeout=30
    )
    gatt_security_client.expect_exact(
        "Key exchanged, key_type ESP_LE_KEY_PENC", timeout=30
    )
    gatt_security_client.expect_exact(
        "Key exchanged, key_type ESP_LE_KEY_LID", timeout=30
    )
    ...
    if target == ("esp32", "esp32"):
        gatt_security_server.expect_exact(
            "Authentication complete, addr_type 1, addr ", timeout=30
        )
    else:
        gatt_security_server.expect_exact(
            f"Authentication complete, addr_type 0, addr {gatt_security_client_addr}",
            timeout=30,
        )
    gatt_security_client.expect_exact(
        "Authentication complete, addr_type 1, addr ", timeout=30
    ...
    gatt_security_client_output = gatt_security_client.expect(
        pexpect.TIMEOUT, timeout=10
    )
    gatt_security_server_output = gatt_security_server.expect(
        pexpect.TIMEOUT, timeout=10
    )
    assert "rst:" not in str(gatt_security_client_output) and "boot:" not in str(
        gatt_security_client_output
    )
    assert "rst:" not in str(gatt_security_server_output) and "boot:" not in str(
        gatt_security_server_output
    )
...
def test_c2_26mhz_xtal_gatt_security_func(
    app_path: str, dut: Tuple[IdfDut, IdfDut]
) -> None:
    gatt_security_client = dut[1]
    gatt_security_server = dut[0]
    gatt_security_client_addr = (
...
def test_ble_ibeacon_func(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    ibeacon_receiver.expect_exact(
        f"IBEACON_DEMO: Device address: {ibeacon_sender_addr}", timeout=30
    )
...
def test_c2_26mhz_xtal_gatt_config_func(
    app_path: str, dut: Tuple[IdfDut, IdfDut]
) -> None:
    gatt_client = dut[1]
    gatt_server = dut[0]
...
@pytest.mark.parametrize(
    "config, app_path",
    [("init_deinit", f"{os.path.join(os.path.dirname(__file__), 'gatt_client')}")],
    indirect=True,
)
def test_bluedroid_host_init_deinit(dut: Dut) -> None:
    ...
    for _ in range(20):
        all_hp.append(
            int(dut.expect(r"Free memory: (\d+) bytes").group(1).decode("utf8"))
        )
