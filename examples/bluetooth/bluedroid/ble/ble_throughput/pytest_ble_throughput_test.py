...
def test_gatt_write_throughput(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    while time.time() - start_time < 30:
        throughput = int(
            server.expect(r"GATTC write Bit rate = (\d+) Byte/s", timeout=30)
            .group(1)
            .decode("utf8")
        )
...
def test_c2_26mhz_xtal_write_throughput(
...
def test_gatt_notify_throughput(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    while time.time() - start_time < 30:
        throughput = int(
            client.expect(r"Notify Bit rate = (\d+) Byte/s", timeout=30)
            .group(1)
            .decode("utf8")
        )
        assert throughput > 50000 or throughput < 95000
...
def test_c2_26mhz_xtal_notify_throughput(
    app_path: str, dut: Tuple[IdfDut, IdfDut]
) -> None:
    server = dut[0]
    client = dut[1]
