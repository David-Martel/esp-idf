...
def test_c2_26mhz_xtal_ble50_security_func(
...
def test_period_adv_sync_func(app_path: str, dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    adv_dut.expect_exact(
        "Extended advertising random address set, status 0", timeout=30
    )
...
def test_c2_26mhz_xtal_period_adv_sync_func(
    app_path: str, dut: Tuple[IdfDut, IdfDut]
) -> None:
    adv_dut = dut[0]
    sync_dut = dut[1]
...
def test_c2_26mhz_xtal_ble50_security_config_func(
    app_path: str, dut: Tuple[IdfDut, IdfDut]
) -> None:
    server = dut[0]
    client = dut[1]
